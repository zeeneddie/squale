...
ContextMenu.prototype.getContextMenu = function(target) {
	...
	if (selection)
		menu.push([ i18n["Cut"], function() { self.applyTo.execCommand("cut"); }, null, config.btnList["cut"][1] ],
			  [ i18n["Copy"], function() { self.applyTo.execCommand("copy"); }, null, config.btnList["copy"][1] ]);
	menu.push([ i18n["Paste"], function() { self.applyTo.execCommand("paste"); }, null, config.btnList["paste"][1] ]);
	...
	for (; target; target = target.parentNode) {
		...
		switch (tag) {
			...
			elmenus.push(null,
				     [ i18n["Modify Link"],
				       function() { self.applyTo.execCommand("createlink", true); },
			...
			elmenus.push(null,
				     [ i18n["Justify Left"],
				       function() { self.applyTo.execCommand("justifyleft"); }, null,
				       config.btnList["justifyleft"][1] ],
				     [ i18n["Justify Center"],
				       function() { self.applyTo.execCommand("justifycenter"); }, null,
				       config.btnList["justifycenter"][1] ],
				     [ i18n["Justify Right"],
				       function() { self.applyTo.execCommand("justifyright"); }, null,
				       config.btnList["justifyright"][1] ],
				     [ i18n["Justify Full"],
				       function() { self.applyTo.execCommand("justifyfull"); }, null,
	...
	if (!/html|body|div|p|span/i.test(currentTarget.tagName)) {
