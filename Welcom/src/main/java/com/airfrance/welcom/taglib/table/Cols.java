...
public class Cols {
     ...
     */
    public String tabletrCorps(final int i, final Object o, final Integer pIdIndex) throws JspException {
			...
			if (col instanceof ColSelect){
				...
				if (! GenericValidator.isBlankOrNull(colSelect.getProperty())){
					try {
						...
						log.error(e, e);
						throw new JspException("Impossible de recupere la clef " + colSelect.getProperty() + " sur le bean");
					}					
				}
			}
     ...
     * @throws JspException
     */
    private String writeLegacyCheckBox(final Object o, final Integer pIdIndex, String style, String styleselect, String currentStyle, String wdt) throws JspException {
        final TrimStringBuffer sb= new TrimStringBuffer();
        ...
        if (isSelectTr()) {
            ...
            if (!enableSingleSelect) {
                ...
                sb.append("<input class=\"" + currentStyle + "\" type=\"checkbox\" name=\"" + name + "\" value=\"" + key + "\" onclick='check(this);'");
                sb.append("></td>");
            } else {
                sb.append(
                    "<td"
            ...
            if (o instanceof WISelectable) {
                ...
                if (!enableSingleSelect) {
                    ...
                    if (Util.isFalse(WelcomConfigurator.getMessage(WelcomConfigurator.OPTIFLUX_AUTORESET_CHECKBOX))) {
                        sb.append("<input type=\"hidden\" name=\"checkName\" value=\"" + name + "[" + pIdIndex + "].selected" + "\">");
                    }

                    sb.append("<input class=\"" + currentStyle + "\" type=\"checkbox\" name=\"" + name + "[" + pIdIndex + "].selected" + "\" value=\"true\" onclick='check(this);'");
                    ...
                    sb.append(
                        wdt
                            ...
                            + pIdIndex
                            + "].selected"
                            + "\" value=\"true\" onclick='checkSingle(this);'");
     ...
     */
    private String writeColumnsValue(final Object o, final int index, final String style, final int pIdIndex, final String styleSelect) {
        final TrimStringBuffer sb = new TrimStringBuffer();
     ...
     * @param col Colonne
     */
    private void writeColumnValue(final Object o, final int index, final String style, final int pIdIndex, final String styleSelect, final TrimStringBuffer sb, final Col col) {
        internalValue = "";
        if (col.isWriteTD()) {
            sb.append("\t\t<td ");
     ...
     * @param o object
     * @return retoune comment est ecrit la ligne 
     * si l'objet vide, s'il est null on met chartev2.default.char.if.empty
     */
                ...
                if ((col.getType() != null) && (col.getType().equals(ColComparator.TYPE_DATE))) {
                    if ((ovalue instanceof Date) || (((DateUtil.parseAllDate(internalValue))) != null)) {
                        ...
                        if (col.getDateFormatKey() != null) {
                            df = new SimpleDateFormat(table.getResources().getMessage(table.getLocaleRequest(), col.getDateFormatKey()),table.getLocaleRequest());
    ...
    * @return html correspondant a la cellule : objet o colonne col
     */
    private String writeContentColumnValue(final Object o, final Col col, final int position, final int index, final String style, final String styleSelect) {
        try {
            ...
            if ((o instanceof WIEditable) && ((WIEditable) o).isEdited() && col.isEditable()) {
                ...
                internalValue = InternalTableUtil.getTruncatedString(internalValue, col.getContentTruncate(), col.getType());
