 ...
 * FenÛtre&gt;PrÚfÚrences&gt;Java&gt;GÚnÚration de code&gt;Code et commentaires
 */
public class ColsTag extends BodyTagSupport {
    ...
    */
    ...
    public int doAfterBody() throws JspException {
        ...
        if (firstIterate) {
            if (((collectionForMemory != null) && !collectionForMemory.isEmpty()) || ((collectionForMemory != null) && collectionForMemory.isEmpty() && !GenericValidator.isBlankOrNull(emptyKey))) {
                ...
                if (tableTag.getScrollHeight() != 0) {
                    if (!(collectionForDisplay.isEmpty()) || (collectionForDisplay.isEmpty() && !GenericValidator.isBlankOrNull(emptyKey))) {
                        ...
                        ResponseUtils.write(pageContext, "<div style=\"height:" + tableTag.getScrollHeight() + "px;overflow:auto;\">");
        ...
        if (collectionForMemory.isEmpty() && !GenericValidator.isBlankOrNull(emptyKey)) {
            final String myClassLignePaire = WelcomConfigurator.getMessage(WelcomConfigurator.getCharte().getWelcomConfigFullPrefix() + ".cols.even");
        ...
        initCurrentLine();
        sb.append(currentLine.tabletrCorps(idCount, collectionForDisplay.elementAt(idCount), (Integer) relationDisplayIdMemory.get(collectionForDisplay.elementAt(idCount))));
        ResponseUtils.write(pageContext, sb.toString());
        ...
        if (!(collectionForMemory instanceof HTMLTable)) {
            ...
            if ((collectionForDisplay.size() > idCount) && (idCount < (((HTMLTable) collectionForMemory).getLength() + ((HTMLTable) collectionForMemory).getFrom()))) {
                final Object o = collectionForDisplay.elementAt(idCount);
                pageContext.setAttribute(getId(), o, PageContext.PAGE_SCOPE);
                pageContext.setAttribute(getIdIndex(), (Integer) relationDisplayIdMemory.get(o), PageContext.PAGE_SCOPE);
    ...
    public String getFormName() {
        ...
        //        Tag curParent = null;
        //
        //        for (curParent = getParent();(curParent != null) && !(curParent instanceof FormTag); curParent = curParent.getParent()) {
        //          ;
        //        }
        //
        //        if (curParent != null) {
    ...
    */   
    ...
    private String getSortedTitre(ColHeader colHeader) {
        ...
        // Url de trie
        String urlSort = colHeader.getSortUrl(tableTag, getServletName(), ((HTMLTable) collectionForMemory).getFrom());
