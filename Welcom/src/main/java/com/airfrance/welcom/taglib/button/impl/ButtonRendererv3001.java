...
public class ButtonRendererv3001
	...
	private final static String COULEUR_BLUE_AF =
		WelcomConfigurator.getMessage(
			WelcomConfigurator.CHARTEV3_BOUTON_BLUEAF);
	/** le blanc */
	...
	private String getDirection(final String name, final String type) {
		if (Util.isEquals("form", type)) {
			String direction =
				WelcomConfigurator.getMessage(
					"chartev3.bouton." + type + "." + name + ".alignement");

			if (GenericValidator.isBlankOrNull(direction)) {
				direction =
					WelcomConfigurator.getMessage(
						"chartev3.bouton." + name + ".alignement");
			}
	...
	private String getPathPicto(
		...
		String picto =
			WelcomConfigurator.getMessage(
				"chartev3.bouton." + type + "." + name + ".icone",
				couleur);
	 ...
	 * <li><a href="#" style="BACKGROUND-IMAGE: url(http://cmsintranet.airfrance.fr/charte_v02_002/img/ico/neutre/print.gif)">imprimer</a></li>
	 */
	public String drawRenderMenuHRefTag(
		...
		results.append(
			" style=\"BACKGROUND-IMAGE:url("
				+ getPathPicto(parentTag, name, "menu")
				+ ")\"");
		results.append(" >");
