...
public class ButtonTag extends BodyTagSupport {
	...
	/** render */
	private static IButtonRenderer render =
		(IButtonRenderer) RendererFactory.getRenderer(RendererFactory.BUTTON);
	...
	public int doEndTag() throws JspException {
		/*
		    ...
		    resources = (MessageResources) pageContext.getServletContext().getAttribute(
		...
		for (curParent = getParent();
			(curParent != null)
				&& !((curParent instanceof CanvasLeftMenuTag)
					|| (curParent instanceof TableBottomTag)
					|| (curParent instanceof JSOngletBottomTag)
					|| (curParent instanceof ButtonBarTag)
					|| (curParent instanceof FormulaireBottomTag));
		...
		if ((curParent instanceof JSOngletBottomTag)
			|| (curParent instanceof TableBottomTag)
			|| (curParent instanceof ButtonBarTag)
			|| (curParent instanceof FormulaireBottomTag)) {
			...
			if (WelcomConfigurator.getCharte() == Charte.V2_002) {
				...
				if (Util.isTrimNonVide(bouttonBody)) {
					final String pix_grey =
						WelcomConfigurator.getMessage(
							WelcomConfigurator.CHARTEV2_PIX_GREY_GIF);
					results.append("<tr><td><img src=\"");
					results.append(pix_grey);
					results.append(
						"\" width=\"140\" height=\"1\" border=\"0\"></td></tr>\n");
				}
			} else {
				if (Util.isTrimNonVide(bouttonBody)) {
					results.append("<ul>");
	 ...
	 */
	private String getBouttonBody() throws JspException {
		// Recupere le droit sur la page
		final StringBuffer results = new StringBuffer();
		...
		boolean myForceReadWrite =
			forceReadWrite
				|| (pageContext.getAttribute("access") == null
					&& Util.isFalse(
						WelcomConfigurator.getMessage(
							WelcomConfigurator.ACCESS_KEY_USE_LEGACY)));
		String computedAccess =
			Access.computeTagReadWriteAccess(
				pageContext,
				accessKey,
				myForceReadWrite,
	...
	private String getFormButtonBody(String computedAccess)
		...
		if (READWRITE.equals(computedAccess)) {
			...
			if ((GenericValidator.isBlankOrNull(causesValidation)
				&& checkIfInDefaultCausesValidation(name))
				|| Util.isTrue(
					causesValidation)) //if (name.equals("rechercher") || name.equals("valider"))
				{
				final FormTag ftag = getFormTag();
				if (ftag != null) {
					if (GenericValidator
						.isBlankOrNull(ftag.getDefaultAction())) {
						ftag.setDefaultAction(callMethod);
					}
				}
				results =
					render.drawRenderFormInputTag(
						pageContext,
						curParent,
						getName(),
						bb,
				...
				if (pageContext.getRequest().getAttribute("org.apache.struts.taglib.html.FORM") != null) {
					results =
						render.drawRenderFormHRefTag(pageContext, curParent, getName(), bb, "", getFormOnclick(), getToolTip(), styleId);
				} else {
					String onClic = getHrefOnClick();
					results =
						render.drawRenderHRefTag(pageContext, curParent, getName(), bb, "", getOnclick(), onClic, getToolTip(), styleId);
				}
			}
		}
	...
	private String getHrefOnClick() {
		if (!GenericValidator.isBlankOrNull(progressbarId)) {
			if ((getOnclick() != null)
				&& (getOnclick().indexOf("javascript") == -1)
				&& (!getOnclick().equals("#"))) {
	 ...
	 */
	private String getToolTip() throws JspException {
	 ...
	 * @throws JspException exception pouvant etre levee
	 */
	private String getFormOnclick() throws JspException {
		final StringBuffer buf = new StringBuffer();
		doFormOnclick(buf);
	...
	private void doFormOnclick(final StringBuffer results)
		...
		String messageConfirmation =
			WRequestUtils.message(super.pageContext, messageConfirmationKey);
		...
		if (!GenericValidator.isBlankOrNull(messageConfirmationKey)
			...
			results.append(
				"setMsg('"
					+ Util.formatJavaScript(messageConfirmation)
					+ "');");
		} else if (
			!GenericValidator.isBlankOrNull(messageConfirmationCallBack)) {
			results.append(
				"setMsg(getRemoteMessage('"
					+ Util.formatJavaScript(messageConfirmationCallBack)
					+ "'));");
		}
		...
		if (!GenericValidator.isBlankOrNull(callMethod)) {
			...
			final FormTag formTag =
				(FormTag) pageContext.getRequest().getAttribute(
	...
	public boolean checkIfInDefaultCausesValidation(final String buttonName) {
		final String bouttonCausesValidation =
			WelcomConfigurator.getMessageWithCfgChartePrefix(
				".bouton.default.causesValidation");
		final StringTokenizer st =
			new java.util.StringTokenizer(bouttonCausesValidation, ";", false);

		while (st.hasMoreElements()) {
			final String elements = (String) st.nextElement();

			if (Util
				.isEquals(elements.toLowerCase(), buttonName.toLowerCase())) {
				return true;
			}
		}
	...
	public boolean checkIfInDefaultForceReadWrite(final String buttonName) {
		final String bouttonForceReadWrite =
			WelcomConfigurator.getMessageWithCfgChartePrefix(
				".bouton.default.forceReadWrite");
		final StringTokenizer st =
			new java.util.StringTokenizer(bouttonForceReadWrite, ";", false);
