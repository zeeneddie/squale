...
public class InternalTableUtil {
    ...
    /** Constante*/
    private static final String TABLE_TAG_KEY_SORT = "com.airfrance.welcom.tableTag";
     ...
     */
    private static void razTheCheckBoxAttribute(final HttpServletRequest request, final ActionForm form, final String attribute) {
        try {
            ...
            if (String.class.equals(propertyType)) {
                ...
                log.warn("Impossible de reinitialiser la checkbox de la liste, type de l'attribut : " + attribute + "=" + propertyType);
            }
    ...
    public static boolean isTruncated(final String label, final String truncate, final String type) {
        if (!GenericValidator.isBlankOrNull(truncate) && (GenericValidator.isBlankOrNull(type) || type.equalsIgnoreCase("STRING")) && (label.length() > parseTruncate(truncate))) {
            return true;
        }
        return false;
    }
