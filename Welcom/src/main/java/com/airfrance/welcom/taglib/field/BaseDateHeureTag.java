...
public abstract class BaseDateHeureTag extends org.apache.struts.taglib.html.BaseFieldTag{
    ...
    /**
     * Retourne si le check a etait mis en place
     * le supprime la cas echeant
     ...
     */
    protected abstract void doRender(StringBuffer sb) throws JspException;
     ...
     * @throws JspException erreur sur le recupration de la value
     */
    protected void doRenderInput(StringBuffer sb,int currentType) throws JspException {
        ...
        setOnblur(TagUtils.getJavascriptCheckValue(property, getJavascriptDateOrHour(currentType), false, false, false, false,getDateOrHourPattern(currentType))+(oldOnBlur==null?"":oldOnBlur));
     ...
     *            le stringBuffer
     * @param currentType
     *            TYPE_WDATE / TYPE_WHOUR
     * @throws JspException
     *             Exception pouvant etre levee
     */
    public void doRenderValue(final StringBuffer results, int currentType) throws JspException {
                ...
                if (value instanceof Date) {
                    ...
                    try {
                        if ((((String) value).length() < 1) || (((String) value).indexOf("null") >= 0)) {
                            ...
                            // Format la chaine
                            if (getFormatKeyDateOrHourFormat(currentType) != null && GenericValidator.isLong((String) value)) {
                                renderValue = getSimpleDateOrHourFormat(currentType).format(new Date(Long.parseLong((String) value)));                       
                            } else {
                                renderValue = getSimpleDateOrHourFormat(currentType).format(DateUtil.parseAllDate((String) value));
                            }
                        }
    ...
    public void updateSimpleDateFormats() {
        ...
        HashMap hashMap = (HashMap) ((HttpServletRequest) pageContext.getRequest()).getSession().getAttribute("dateformats");
        if (hashMap == null) {
            hashMap = new HashMap();
     ...
     */
    protected abstract void updateSimpleDateFormats(final MessageResources resources, final Locale localeRequest, HashMap hashMap);
     ...
     *            locale
     * @param hashMap
     *            hashmap des formats de clefs ...
     */
    protected void updateSimpleDateFormat(final int currentType, final MessageResources resources, final Locale localeRequest, HashMap hashMap) {
        ...
        if (Util.isFalse(WelcomConfigurator.getMessage(WelcomConfigurator.DATE_USE_LEGACY)) || !GenericValidator.isBlankOrNull(getFormatKeyDateOrHourFormat(currentType))){
