...
public class JSOngletItem {
    ...
    public String doPrintCorps(final int indice) throws JspException {
        ...
        buf.append(
            render.drawBodyStart(
                name,
                isOngletSelected.equals("true") || isOngletSelected.equals(""),
                WLazyUtil.isLazy(lazyLoading) && Util.isTrue(WelcomConfigurator.getMessage(WelcomConfigurator.OPTIFLUX_GLOBAL_LAZYLOADING_ONGLETS))));
        ...
        if (!Util.isTrue(getIsOnglet()) && WLazyUtil.isLazy(lazyLoading) && Util.isTrue(WelcomConfigurator.getMessage(WelcomConfigurator.OPTIFLUX_GLOBAL_LAZYLOADING_ONGLETS))) {
