...
import com.airfrance.welcom.taglib.field.wrap.WAutoCompleteTag;
import com.airfrance.welcom.taglib.field.wrap.WCheckBoxTag;
import com.airfrance.welcom.taglib.field.wrap.WDateHeureTag;
import com.airfrance.welcom.taglib.field.wrap.WDateTag;
import com.airfrance.welcom.taglib.field.wrap.WEmailTag;
import com.airfrance.welcom.taglib.field.wrap.WNumberTag;
import com.airfrance.welcom.taglib.field.wrap.WPasswordTag;
import com.airfrance.welcom.taglib.field.wrap.WRadioTag;
import com.airfrance.welcom.taglib.field.wrap.WTextTag;
import com.airfrance.welcom.taglib.field.wrap.WTextareaTag;
import com.airfrance.welcom.taglib.table.ColTag;
...
public class FieldTag extends BaseFieldTag {
    ...
    public int doStartTag() throws JspException {
        ...
        if (((pageAccess != null) && pageAccess.equals(READONLY) && (forceReadWrite == false)) || ((access != null) && access.equals("READONLY"))) {
            access = READONLY;
    ...
    private Object writeOldField(final StringBuffer buffer) throws JspException {
        ...
        if ((type.equals(CHECKBOX) || type.equals(RADIO)) && WelcomConfigurator.getCharte() == Charte.V2_002 && GenericValidator.isBlankOrNull(fieldTag.getStyleClass())) {
        ...
        if (!access.equals(READWRITE) && (objvalue != null) && (!type.equals(CHECKBOX) && !type.equals(TEXTAREA) && !type.equals(RADIO))) {
            ...
            if (GenericValidator.isBlankOrNull(objvalue.toString())) {
                ...
                if (type.equals(DATEHEURE)) {
                    try {
                        ...
                        buffer.append(((DateHeureTag) fieldTag).getSimpleDateFormat().format(date) + " " + ((DateHeureTag) fieldTag).getSimpleHourFormat().format(date));
                    ...
                    buffer.append(LayoutUtils.getLabel(pageContext, "checkbox." + objvalue.toString(), (String[]) null));
    ...
    private void writeTagInReadWriteMode() throws JspException {
        ...
        if ((access.equals(READWRITE)) && (!type.equals(CHECKBOX)) && (!type.equals(RADIO))) {
            if (GenericValidator.isBlankOrNull(fieldTag.getOnchange())) {
                fieldTag.setOnchange("checkValue(this, '" + property + "','" + type + "'," + check + "," + isUpperCase() + "," + isFirstUpperCase() + "," + accent + ");");
            } else {
                fieldTag.setOnchange("checkValue(this, '" + property + "','" + type + "'," + check + "," + isUpperCase() + "," + isFirstUpperCase() + "," + accent + ");" + fieldTag.getOnchange());
            }
        }
        ...
        if (!GenericValidator.isBlankOrNull(easyCompleteName) || !GenericValidator.isBlankOrNull(easyCompleteCallBackUrl)) {
            ...
            if (js == null) {
                ...
                CanvasUtil.addJs(WelcomConfigurator.getMessage(WelcomConfigurator.HEADER_LOCALJS_PATH_KEY) + "auto-complete.js", this, pageContext);
            }
        }
     ...
     */
    private void checkColInclusion() throws JspException {
    ...
    private void writeEasyComplete(final BaseHandlerTag myFieldTag) {
        if ((!GenericValidator.isBlankOrNull(easyCompleteName) || !GenericValidator.isBlankOrNull(easyCompleteCallBackUrl)) && !access.equals(READONLY)) {
            ...
            if (!GenericValidator.isBlankOrNull(easyCompleteDecorationClass)) {
                ...
                buf.append(",'" + WelcomConfigurator.getMessage(WelcomConfigurator.EASY_COMPLETE_DEFAULT_DECORATION_CLASS) + "'");
            }
    ...
    private BaseHandlerTag createSpecificIntelligentField() throws JspException {
        ...
        } else if ((type.equals(TEXT)) && (!GenericValidator.isBlankOrNull(easyCompleteName) || !GenericValidator.isBlankOrNull(easyCompleteCallBackUrl))) {
    ...
    private BaseHandlerTag createSpecificField() throws JspException {
        ...
        if (type.equals(PASSWORD)) {
            myFieldTag = new com.airfrance.welcom.taglib.field.PasswordTag();
        }

        if ((type.equals(TEXT)) && (!GenericValidator.isBlankOrNull(easyCompleteName) || !GenericValidator.isBlankOrNull(easyCompleteCallBackUrl)) && !access.equals(READONLY)) {
        ...
        if (type.equals(DATE)) {
            myFieldTag = new com.airfrance.welcom.taglib.field.DateTag();
        }

        if (type.equals(DATEHEURE)) {
            myFieldTag = new com.airfrance.welcom.taglib.field.DateHeureTag();
        }
    ...
    private String getPageAccess() throws JspException {
        ...
        if (!GenericValidator.isBlankOrNull(accessKey)) {
            if ((pageAccess != null) && !(Util.isEquals(pageAccess, Access.READONLY) || Util.isEquals(pageAccess, Access.READWRITE) || Util.isEquals(pageAccess, Access.NONE))) {
                throw new JspException("L'attribut accessKey doit retourner une valeur READWRITE ou READONLY ou NONE (cf getSecuritePage()) : " + pageAccess);
            }
            ...
            if (o != null) {
                if (o instanceof WILogonBeanSecurity) {
                    ...
                    if ((overridePageAccess == null) || Util.isTrue(overridePageAccess)) {
                        ...
                        if (Util.isEquals(pageAccess, Access.READONLY) && Util.isEquals(accessTag, Access.READWRITE)) {
    ...
    public void setType(final String newType) {
        ...
        if (newType.equalsIgnoreCase("DATEHOUR") || newType.equalsIgnoreCase("DATETIME") || newType.equalsIgnoreCase(DATEHEURE)) {
            type = DATEHEURE;
        }
