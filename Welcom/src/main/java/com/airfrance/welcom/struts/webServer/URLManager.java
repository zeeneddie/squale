...
public class URLManager
{
     ...
     */
    public URLManager( final String pSUrl, final HttpServlet pServlet )
        throws IOException
    {
        sUrl = pSUrl;
    ...
    /**
     * Recontruit l'URL et recherche la derniere date de modification
     * 
     * @throws IOException Pb sur la stream
     */
        ...
        throws IOException
    {
        ...
        // Si webFile est null ..
        if ( mWebFile == null
            && Util.isTrue( WelcomConfigurator.getMessage( WelcomConfigurator.OPTIFLUX_JSOBFUSCATOR_DISABLED ) ) )
        {

            if ( mWebFile == null )
            {
                mWebFile =
                    getWebFileFromClassPath( "/web/" + WelcomConfigurator.getCharte().getWelcomConfigPrefix()
                        + "/debug/" + sUrl );
            }
        ...
        {
            mWebFile =
                getWebFileFromClassPath( "/web/" + WelcomConfigurator.getCharte().getWelcomConfigPrefix() + "/" + sUrl );
        }
     ...
     * @return webfile
     */
    private WebFile getWebFileFromHTTP( String pUrl )
     ...
     */
    public Date getURLDate( final URL pUrl )
        throws IOException
    {
    ...
    public Date getLastDate()
    {
        ...
        if ( ( WelcomConfigurator.getLastDate() != null )
            && ( WelcomConfigurator.getLastDate().getTime() >= mWebFile.getLastDate().getTime() ) )
        {
