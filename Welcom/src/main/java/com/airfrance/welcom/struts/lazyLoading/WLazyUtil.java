...
public class WLazyUtil {
    ...
    private static Hashtable searchParameter(final String s) {
        ...
        final Pattern pattern = Pattern.compile("(name|type|value|checked)\\s*=\\s*([\"\'])", Pattern.CASE_INSENSITIVE | Pattern.MULTILINE);
    ...
    public static boolean isLazy(final boolean lazyTag) {
        final boolean lazyGlobal = new Boolean(WelcomConfigurator.getMessage(WelcomConfigurator.LAZYLOADING_KEY)).booleanValue();
    ...
    public static String convertSelectToLightInput(final String s) {
        ...
        final Pattern reOptionSelected = Pattern.compile("<\\s*option[^>]+selected[^>]*>", Pattern.CASE_INSENSITIVE | Pattern.MULTILINE);
        ...
        if (maSelect.find()) {
            ...
            if (maOptionSelected.find()) {
                ...
                if (parameters.containsKey("value")) {
                    ...
                    final Pattern reOptionEnd = Pattern.compile("([^<]*)<\\s*\\/option\\s*>", Pattern.CASE_INSENSITIVE | Pattern.MULTILINE);
    ...
    public static String getLightBody(final String corps) {
        ...
        reg = Pattern.compile("(<\\s*textarea[^>]*>[^<]*<\\/\\s*textarea\\s*>)", Pattern.CASE_INSENSITIVE | Pattern.MULTILINE);
    ...
    public static String getSuperLightBody(final String corps) {
        ...
        while (mat.find(pos)) {
            ...
            if (Util.isEqualsIgnoreCase(Util.removeQuotes((String) parameters.get("type")), "checkbox") && parameters.containsKey("checked")) {
    ...
    public static String getLightCombo(final String s) {
        ...
        // Search name
        final Pattern reOptionSelected = Pattern.compile("<\\s*option[^>]+selected[^>]*>[^<]*<\\/\\s*option\\s*>", Pattern.CASE_INSENSITIVE | Pattern.MULTILINE);
        final Pattern reOption = Pattern.compile("<\\s*option[^>]*>([^<]*)<\\/\\s*option\\s*>", Pattern.CASE_INSENSITIVE | Pattern.MULTILINE);
        final Matcher maOptionSelected = reOptionSelected.matcher(s);
        final Matcher maOption = reOption.matcher(s);

        //RE reOptionSelected = new RE ("<\\s*option[^>]+selected[^>]*>[^<]*<\\/\\s*option\\s*>",RE.MATCH_CASEINDEPENDENT);
