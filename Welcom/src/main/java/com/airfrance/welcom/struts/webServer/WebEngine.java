...
public class WebEngine {
    ...
    public OutputStream getOutputStream() throws IOException {
        OutputStream out;
        if ((request.getHeader("Accept-Encoding") != null) && (request.getHeader("Accept-Encoding").indexOf(WelcomConfigurator.getMessage(WelcomConfigurator.OPTIFLUX_COMPRESSION_MODE).toLowerCase()) > -1)) {
            response.setHeader("Content-Encoding", WelcomConfigurator.getMessage(WelcomConfigurator.OPTIFLUX_COMPRESSION_MODE).toLowerCase());
    ...
    public void process(String pathFile) throws IOException {
        ...
        if (!isModified(urlManager)) {
            ...
            if (urlManager.isImage()) {
                ...
                if (Util.isEquals(urlManager.getMimeType(), "text/css") && Util.isTrue(WelcomConfigurator.getMessage(WelcomConfigurator.OPTIFLUX_REMOVECOMMENTS_CSS))) {
                    final int lastSlashPos = urlManager.getSUrl().lastIndexOf("/");
                    s = s.replaceAll("@import url\\(\\.\\/", "@import url(css.do?value=" + urlManager.getSUrl().substring(0, lastSlashPos + 1));
                    s = smartReplaceAll(s);
                    s = removeComments(s);
                    s = s.replaceAll("\\n\\p{Space}*", "\n");
                    s = s.replaceAll("[\n\r\t\f]", "");
                    s = s.replaceAll("[^}]*(UNI|MAJ)[^}]*}", ""); // Suppresion de tout ce qui contient la charte unitaire
                }

                //Optimization des .js
                if (Util.isEquals(urlManager.getMimeType(), "application/x-javascript") && Util.isTrue(WelcomConfigurator.getMessage(WelcomConfigurator.OPTIFLUX_REMOVECOMMENTS_JS))) {
                    s = smartReplaceAll(s);
                    s = removeComments(s);
                    s = s.replaceAll("\\n\\p{Space}*", "\n");
