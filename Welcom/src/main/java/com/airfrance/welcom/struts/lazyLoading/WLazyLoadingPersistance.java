 ...
 * Fenêtre&gt;Préférences&gt;Java&gt;Génération de code&gt;Code et commentaires
 */
public class WLazyLoadingPersistance implements Serializable {
    /**
     * 
     */
    ...
    public void add(final WLazyLoadingType type, final String key, final String data) {
        if (!htObjectsPerType.containsKey(type)) {
            ...
            final int limit = Integer.parseInt(WelcomConfigurator.getMessage(WelcomConfigurator.OPTIFLUX_GLOBAL_LAZYLOADING_HISTORY));
     ...
     */
    public String get(final WLazyLoadingType type, final String key) throws WLazyPersistanceExeption {
        if (!htObjectsPerType.containsKey(type)) {
    ...
    public boolean contains(final WLazyLoadingType type, final String key) {
        return htObjectsPerType.containsKey(type) && ((WLazyLoadingTypeObject) htObjectsPerType.get(type)).contains(key);
    }
     ...
     * Fenêtre&gt;Préférences&gt;Java&gt;Génération de code&gt;Code et commentaires
     */
    private class WLazyLoadingTypeObject implements Serializable {
         ...
         */
        public String get(final String key) throws WLazyPersistanceExeption {
            if (!htObjects.containsKey(key)) {
