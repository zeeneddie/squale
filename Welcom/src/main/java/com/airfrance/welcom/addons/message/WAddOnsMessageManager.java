 ...
 * Fenêtre&gt;Préférences&gt;Java&gt;Génération de code&gt;Code et commentaires
 */
public class WAddOnsMessageManager extends WAddOns {
    ...
    /**
     * Initilise tle thread de verification de mise a jour des 
     * libellé si on est clonné.
    ...
    public void initSynchroUpdate(final ActionServlet servlet) {
        ...
        if (Util.isTrue(WelcomConfigurator.getMessage(WelcomConfigurator.ADDONS_MESSAGE_MANAGER_CLONNAGE_ENABLED))) {
            ...
            long s = 0;
            try {
                s = Long.parseLong(WelcomConfigurator.getMessage(WelcomConfigurator.ADDONS_MESSAGE_MANAGER_REFRESH_DELAY_MS));
    ...
    public void initMappings(final ActionServlet servlet, final ModuleConfig config) throws AddonsException {
        ...
        initMappings(servlet, config, "wMessagesBean", "addons.messageManager.jsp.messages", "/wMessageList", "com.airfrance.welcom.addons.message.action.WMessageListeAction");
