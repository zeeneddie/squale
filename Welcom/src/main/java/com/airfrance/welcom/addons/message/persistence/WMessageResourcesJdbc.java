...
public class WMessageResourcesJdbc implements WIMessageResourcesPersistence {
    ...
    private static final String TABLE_MSG_LIBELLE = WelcomConfigurator.getMessage(WelcomConfigurator.ADDONS_MESSAGE_MANAGER_TABLE_LIBELLES);
    /**
     *   executeQuery
     *
     * simple Method to provide access to jdbc persistence
     * submit a sql query and returns resultset as vector of String[]
     ...
     *
     *  ask database for given locale
     *  get all translations from database
     *  normally you would use persistence beans
     */
    public Vector getAppMessagesByLocale(final String locale) {
        final Vector localeMessages = new Vector();

        final Vector translationResult = executeQuery("SELECT messageKey, message FROM " + TABLE_MSG_LIBELLE + " WHERE locale = '" + locale + "'");
