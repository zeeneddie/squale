 ...
 * @author M327837
 *
 * Gere un collection de connection lors du possage de Welcom en Mode : MJDBC
 * (Multiple connexion JDBC)
 */
public class WConnectionPool {
    ...
    public static Connection getConnection(final String name) throws SQLException {
        ...
        if (!connectionPool.containsKey(name)) {
            throw new SQLException("Aucune dataSource declaré sous le nom de " + name + ", verifier votre struts config");
        }
    ...
    private boolean testConnection() {
        try {
            ...
            logStartup.error("Impossible d'établir une connexion à la base de données sous le nom " + connectionString.getName());
