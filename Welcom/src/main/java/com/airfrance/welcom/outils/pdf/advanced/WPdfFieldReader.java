...
public class WPdfFieldReader {
    ...
    public void init() {
        for (int i = 0; i < pdfReader.getAcroForm().size(); i++) {
            ...
            final PRAcroForm.FieldInformation p = (PRAcroForm.FieldInformation) pdfReader.getAcroForm().getFields().get(i);

            final WPdfField pf = new WPdfField();
            pf.setName(p.getName());

            // Si ce n'est pas un champs text alors on l'ignore
            if (!p.getInfo().contains(PdfName.FT) || !(((PdfName) p.getInfo().get(PdfName.FT)).toString().equals(PdfName.TX.toString()))) {
     ...
     */
    public void fill(final OutputStream out) throws IOException, DocumentException {
