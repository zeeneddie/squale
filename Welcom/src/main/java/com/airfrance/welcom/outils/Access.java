...
public class Access implements Serializable {
     ...
     * 
     * @param access1 :
     *            Droit d'accés 1 (READWRITE,READONLY,NONE,YES,NO)
     * @param access2 :
     *            Droit d'accés 2 (READWRITE,READONLY,NONE,YES,NO)
    ...
    public static boolean checkAccessDroit(final String pageAccess) {
        return (checkAccessDroit(pageAccess, Access.READONLY) || checkAccessDroit(pageAccess, Access.YES) || checkAccessDroit(pageAccess, Access.READWRITE));
    }
     ...
     * 
     * @param logonbean :
     *            Le bien de logon pour la verifieation de la clef
     * @param accessKey :
     *            Valeur a tester
     * @return Vrai si OK
     */
    public static String getMultipleSecurityPage(final WILogonBeanSecurity logonbean, final String accessKey) {
        ...
        while (droitSplitter.hasMoreElements()) {
            ...
            if (((checkAccessDroit(access, NONE) || checkAccessDroit(access, NO)) && checkAccessDroit(acc)) || (checkAccessDroit(access, READONLY) && checkAccessDroit(acc, READWRITE))) {
                access = acc;
            }
        }
     ...
     *            contexte
     * @param accessKey
     *            valeur de l'accessKey du tag
     * @param forceReadWrite
     *            si on est on forcage
     * @param overridePageAccess
     *            sur on surchage le page page
     * @return le droit calculé
     * @throws JspException
     *             leve une execption si le droit retouné n'est pas NONE,
     *             READWRITE, READONLY
     */
    public static String computeTagReadWriteAccess(PageContext pageContext, String accessKey, boolean forceReadWrite, boolean overridePageAccess) throws JspException {
        ...
        if (!GenericValidator.isBlankOrNull(accessKey)) {
            ...
            String accessTagMerge = getAccessTagMergeWithPageAccess(pageContext,overridePageAccess, pageAccess, accessKey);
     ...
     */
    private static String getAccessTagMergeWithPageAccess(PageContext pageContext,boolean overridePageAccess, String pageAccess, final String accessKey) {
        ...
        if (o != null) {
            ...
            if (o instanceof WILogonBeanSecurity) {
                ...
                if (Util.isEquals(pageAccess, Access.READONLY) && Util.isEquals(computedAccessTag, Access.READWRITE)) {
    ...
    /**
     * @param access
     *            Accées NONE / NO / YES / READONLY / READWRITE
     * @return vrais si du type NONE / READONLY / READWRITE
     */
    private static boolean isNotPageAccess(String access) {
        return (access != null) && !(Util.isEquals(access, Access.READONLY) || Util.isEquals(access, Access.READWRITE) || Util.isEquals(access, Access.NONE));
    }
