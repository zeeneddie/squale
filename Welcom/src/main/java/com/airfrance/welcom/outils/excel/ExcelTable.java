...
public class ExcelTable {
     ...
     */
    public void writeTable(final WritableWorkbook workbook) throws WriteException {
     ...
     */
    private double writeCell(final WritableSheet sheet, final WritableCellFormat cellFormat, final Object element, final HeaderElement headerElement, final int posY, final int posX)
            ...
            if (val != null) {
                switch (headerElement.getType()) {
					case HeaderElement.DATE_HEURE :
						final DateTime dateHeureCell = new DateTime(posX, posY, (Date)val, getWritableCellFormat(headerElement.getDateHeureFormat()));
						sheet.addCell(dateHeureCell);
						return WFontSimulator.getSize(headerElement.getDateHeureFormat());
                    case HeaderElement.DATE :
						final DateTime dateCell = new DateTime(posX, posY, (Date)val, getWritableCellFormat(headerElement.getDateFormat()));
     ...
     * Renvoie une date ou une string selon le cas
    ...
    private Object getValue(final Object element, final HeaderElement headerElement) {
        try {
            ...
            else if ( (headerElement.getType() == HeaderElement.DATE_HEURE || headerElement.getType() == HeaderElement.DATE ) && value instanceof String)
            {
                try {  
     ...
     * @throws RowsExceededException : Trop de lignes
     */
    private void writeHeader(final int fontSize, final WritableSheet sheet) throws WriteException, RowsExceededException {
        for (int i = 0; i < header.size(); i++) {
