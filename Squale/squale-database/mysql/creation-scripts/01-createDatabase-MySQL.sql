
    alter table Analysis_Task 
        drop 
        foreign key FK91CAC908A096529F;

    alter table Analysis_Task 
        drop 
        foreign key FK91CAC908BE3D10F3;

    alter table AuditDisplayConfBO 
        drop 
        foreign key FKBF5515D87E8A4226;

    alter table AuditDisplayConfBO 
        drop 
        foreign key FKBF5515D883596FA2;

    alter table AuditDisplayConfBO 
        drop 
        foreign key FKBF5515D873586D71;

    alter table AuditGridBO 
        drop 
        foreign key FK71AB79AE7E8A4226;

    alter table AuditGridBO 
        drop 
        foreign key FK71AB79AE83596FA2;

    alter table AuditGridBO 
        drop 
        foreign key FK71AB79AED2B0987;

    alter table Component 
        drop 
        foreign key FK24013CDDF53F33A0;

    alter table Component 
        drop 
        foreign key FK24013CDD49BB62A2;

    alter table Component 
        drop 
        foreign key FK24013CDD76518568;

    alter table Component 
        drop 
        foreign key FK24013CDD649B6A3E;

    alter table Component 
        drop 
        foreign key FK24013CDD78D3A978;

    alter table Component 
        drop 
        foreign key FK24013CDDD0519AC;

    alter table Component 
        drop 
        foreign key FK24013CDD7052981E;

    alter table Components_Audits 
        drop 
        foreign key FK5D3B01417E8A4226;

    alter table Components_Audits 
        drop 
        foreign key FK5D3B014163D3E5E8;

    alter table CriteriumPractice_Rule 
        drop 
        foreign key FK8ADDE86E3A648E9;

    alter table CriteriumPractice_Rule 
        drop 
        foreign key FK8ADDE86BEFBA921;

    alter table Error 
        drop 
        foreign key FK401E1E87E8A4226;

    alter table Error 
        drop 
        foreign key FK401E1E883596FA2;

    alter table FactorCriterium_Rule 
        drop 
        foreign key FK89734030CFD14A09;

    alter table FactorCriterium_Rule 
        drop 
        foreign key FK89734030E3A648E9;

    alter table FactorRef 
        drop 
        foreign key FK2854B0A42CC32BDC;

    alter table FactorRef 
        drop 
        foreign key FK2854B0A44AD989DF;

    alter table Formula_Conditions 
        drop 
        foreign key FKB3141771A528EE84;

    alter table Formula_Measures 
        drop 
        foreign key FK7A19C1CE1DADC1A7;

    alter table GridFactor_Rule 
        drop 
        foreign key FKBC5A70C6CFD14A09;

    alter table GridFactor_Rule 
        drop 
        foreign key FKBC5A70C6D2B0987;

    alter table Mark 
        drop 
        foreign key FK247AED4B98962;

    alter table Mark 
        drop 
        foreign key FK247AED63D3E5E8;

    alter table Measure 
        drop 
        foreign key FK9B263D3E7E8A4226;

    alter table Measure 
        drop 
        foreign key FK9B263D3EE5BC5FE3;

    alter table Measure 
        drop 
        foreign key FK9B263D3E63D3E5E8;

    alter table Metric 
        drop 
        foreign key FK892AE1D02FA052BA;

    alter table Module 
        drop 
        foreign key FK89B0928C612DF65D;

    alter table PracticeResult_Repartition 
        drop 
        foreign key FK84B2F9904B98962;

    alter table Profile_DisplayConfBO 
        drop 
        foreign key FKC122A97D802460C7;

    alter table Profile_DisplayConfBO 
        drop 
        foreign key FKC122A97D78D3AA46;

    alter table Profile_Rights 
        drop 
        foreign key FK62F82D6D9B681818;

    alter table Profile_Rights 
        drop 
        foreign key FK62F82D6D75D5ECAE;

    alter table Profiles_Grids 
        drop 
        foreign key FK9949B158E49543C8;

    alter table Profiles_Grids 
        drop 
        foreign key FK9949B15878D3AA46;

    alter table ProjectParameter 
        drop 
        foreign key FK6D7538B0AA5FCCA5;

    alter table ProjectParameter 
        drop 
        foreign key FK6D7538B081587B7D;

    alter table QualityResult 
        drop 
        foreign key FKE9E7A9DC7E8A4226;

    alter table QualityResult 
        drop 
        foreign key FKE9E7A9DC83596FA2;

    alter table QualityResult 
        drop 
        foreign key FKE9E7A9DC330CF5F3;

    alter table QualityRule 
        drop 
        foreign key FK420ADC7B883C760C;

    alter table Rule 
        drop 
        foreign key FK270B1CE5BC5FE3;

    alter table RuleCheckingTransgressionItem 
        drop 
        foreign key FKDF0D4973B5ECF9F1;

    alter table RuleCheckingTransgressionItem 
        drop 
        foreign key FKDF0D4973B26EE3F8;

    alter table RuleCheckingTransgressionItem 
        drop 
        foreign key FKDF0D497363D3E5E8;

    alter table RuleCheckingTransgressionItem 
        drop 
        foreign key FKDF0D4973C617EA6F;

    alter table RuleSet 
        drop 
        foreign key FKBF8713A683596FA2;

    alter table SqualeReference 
        drop 
        foreign key FK32FD7E08D0519AC;

    alter table Stats_squale_dict 
        drop 
        foreign key FK9B3A9E52F53F33A0;

    alter table Stats_squale_dict_annexe 
        drop 
        foreign key FKBC8FB71EF53F33A0;

    alter table TaskParameter 
        drop 
        foreign key FK16AD3384A096529F;

    alter table TaskRef 
        drop 
        foreign key FK797F8AE1ADBED57;

    alter table Termination_Task 
        drop 
        foreign key FKC739A220A096529F;

    alter table Termination_Task 
        drop 
        foreign key FKC739A220BE3D10F3;

    alter table UserAccess 
        drop 
        foreign key FKB60A252F6EEDFC90;

    alter table UserBO 
        drop 
        foreign key FK9790197875D5ECAE;

    alter table User_Rights 
        drop 
        foreign key FK21885CCBFDF9038A;

    alter table User_Rights 
        drop 
        foreign key FK21885CCB75D5ECAE;

    alter table User_Rights 
        drop 
        foreign key FK21885CCB6EEDFC90;

    alter table Volumetry_Measures 
        drop 
        foreign key FK92AE693368A4458F;

    drop table if exists Analysis_Task;

    drop table if exists AtomicRights;

    drop table if exists AuditBO;

    drop table if exists AuditDisplayConfBO;

    drop table if exists AuditFrequency;

    drop table if exists AuditGridBO;

    drop table if exists Component;

    drop table if exists Components_Audits;

    drop table if exists CriteriumPractice_Rule;

    drop table if exists Error;

    drop table if exists FactorCriterium_Rule;

    drop table if exists FactorRef;

    drop table if exists Formula;

    drop table if exists Formula_Conditions;

    drop table if exists Formula_Measures;

    drop table if exists GridFactor_Rule;

    drop table if exists Mark;

    drop table if exists Measure;

    drop table if exists Message;

    drop table if exists Metric;

    drop table if exists Module;

    drop table if exists News;

    drop table if exists PracticeResult_Repartition;

    drop table if exists ProfileBO;

    drop table if exists Profile_DisplayConfBO;

    drop table if exists Profile_Rights;

    drop table if exists Profiles_Grids;

    drop table if exists ProjectParameter;

    drop table if exists QualityGrid;

    drop table if exists QualityResult;

    drop table if exists QualityRule;

    drop table if exists Rule;

    drop table if exists RuleCheckingTransgressionItem;

    drop table if exists RuleSet;

    drop table if exists Serveur;

    drop table if exists SqualeReference;

    drop table if exists Stats_squale_dict;

    drop table if exists Stats_squale_dict_annexe;

    drop table if exists StopTimeBO;

    drop table if exists Task;

    drop table if exists TaskParameter;

    drop table if exists TaskRef;

    drop table if exists Tasks_User;

    drop table if exists Termination_Task;

    drop table if exists UserAccess;

    drop table if exists UserBO;

    drop table if exists User_Rights;

    drop table if exists Volumetry_Measures;

    drop table if exists adminParams;

    drop table if exists displayConf;

    create table Analysis_Task (
        TasksUserId bigint not null,
        TaskRefId bigint not null,
        AnalysisTaskIndex integer not null,
        primary key (TasksUserId, AnalysisTaskIndex)
    ) type=InnoDB;

    create table AtomicRights (
        AtomicRightsId bigint not null auto_increment,
        Name varchar(255),
        primary key (AtomicRightsId)
    ) type=InnoDB;

    create table AuditBO (
        AuditId bigint not null auto_increment,
        Name varchar(255),
        auditDate datetime,
        auditType varchar(255),
        Status integer not null,
        Comments varchar(255),
        historicalDate datetime,
        Duration varchar(10),
        END_DATE datetime,
        MAX_FILE_SYSTEM_SIZE bigint,
        BEGINNING_DATE datetime,
        squale_version double precision,
        primary key (AuditId)
    ) type=InnoDB;

    create table AuditDisplayConfBO (
        AuditConfId bigint not null auto_increment,
        ConfId bigint,
        ProjectId bigint,
        AuditId bigint,
        primary key (AuditConfId)
    ) type=InnoDB;

    create table AuditFrequency (
        AuditFrequencyId bigint not null auto_increment,
        Nb_days integer not null,
        Frequency integer not null,
        primary key (AuditFrequencyId)
    ) type=InnoDB;

    create table AuditGridBO (
        AuditGridId bigint not null auto_increment,
        QualityGridId bigint,
        ProjectId bigint,
        AuditId bigint,
        primary key (AuditGridId)
    ) type=InnoDB;

    create table Component (
        ComponentId bigint not null auto_increment,
        subclass varchar(255) not null,
        Excluded bit not null,
        Justification text,
        Name text not null,
        Parent bigint,
        ProjectId bigint,
        AuditFrequency integer,
        ResultsStorageOptions integer,
        Status integer,
        PublicApplication bit,
        LastUpdate datetime,
        EXTERNAL_DEV bit,
        IN_PRODUCTION bit,
        lastUser text,
        Serveur bigint,
        LongFileName text,
        ProfileBO bigint,
        ParametersSet bigint,
        QualityGrid bigint,
        SourceManager bigint,
        StartLine integer,
        primary key (ComponentId)
    ) type=InnoDB;

    create table Components_Audits (
        ComponentId bigint not null,
        AuditId bigint not null,
        primary key (ComponentId, AuditId)
    ) type=InnoDB;

    create table CriteriumPractice_Rule (
        CriteriumRuleId bigint not null,
        Weight float not null,
        PracticeRuleId bigint not null,
        primary key (CriteriumRuleId, PracticeRuleId)
    ) type=InnoDB;

    create table Error (
        ErrorId bigint not null auto_increment,
        InitialMessage text,
        Message text,
        CriticityLevel varchar(255),
        TaskName varchar(255),
        AuditId bigint not null,
        ProjectId bigint not null,
        primary key (ErrorId)
    ) type=InnoDB;

    create table FactorCriterium_Rule (
        FactorRuleId bigint not null,
        Weight float not null,
        CriteriumRuleId bigint not null,
        primary key (FactorRuleId, CriteriumRuleId)
    ) type=InnoDB;

    create table FactorRef (
        ReferencielId bigint not null,
        Factor_Value float,
        Rule bigint not null,
        primary key (ReferencielId, Rule)
    ) type=InnoDB;

    create table Formula (
        FormulaId bigint not null auto_increment,
        subclass varchar(255) not null,
        ComponentLevel varchar(255),
        TriggerCondition text,
        Formula text,
        primary key (FormulaId)
    ) type=InnoDB;

    create table Formula_Conditions (
        FormulaId bigint not null,
        Value text,
        Rank integer not null,
        primary key (FormulaId, Rank)
    ) type=InnoDB;

    create table Formula_Measures (
        FormulaId bigint not null,
        Measure varchar(255)
    ) type=InnoDB;

    create table GridFactor_Rule (
        QualityGridId bigint not null,
        FactorRuleId bigint not null,
        primary key (QualityGridId, FactorRuleId)
    ) type=InnoDB;

    create table Mark (
        MarkId bigint not null auto_increment,
        Value float not null,
        ComponentId bigint not null,
        PracticeResultId bigint not null,
        primary key (MarkId)
    ) type=InnoDB;

    create table Measure (
        MeasureId bigint not null auto_increment,
        subclass varchar(255) not null,
        TaskName varchar(255),
        AuditId bigint not null,
        ComponentId bigint not null,
        RuleSetId bigint,
        primary key (MeasureId)
    ) type=InnoDB;

    create table Message (
        MessageKey varchar(255) not null,
        lang varchar(6) not null,
        Title text,
        Text text not null,
        primary key (MessageKey, lang)
    ) type=InnoDB;

    create table Metric (
        MetricId bigint not null auto_increment,
        subclass varchar(255) not null,
        Name varchar(255),
        MeasureId bigint,
        Blob_val blob,
        Boolean_val bit,
        Number_val float,
        String_val text,
        primary key (MetricId)
    ) type=InnoDB;

    create table Module (
        ModuleId bigint not null auto_increment,
        Message varchar(255),
        Name varchar(255),
        RuleId bigint not null,
        primary key (ModuleId)
    ) type=InnoDB;

    create table News (
        Id bigint not null auto_increment,
        NewsKey text not null,
        Beginning_Date datetime not null,
        End_Date datetime not null,
        primary key (Id)
    ) type=InnoDB;

    create table PracticeResult_Repartition (
        PracticeResultId bigint not null,
        Repartition_value integer,
        Repartition integer not null,
        primary key (PracticeResultId, Repartition)
    ) type=InnoDB;

    create table ProfileBO (
        ProfileId bigint not null auto_increment,
        Name varchar(255),
        primary key (ProfileId)
    ) type=InnoDB;

    create table Profile_DisplayConfBO (
        ConfId bigint not null auto_increment,
        Profile_ConfId bigint,
        ProfileId bigint,
        primary key (ConfId)
    ) type=InnoDB;

    create table Profile_Rights (
        ProfileId bigint not null,
        Rights_Value varchar(255),
        AtomicRightsId bigint not null,
        primary key (ProfileId, AtomicRightsId)
    ) type=InnoDB;

    create table Profiles_Grids (
        ProfileId bigint not null,
        GridId bigint not null,
        primary key (ProfileId, GridId)
    ) type=InnoDB;

    create table ProjectParameter (
        ParameterId bigint not null auto_increment,
        subclass varchar(255) not null,
        Value text,
        ListId bigint,
        Rank integer,
        MapId bigint,
        IndexKey varchar(255),
        primary key (ParameterId)
    ) type=InnoDB;

    create table QualityGrid (
        QualityGridId bigint not null auto_increment,
        Name varchar(255) not null,
        DateOfUpdate datetime not null,
        primary key (QualityGridId)
    ) type=InnoDB;

    create table QualityResult (
        QualityResultId bigint not null auto_increment,
        subclass varchar(255) not null,
        QualityRuleId bigint not null,
        MeanMark float not null,
        ProjectId bigint not null,
        AuditId bigint not null,
        primary key (QualityResultId)
    ) type=InnoDB;

    create table QualityRule (
        QualityRuleId bigint not null auto_increment,
        subclass varchar(255) not null,
        Help_Key varchar(255),
        Name varchar(255) not null,
        DateOfCreation datetime not null,
        Formula bigint,
        WeightFunction varchar(255),
        effort integer,
        primary key (QualityRuleId)
    ) type=InnoDB;

    create table Rule (
        RuleId bigint not null auto_increment,
        subclass varchar(255) not null,
        Category varchar(255),
        Code varchar(255),
        RuleSetId bigint not null,
        Severity varchar(255),
        primary key (RuleId)
    ) type=InnoDB;

    create table RuleCheckingTransgressionItem (
        ItemId bigint not null auto_increment,
        Line integer not null,
        Path text,
        Message text not null,
        ComponentId bigint,
        RuleId bigint not null,
        ComponentInvolvedId bigint,
        MeasureId bigint,
        primary key (ItemId)
    ) type=InnoDB;

    create table RuleSet (
        RuleSetId bigint not null auto_increment,
        subclass varchar(255) not null,
        Name varchar(255),
        DateOfUpdate datetime not null,
        ProjectId bigint,
        FileContent blob,
        CppTestName varchar(255) unique,
        Language varchar(255),
        primary key (RuleSetId)
    ) type=InnoDB;

    create table Serveur (
        ServeurId bigint not null,
        Name varchar(255) not null unique,
        primary key (ServeurId)
    ) type=InnoDB;

    create table SqualeReference (
        ReferencielId bigint not null auto_increment,
        QualityGrid bigint,
        PublicApplication bit,
        ApplicationName varchar(255),
        ProjectName varchar(255),
        ProjectLanguage varchar(255),
        Version varchar(255),
        AuditDate datetime,
        CodeLineNumber integer,
        MethodNumber integer,
        ClassNumber integer,
        HIDDEN bit not null,
        AUDIT_TYPE varchar(50) not null,
        primary key (ReferencielId)
    ) type=InnoDB;

    create table Stats_squale_dict (
        Id bigint not null auto_increment,
        NB_FACTEURS_ACCEPTES integer,
        NB_FACTEURS_ACCEPTES_RESERVES integer,
        NB_FACTEURS_REFUSES integer,
        NB_TOTAL_APPLI integer,
        NB_APPLI_AVEC_AUDIT_EXECUTE integer,
        NB_APPLI_AVEC_AUDIT_REUSSI integer,
        Date_calcul datetime,
        ROI_EN_MHI double precision,
        NB_APPLI_A_VALIDER integer,
        NB_AUDITS_ECHECS integer,
        NB_AUDITS_PROGRAMME integer,
        NB_AUDITS_PARTIELS integer,
        NB_AUDITS_REUSSIS integer,
        NB_APPLI_AVEC_AUCUN_AUDIT integer,
        Serveur bigint,
        primary key (Id)
    ) type=InnoDB;

    create table Stats_squale_dict_annexe (
        Id bigint not null auto_increment,
        NB_LIGNES integer,
        Profil varchar(50),
        NB_PROJETS integer,
        Serveur bigint,
        primary key (Id)
    ) type=InnoDB;

    create table StopTimeBO (
        StopTimeId bigint not null auto_increment,
        DayOfWeek varchar(9) not null,
        TimeOfDay varchar(5) not null,
        primary key (StopTimeId)
    ) type=InnoDB;

    create table Task (
        TaskId bigint not null auto_increment,
        Name varchar(255) not null unique,
        Class text not null,
        Configurable bit not null,
        Standard bit,
        Mandatory bit,
        primary key (TaskId)
    ) type=InnoDB;

    create table TaskParameter (
        TaskParameterId bigint not null auto_increment,
        Name varchar(255) not null,
        Value varchar(255) not null,
        TaskRefId bigint,
        primary key (TaskParameterId)
    ) type=InnoDB;

    create table TaskRef (
        TaskRefId bigint not null auto_increment,
        TaskId bigint,
        primary key (TaskRefId)
    ) type=InnoDB;

    create table Tasks_User (
        AbstractTasksUserId bigint not null auto_increment,
        subclass varchar(255) not null,
        Name varchar(255) not null unique,
        export_IDE bit,
        MilestoneAudit bit,
        NormalAudit bit,
        primary key (AbstractTasksUserId)
    ) type=InnoDB;

    create table Termination_Task (
        TasksUserId bigint not null,
        TaskRefId bigint not null,
        TerminationTaskIndex integer not null,
        primary key (TasksUserId, TerminationTaskIndex)
    ) type=InnoDB;

    create table UserAccess (
        UserAccessId bigint not null auto_increment,
        ApplicationId bigint,
        accessDate datetime not null,
        matricule text not null,
        AccessIndex integer,
        primary key (UserAccessId)
    ) type=InnoDB;

    create table UserBO (
        UserId bigint not null auto_increment,
        FullName varchar(255),
        Matricule varchar(255),
        Password varchar(255),
        Email varchar(255),
        ProfileId bigint not null,
        unsubscribed bit,
        primary key (UserId)
    ) type=InnoDB;

    create table User_Rights (
        UserId bigint not null,
        ProfileId bigint not null,
        ApplicationId bigint not null,
        primary key (UserId, ApplicationId)
    ) type=InnoDB;

    create table Volumetry_Measures (
        VolumetryId bigint not null,
        Measure varchar(255) not null,
        primary key (VolumetryId, Measure)
    ) type=InnoDB;

    create table adminParams (
        AdminParamsId bigint not null auto_increment,
        paramKey varchar(255) not null,
        paramaValue varchar(255) not null,
        primary key (AdminParamsId)
    ) type=InnoDB;

    create table displayConf (
        ConfId bigint not null auto_increment,
        subclass varchar(255) not null,
        X_TRE text,
        Y_TRE text,
        X_POS bigint,
        Y_POS bigint,
        componentType varchar(255),
        primary key (ConfId)
    ) type=InnoDB;

    alter table Analysis_Task 
        add index FK91CAC908A096529F (TaskRefId), 
        add constraint FK91CAC908A096529F 
        foreign key (TaskRefId) 
        references TaskRef (TaskRefId);

    alter table Analysis_Task 
        add index FK91CAC908BE3D10F3 (TasksUserId), 
        add constraint FK91CAC908BE3D10F3 
        foreign key (TasksUserId) 
        references Tasks_User (AbstractTasksUserId);

    alter table AuditDisplayConfBO 
        add index FKBF5515D87E8A4226 (AuditId), 
        add constraint FKBF5515D87E8A4226 
        foreign key (AuditId) 
        references AuditBO (AuditId)
		on delete cascade;

    alter table AuditDisplayConfBO 
        add index FKBF5515D883596FA2 (ProjectId), 
        add constraint FKBF5515D883596FA2 
        foreign key (ProjectId) 
        references Component (ComponentId)
		on delete cascade;

    alter table AuditDisplayConfBO 
        add index FKBF5515D873586D71 (ConfId), 
        add constraint FKBF5515D873586D71 
        foreign key (ConfId) 
        references displayConf (ConfId);

    alter table AuditGridBO 
        add index FK71AB79AE7E8A4226 (AuditId), 
        add constraint FK71AB79AE7E8A4226 
        foreign key (AuditId) 
        references AuditBO (AuditId)
		on delete cascade;

    alter table AuditGridBO 
        add index FK71AB79AE83596FA2 (ProjectId), 
        add constraint FK71AB79AE83596FA2 
        foreign key (ProjectId) 
        references Component (ComponentId)
		on delete cascade;

    alter table AuditGridBO 
        add index FK71AB79AED2B0987 (QualityGridId), 
        add constraint FK71AB79AED2B0987 
        foreign key (QualityGridId) 
        references QualityGrid (QualityGridId);

    alter table Component 
        add index FK24013CDDF53F33A0 (Serveur), 
        add constraint FK24013CDDF53F33A0 
        foreign key (Serveur) 
        references Serveur (ServeurId);

    alter table Component 
        add index FK24013CDD49BB62A2 (SourceManager), 
        add constraint FK24013CDD49BB62A2 
        foreign key (SourceManager) 
        references Tasks_User (AbstractTasksUserId);

    alter table Component 
        add index FK24013CDD76518568 (ProjectId), 
        add constraint FK24013CDD76518568 
        foreign key (ProjectId) 
        references Component (ComponentId)
		on delete cascade;

    alter table Component 
        add index FK24013CDD649B6A3E (ParametersSet), 
        add constraint FK24013CDD649B6A3E 
        foreign key (ParametersSet) 
        references ProjectParameter (ParameterId);

    alter table Component 
        add index FK24013CDD78D3A978 (ProfileBO), 
        add constraint FK24013CDD78D3A978 
        foreign key (ProfileBO) 
        references Tasks_User (AbstractTasksUserId);

    alter table Component 
        add index FK24013CDDD0519AC (QualityGrid), 
        add constraint FK24013CDDD0519AC 
        foreign key (QualityGrid) 
        references QualityGrid (QualityGridId);

    alter table Component 
        add index FK24013CDD7052981E (Parent), 
        add constraint FK24013CDD7052981E 
        foreign key (Parent) 
        references Component (ComponentId)
		on delete cascade;

    alter table Components_Audits 
        add index FK5D3B01417E8A4226 (AuditId), 
        add constraint FK5D3B01417E8A4226 
        foreign key (AuditId) 
        references AuditBO (AuditId)
		on delete cascade;

    alter table Components_Audits 
        add index FK5D3B014163D3E5E8 (ComponentId), 
        add constraint FK5D3B014163D3E5E8 
        foreign key (ComponentId) 
        references Component (ComponentId)
		on delete cascade;

    alter table CriteriumPractice_Rule 
        add index FK8ADDE86E3A648E9 (CriteriumRuleId), 
        add constraint FK8ADDE86E3A648E9 
        foreign key (CriteriumRuleId) 
        references QualityRule (QualityRuleId);

    alter table CriteriumPractice_Rule 
        add index FK8ADDE86BEFBA921 (PracticeRuleId), 
        add constraint FK8ADDE86BEFBA921 
        foreign key (PracticeRuleId) 
        references QualityRule (QualityRuleId);

    alter table Error 
        add index FK401E1E87E8A4226 (AuditId), 
        add constraint FK401E1E87E8A4226 
        foreign key (AuditId) 
        references AuditBO (AuditId)
		on delete cascade;

    alter table Error 
        add index FK401E1E883596FA2 (ProjectId), 
        add constraint FK401E1E883596FA2 
        foreign key (ProjectId) 
        references Component (ComponentId)
		on delete cascade;

    alter table FactorCriterium_Rule 
        add index FK89734030CFD14A09 (FactorRuleId), 
        add constraint FK89734030CFD14A09 
        foreign key (FactorRuleId) 
        references QualityRule (QualityRuleId);

    alter table FactorCriterium_Rule 
        add index FK89734030E3A648E9 (CriteriumRuleId), 
        add constraint FK89734030E3A648E9 
        foreign key (CriteriumRuleId) 
        references QualityRule (QualityRuleId);

    alter table FactorRef 
        add index FK2854B0A42CC32BDC (ReferencielId), 
        add constraint FK2854B0A42CC32BDC 
        foreign key (ReferencielId) 
        references SqualeReference (ReferencielId);

    alter table FactorRef 
        add index FK2854B0A44AD989DF (Rule), 
        add constraint FK2854B0A44AD989DF 
        foreign key (Rule) 
        references QualityRule (QualityRuleId);

    alter table Formula_Conditions 
        add index FKB3141771A528EE84 (FormulaId), 
        add constraint FKB3141771A528EE84 
        foreign key (FormulaId) 
        references Formula (FormulaId);

    alter table Formula_Measures 
        add index FK7A19C1CE1DADC1A7 (FormulaId), 
        add constraint FK7A19C1CE1DADC1A7 
        foreign key (FormulaId) 
        references Formula (FormulaId);

    alter table GridFactor_Rule 
        add index FKBC5A70C6CFD14A09 (FactorRuleId), 
        add constraint FKBC5A70C6CFD14A09 
        foreign key (FactorRuleId) 
        references QualityRule (QualityRuleId);

    alter table GridFactor_Rule 
        add index FKBC5A70C6D2B0987 (QualityGridId), 
        add constraint FKBC5A70C6D2B0987 
        foreign key (QualityGridId) 
        references QualityGrid (QualityGridId);

    alter table Mark 
        add index FK247AED4B98962 (PracticeResultId), 
        add constraint FK247AED4B98962 
        foreign key (PracticeResultId) 
        references QualityResult (QualityResultId)
		on delete cascade;

    alter table Mark 
        add index FK247AED63D3E5E8 (ComponentId), 
        add constraint FK247AED63D3E5E8 
        foreign key (ComponentId) 
        references Component (ComponentId)
		on delete cascade;

    alter table Measure 
        add index FK9B263D3E7E8A4226 (AuditId), 
        add constraint FK9B263D3E7E8A4226 
        foreign key (AuditId) 
        references AuditBO (AuditId)
		on delete cascade;

    alter table Measure 
        add index FK9B263D3EE5BC5FE3 (RuleSetId), 
        add constraint FK9B263D3EE5BC5FE3 
        foreign key (RuleSetId) 
        references RuleSet (RuleSetId);

    alter table Measure 
        add index FK9B263D3E63D3E5E8 (ComponentId), 
        add constraint FK9B263D3E63D3E5E8 
        foreign key (ComponentId) 
        references Component (ComponentId)
		on delete cascade;

    alter table Metric 
        add index FK892AE1D02FA052BA (MeasureId), 
        add constraint FK892AE1D02FA052BA 
        foreign key (MeasureId) 
        references Measure (MeasureId)
		on delete cascade;

    alter table Module 
        add index FK89B0928C612DF65D (RuleId), 
        add constraint FK89B0928C612DF65D 
        foreign key (RuleId) 
        references Rule (RuleId);

    alter table PracticeResult_Repartition 
        add index FK84B2F9904B98962 (PracticeResultId), 
        add constraint FK84B2F9904B98962 
        foreign key (PracticeResultId) 
        references QualityResult (QualityResultId)
		on delete cascade;

    alter table Profile_DisplayConfBO 
        add index FKC122A97D802460C7 (Profile_ConfId), 
        add constraint FKC122A97D802460C7 
        foreign key (Profile_ConfId) 
        references displayConf (ConfId);

    alter table Profile_DisplayConfBO 
        add index FKC122A97D78D3AA46 (ProfileId), 
        add constraint FKC122A97D78D3AA46 
        foreign key (ProfileId) 
        references Tasks_User (AbstractTasksUserId);

    alter table Profile_Rights 
        add index FK62F82D6D9B681818 (AtomicRightsId), 
        add constraint FK62F82D6D9B681818 
        foreign key (AtomicRightsId) 
        references AtomicRights (AtomicRightsId);

    alter table Profile_Rights 
        add index FK62F82D6D75D5ECAE (ProfileId), 
        add constraint FK62F82D6D75D5ECAE 
        foreign key (ProfileId) 
        references ProfileBO (ProfileId);

    alter table Profiles_Grids 
        add index FK9949B158E49543C8 (GridId), 
        add constraint FK9949B158E49543C8 
        foreign key (GridId) 
        references QualityGrid (QualityGridId);

    alter table Profiles_Grids 
        add index FK9949B15878D3AA46 (ProfileId), 
        add constraint FK9949B15878D3AA46 
        foreign key (ProfileId) 
        references Tasks_User (AbstractTasksUserId);

    alter table ProjectParameter 
        add index FK6D7538B0AA5FCCA5 (ListId), 
        add constraint FK6D7538B0AA5FCCA5 
        foreign key (ListId) 
        references ProjectParameter (ParameterId);

    alter table ProjectParameter 
        add index FK6D7538B081587B7D (MapId), 
        add constraint FK6D7538B081587B7D 
        foreign key (MapId) 
        references ProjectParameter (ParameterId);

    alter table QualityResult 
        add index FKE9E7A9DC7E8A4226 (AuditId), 
        add constraint FKE9E7A9DC7E8A4226 
        foreign key (AuditId) 
        references AuditBO (AuditId)
		on delete cascade;

    alter table QualityResult 
        add index FKE9E7A9DC83596FA2 (ProjectId), 
        add constraint FKE9E7A9DC83596FA2 
        foreign key (ProjectId) 
        references Component (ComponentId)
		on delete cascade;

    alter table QualityResult 
        add index FKE9E7A9DC330CF5F3 (QualityRuleId), 
        add constraint FKE9E7A9DC330CF5F3 
        foreign key (QualityRuleId) 
        references QualityRule (QualityRuleId);

    alter table QualityRule 
        add index FK420ADC7B883C760C (Formula), 
        add constraint FK420ADC7B883C760C 
        foreign key (Formula) 
        references Formula (FormulaId);

    alter table Rule 
        add index FK270B1CE5BC5FE3 (RuleSetId), 
        add constraint FK270B1CE5BC5FE3 
        foreign key (RuleSetId) 
        references RuleSet (RuleSetId)
		on delete cascade;

    alter table RuleCheckingTransgressionItem 
        add index FKDF0D4973B5ECF9F1 (RuleId), 
        add constraint FKDF0D4973B5ECF9F1 
        foreign key (RuleId) 
        references Rule (RuleId);

    alter table RuleCheckingTransgressionItem 
        add index FKDF0D4973B26EE3F8 (MeasureId), 
        add constraint FKDF0D4973B26EE3F8 
        foreign key (MeasureId) 
        references Measure (MeasureId)
		on delete cascade;

    alter table RuleCheckingTransgressionItem 
        add index FKDF0D497363D3E5E8 (ComponentId), 
        add constraint FKDF0D497363D3E5E8 
        foreign key (ComponentId) 
        references Component (ComponentId)
		on delete cascade;

    alter table RuleCheckingTransgressionItem 
        add index FKDF0D4973C617EA6F (ComponentInvolvedId), 
        add constraint FKDF0D4973C617EA6F 
        foreign key (ComponentInvolvedId) 
        references Component (ComponentId);

    alter table RuleSet 
        add index FKBF8713A683596FA2 (ProjectId), 
        add constraint FKBF8713A683596FA2 
        foreign key (ProjectId) 
        references Component (ComponentId)
		on delete cascade;

    alter table SqualeReference 
        add index FK32FD7E08D0519AC (QualityGrid), 
        add constraint FK32FD7E08D0519AC 
        foreign key (QualityGrid) 
        references QualityGrid (QualityGridId);

    alter table Stats_squale_dict 
        add index FK9B3A9E52F53F33A0 (Serveur), 
        add constraint FK9B3A9E52F53F33A0 
        foreign key (Serveur) 
        references Serveur (ServeurId);

    alter table Stats_squale_dict_annexe 
        add index FKBC8FB71EF53F33A0 (Serveur), 
        add constraint FKBC8FB71EF53F33A0 
        foreign key (Serveur) 
        references Serveur (ServeurId);

    alter table TaskParameter 
        add index FK16AD3384A096529F (TaskRefId), 
        add constraint FK16AD3384A096529F 
        foreign key (TaskRefId) 
        references TaskRef (TaskRefId)
		on delete cascade;

    alter table TaskRef 
        add index FK797F8AE1ADBED57 (TaskId), 
        add constraint FK797F8AE1ADBED57 
        foreign key (TaskId) 
        references Task (TaskId)
		on delete cascade;

    alter table Termination_Task 
        add index FKC739A220A096529F (TaskRefId), 
        add constraint FKC739A220A096529F 
        foreign key (TaskRefId) 
        references TaskRef (TaskRefId);

    alter table Termination_Task 
        add index FKC739A220BE3D10F3 (TasksUserId), 
        add constraint FKC739A220BE3D10F3 
        foreign key (TasksUserId) 
        references Tasks_User (AbstractTasksUserId);

    alter table UserAccess 
        add index FKB60A252F6EEDFC90 (ApplicationId), 
        add constraint FKB60A252F6EEDFC90 
        foreign key (ApplicationId) 
        references Component (ComponentId)
		on delete cascade;

    alter table UserBO 
        add index FK9790197875D5ECAE (ProfileId), 
        add constraint FK9790197875D5ECAE 
        foreign key (ProfileId) 
        references ProfileBO (ProfileId);

    alter table User_Rights 
        add index FK21885CCBFDF9038A (UserId), 
        add constraint FK21885CCBFDF9038A 
        foreign key (UserId) 
        references UserBO (UserId);

    alter table User_Rights 
        add index FK21885CCB75D5ECAE (ProfileId), 
        add constraint FK21885CCB75D5ECAE 
        foreign key (ProfileId) 
        references ProfileBO (ProfileId);

    alter table User_Rights 
        add index FK21885CCB6EEDFC90 (ApplicationId), 
        add constraint FK21885CCB6EEDFC90 
        foreign key (ApplicationId) 
        references Component (ComponentId)
		on delete cascade;

    alter table Volumetry_Measures 
        add index FK92AE693368A4458F (VolumetryId), 
        add constraint FK92AE693368A4458F 
        foreign key (VolumetryId) 
        references displayConf (ConfId)
		on delete cascade;