...
public class UMLQualityBeanAdaptator {
     ...
     */
    public boolean adaptComponentResult(final UMLQualityMetricsBO pComponentResult)throws Exception {
        LOGGER.debug(UMLQualityMessages.getString("logs.debug.adapt_composant") + pComponentResult.getComponentName());
    ...
    name=UMLQualityUtility.getName(pComponentResult.getComponentName());//recupere le nom du composant
                                                               ...
                                                               // a été générés  
    if(name!=null){//nom non vide 
        parentName=UMLQualityUtility.getParentName(pComponentResult.getComponentName());//le nom du parent direct
        
        if(null!=parentName){//le parent direct n'existe pas
            AbstractComplexComponentBO conplexComponentBO= (AbstractComplexComponentBO)mComplexeComponents.get(
                                                   parentName);//recherche du parent direct 
            if(null!=conplexComponentBO){//le parent direct existe
                /*on instancie un composant  */
                component=UMLQualityUtility.newInstance(UMLQualityMessages.getString("uml.component.package.name")
                    ...
                    +UMLQualityMessages.getString("uml.component.sufix.name")
                    ,name);
               ...
               pComponentResult.setComponent(adaptedComponent);// établie le lien entre resutat(metrique) et composant
