...
public abstract class AbstractPmdProcessing {
     ...
     */
    protected List buildFilesToProcess(TaskData pData, MapParameterBO pProjectParams) throws ConfigurationException, IOException {
     ...
     * @throws IOException si erreur
     */
    public Report process(TaskData pData, MapParameterBO pProjectParams, File pRuleSetFile) throws IOException, ConfigurationException {
        ...
        try {
            ...
            throw new ConfigurationException(PmdMessages.getString("exception.ruleset.notfound", pRuleSetFile.getAbsolutePath()), e);
        }
        ...
        // Traitement de PMD
        pmd.processFiles(buildFilesToProcess(pData, pProjectParams), ctx, rulesets, false, // mode debug
        true, // simplification des noms de fichier avec le préfixe suivant
         (String) pData.getData(TaskData.VIEW_PATH), new InputStreamReader(System.in).getEncoding());
        report.end();
        return report;
    }
}
