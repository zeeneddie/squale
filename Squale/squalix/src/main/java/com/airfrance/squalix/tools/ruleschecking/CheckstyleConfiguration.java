 ...
 * La configuration Checkstyle est définie dans un fichier XML, celui-ci
 * est lu par cette classe.
 */
public class CheckstyleConfiguration extends XmlImport {
    /**
     * Logger.
     */
    ...
    public void parse(InputStream pStream) throws ConfigurationException {
        ...
        Digester digester = preSetupDigester("-//Checkstyle Configuration DTD 1.0//EN", "/com/airfrance/squalix/tools/ruleschecking/checkstyle-config-1.0.dtd", errors);
        // Traitement du répertoire de génération des reports
        digester.addCallMethod("checkstyle-configuration/reportDirectory", "setReportDirectory", 1, new Class[]{String.class});
        digester.addCallParam("checkstyle-configuration/reportDirectory", 0);
        // Traitement du répertoire des jars
        digester.addCallMethod("checkstyle-configuration/jarDirectory", "setJarDirectory", 1, new Class[]{String.class});
        digester.addCallParam("checkstyle-configuration/jarDirectory", 0);
        digester.push(this);
        // Appel du parser
        parse(digester, pStream, errors);
        if (errors.length()>0) {
            throw new ConfigurationException(RulesCheckingMessages.getString("error.configuration", new Object[]{errors.toString()}));
        }
    }
    /**
     * @return directory
