...
/**
 * Exécute l'analyse McCabe. L'environnement McCabe doit être correctement
 * initialisé avant le lancement de la tâche, ou la commande "cli" appelée doit pointer vers
 * un script qui le met en place.<br><br>
 * Il est important que la génération des rapports débute par celle du niveau méthode, afin
 * de pouvoir correctement créer les composants ClassBO si nécessaire.<br>
 * L'ordre des rapports dans le configuration est respécté.
 */
public abstract class AbstractMcCabeTask extends AbstractTask implements ProcessErrorHandler, ProcessOutputHandler {
    ...
    public void execute() throws TaskException {
        try {
            initialize();
            LOGGER.info(McCabeMessages.getString("logs.analyzing") + mProject.getParent().getName() + " - " + mProject.getName());
    ...
    private void initialize() throws Exception {
        ...
        mConfiguration = McCabeConfiguration.build(mProject, McCabeMessages.getString("configuration.file"), getData());
        try {
            ...
            // On log l'erreur
            LOGGER.warn("Erreur sur le fichier de log des erreurs : " + mConfiguration.getErrorLogger().getAbsolutePath());
            LOGGER.warn(ioe);
        }
        ...
        setParser();
        setClassTemplate();
        mPersistor = new McCabePersistor(mParser, mConfiguration, mAudit, getSession(), getData(), mName, mClassTemplate);
        LOGGER.info(McCabeMessages.getString("logs.initialized") + mProject.getParent().getName() + " - " + mProject.getName());
    }

    /**
     * Création du fichier de configuration McCabe
     * Ce fichier de configuration contient la liste des fichiers
     * @param pFile fichier
    ...
    protected void createProjectConfigurationFile(McCabePCFFile pFile) throws Exception {
        ...
        // est < 10
        File backupDir = new File(mConfiguration.getWorkspace() + File.separator + McCabeMessages.getString("pcf.log.dir"));
        backupDir.mkdirs();
                ...
                if (fileName.matches(".*\\." + (McCabeConfiguration.MAX_PCF_SAVED-1))) {
                    ...
                    if(lastDot > 0) {
                        try {
                            ...
                            ((File)files.get(i)).renameTo(new File(backupDirFiles[i].getParentFile().getAbsoluteFile() + File.separator + newFileName));
    ...
    /**
     * Crée le fichier contenant le chemin vers les modèles de rapports McCabe et celui
     * avec les paramètres spéciaux utilisateurs.
     * 
    ...
    protected void createSpcFile(final McCabeConfiguration pConfiguration) throws IOException {
        String spcFileName = pConfiguration.getSubWorkspace().getAbsolutePath() + File.separator + McCabeMessages.getString("reports_db.name");
        ...
        bw.close();
        File repository = new File(pConfiguration.getSubWorkspace().getAbsolutePath() + File.separator + McCabeMessages.getString("file.user.def.header"));
        repository.mkdir();
        String userFileName = pConfiguration.getSubWorkspace().getAbsolutePath() + File.separator + McCabeMessages.getString("user_def.name");
    ...
    protected int parseSource() throws Exception {
        ...
        LOGGER.info(McCabeMessages.getString("logs.running_parsing", new Object[] { mProject.getParent().getName(), mProject.getName()}));
        ...
        LOGGER.info(McCabeMessages.getString("logs.return_parsing", new Object[] { mProject.getParent().getName(), mProject.getName(), new Integer(resultParse)}));
    ...
    protected void createReport(final String pReport) throws Exception {
        ...
        for (int j = 0; j < mConfiguration.getMetricsCommand().length; j++) {
            ...
            param = mConfiguration.getMetricsCommand()[j].replaceAll(McCabeMessages.getString("key.substition.report_name"), pReport);
     ...
     * @throws Exception si erreur
     */
    private void parseReport(final String pReport) throws Exception {
        String reportFileName = computeReportFileName(pReport);
    ...
    /**
     * Crée le ProcessManager.
     * On ne fait pas de new mais un set pour implémenter 
     * le pattern IOC pour pouvoir tester sur un environnement windows
    ...
    public void processOutput(String pOutputLine) {
        ...
        if (mConfiguration != null && !mConfiguration.ignoreMsg(pOutputLine)) {
            ...
            if (pOutputLine.startsWith("ERR")) {
                ...
                mCurrentComponent = pOutputLine.replaceFirst("cli: Processing " + (String) mData.getData(TaskData.VIEW_PATH), "");
            }
