...
/**
 */
public abstract class RSMTask extends AbstractTask implements ProcessErrorHandler, ProcessOutputHandler, CSVParser.CSVHandler {
     ...
     */
    private List buildFilesToProcess(TaskData pData, MapParameterBO pProjectParams) throws ConfigurationException {
        ...
        List resultSources = FileUtility.getIncludedFiles(
            viewPath,
    ...
    public void execute() throws TaskException {
        try {
            ...
            LOGGER.info(RSMMessages.getString("logs.analyzing") + mProject.getParent().getName() + " - " + mProject.getName());
            int execResult = parseSource();
            // un nombre négatif est une erreur d'exécution RSM
            if (execResult < 0) {
                throw new TaskException(RSMMessages.getString("rsm.exec.erreur", new Object[] { new Integer(execResult)}));
            }
     ...
     */
    private void buildDotLstFile(List pFilesToAnalyseList) throws IOException {
    ...
    private void initialize() throws Exception {
        ...
        LOGGER.info(RSMMessages.getString("logs.initialized") + mProject.getParent().getName() + " - " + mProject.getName());
    }
     ...
     */
    private int parseSource() throws Exception {
        // le résultat renvoyé
        int resultParse = 0;
        ...
        LOGGER.info(RSMMessages.getString("logs.running_parsing", new Object[] { mProject.getParent().getName(), mProject.getName()}));
        ...
        LOGGER.info(RSMMessages.getString("logs.return_parsing", new Object[] { mProject.getParent().getName(), mProject.getName(), new Integer(resultParse)}));
        return resultParse;
    }
