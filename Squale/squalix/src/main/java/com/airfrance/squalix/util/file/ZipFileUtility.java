...
/**
 * Utilitaire pour les fichiers compressés
 * Le type d'archive est connu en exécutant la commande 'file' d'unix.
 * Dans le cas où cette commande échouerait (si l'exécution se fait sous windows par exemple), on reconnait
 * le type grâce à l'extension du fichier.
 */
public class ZipFileUtility {
    ...
    public static final String[] ARCHIVE_RESULTS = { POSIX_TAR_RESULT, TAR_RESULT, GZIP_RESULT, BZIP2_RESULT, ZIP_RESULT, JAR_RESULT };
    ...
    public static final String[] ARCHIVE_EXTENSIONS = { TAR_EXTENSION, JAR_EXTENSION, GZIP_EXTENSION, BZIP2_EXTENSION, ZIP_EXTENSION, EAR_EXTENSION, WAR_EXTENSION, TGZ_EXTENSION };

    /**
     * Extrait les fichiers et répertoires contenus dans pArchive
     * dans le répertoire de destination pDest
     * PRECONDITION : pArchive et pDest existent
     * @param pArchive l'archive à décompresser
     * @param pDest le répertoire de destination
     * @throws Exception si erreur
     */
    public static void extractArchiveFile(File pArchive, File pDest) throws Exception {
     ...
     * @throws Exception si erreur
     */
    public static void extractArchiveFile(File pArchive, File pDest, String pArchiveType) throws Exception {
    ...
    private static boolean isZipFile(String pArchiveType) {
        ...
        result =
            result
                || pArchiveType.equalsIgnoreCase(ZIP_EXTENSION)
                || pArchiveType.equalsIgnoreCase(EAR_EXTENSION)
                || pArchiveType.equalsIgnoreCase(WAR_EXTENSION)
                || pArchiveType.equalsIgnoreCase(JAR_EXTENSION);
        return result;
    }
    ...
    private static boolean isTarFile(String pArchiveType) {
        // Dans le cas de la commande file
        boolean result = pArchiveType.lastIndexOf(TAR_RESULT) != -1 || pArchiveType.lastIndexOf(POSIX_TAR_RESULT) != -1;
        ...
        result = result || pArchiveType.equalsIgnoreCase(TAR_EXTENSION) || pArchiveType.equalsIgnoreCase(TGZ_EXTENSION);
        ;
        return result;
    }
     ...
     * @return l'exécution de la commande "type" d'unix si pFile est une archive
     ...
     */
    public static String isArchiveFile(File pFile) throws IOException, InterruptedException {
        String cmdResult = "";
