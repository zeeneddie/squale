 ...
 * Une tâche est responsable de la création et de la persistance de ses objets de 
 * résultats (implémentant l'interface <code>MeasureBO</code>) ainsi que de ses 
 * erreurs (classe <code>ErrorBO</code>).
 * <br /><br />
 * Une tâche peut posséder :
 * <ul>
 * <li>une tâche parent : tant que la tâche parent n'est pas terminée avec succès, celle-ci
 * ne peut être lancée,</li>
 * <li>et/ou des tâches enfants : lancées dès la complétude avec succès de la tâche courante.<li>
 * </ul>
 * <br />
 * Les attributs nécessaires à une classe lui sont attribués via un pattern IOC. Une fois
 * que la tâche a été instanciée, c'est son observateur (le Scheduler ou l'exécuteur d'analyse)
 * qui lui transmet l'instance d'audit, de projet, son statut, et ses tâches enfants. Elle
 * n'est responsable que de se nommer lors de son instanciation. Le constructeur ne doit prendre
 * aucun paramètre.
 * <br /><br />
 * Si la tâche échoue, elle doit passer en statut FAILED, et générer des erreurs dans la base.
 * Il est plus que vivement conseillé de ne pas utiliser les instance de projet et d'audit
 * lorsque les résultats sont persistés, il faut utiliser des nouvelles instances directement
 * à partir des DAO, avec les id des instances offertes par l'observateur. 
 * <br /><br />
...
public abstract class AbstractTask implements Runnable {
    ...
    /**
     * Précise que l'exécution de la tâche est suspendue en raison
    ...
    /**
     * indique si la tache est obligatoire ou optionnelle
    ...
    /**
     * @return l'objet contenant l'ensemble des parametres temporaires
     * définis
     */
        ...
        if (mErrors.size() > 0) {
            ...
            while (it.hasNext()) {
                ...
                if (!it.hasNext() && mStatus == FAILED) {
                    ...
                    String[] infos = new String[] { mName, mApplication.getName(), mApplication.getServeurBO().getName(), mProject.getName(), "" + mAudit.getId(), hour };
                    String object = Messages.getString("mail.sender.squalix.task", new String[]{mApplication.getName()}) + Messages.getString("mail.task.failed.object", infos);
     ...
     * qui effectue le travail propre à la tache
     * et ferme la session
     */
                ...
                if (pParam instanceof StringParameterBO) {
                    ...
                    if (pParam instanceof ListParameterBO) {
                        ...
                        for (int i = 0; i < list.size(); i++) {
                            ...
                            mMaxFileSystemSize += calculateRecursiveSize(new File(((StringParameterBO) (list.get(i))).getValue()));
                        }
                    }
                }
            }
     ...
     * @return la taille du fichier ou la taille de l'ensemble du répertoire
     * si f est un répertoire
     */
