...
/**
 * Calcule tous les ROI pour les audits terminés
 * (Pour exécuter le main sous unix mettre son nom dans le manifest.mf dans Main-Class)
 */
public class RoiMain {
     ...
     */
    private static void createROI(ISession pSession, int pValue, ApplicationBO pApplication, AuditBO pAudit) throws JrafDaoException {
     ...
     * @throws JrafDaoException si erreur
     */
    public static int calculateNbCorrection(ISession pSession, AuditBO pPreviousAudit, AuditBO pCurrentAudit, Log pLogger) throws JrafDaoException {
     ...
     */
    public static void main(String pArgs[]) throws JrafPersistenceException {
        ...
        try {
            ...
            for (int i = 0; i < audits.size(); i++) {
                ...
                if (null == measureDAO.load(session, new Long(currentApplication.getId()), auditId, RoiMetricsBO.class)) {
                    // On récupère l'audit précédent
                    previousAudit = auditDAO.findPreviousAudit(session, currentAudit.getId(), currentAudit.getHistoricalDate(), currentApplication.getId(), null);
