 ...
 * Il s'agit d'une tâche SQUALIX. En conséquence, cette classe hérite de 
 * la classe <code>Task</code>.
 * @author m400832 (by rose)
 * @version 2.0
 * @see com.airfrance.squalix.core.Task
 */
public class ClearCaseTask extends AbstractTask implements ProcessErrorHandler {
    ...
    /**
     * {@inheritDoc}
     * Constructeur. Initialisation des premiers paramètres. <br />
    ...
    /**
     * {@inheritDoc}
     * Créé la configuration clearcase 
     * et charge l'application et le projet en base 
    ...
    /**
     * Cette méthode modifie le paramètre 
     * <code>view_path</code> de la <code>
     * HashMap</code> des paramètres temporaires de l'objet <code>
     * ProjectBO</code>.
     * @throws Exception exception lors du traitement.
     */
    private void modifyTempMap() throws Exception {
        /* on récupère le chemin du dossier dans lequel ont été mis les fichiers 
         * de la vue, et on met cette valeur dans la hashmap de paramètres. */
    ...
    /**
     * Cette méthode vérifie si une vue est actuellement montée sur la machine 
     * et/ou si le répertoire de stockage de la vue existe déjà.
     * @throws Exception si le processus ne peut être lancé.
     * @return <code>true</code> si elle est actuellement montée et/ou si le répertoire de stockage de la vue existe déjà, 
     * <code>false</code> sinon.
     */
    protected boolean checkViewOrStorageDirectoryExistence() throws Exception {
        ...
        return (mgr.startProcess(this) == 0 || new File(removeCommand.substring(removeCommand.indexOf("/"), removeCommand.length())).exists());
    }

    /**
     * Cette méthode monte la vue ClearCase.
     * @return <code>true</code> en cas de succès, <code>false</code> sinon.
     * @throws Exception si le script de montage de la vue a produit une ou 
     * plusieurs erreurs.
     */
    protected boolean mountView() throws Exception {
        String cmdLine;
    ...
    protected boolean cleanView() throws Exception {
        ...
        if (!cleaningOk) {
            ...
            // le .vws
            File f = new File(removeDircommand.substring(removeDircommand.indexOf("/"), removeDircommand.length()));
            if (f.exists()) {
                mgr = new ProcessManager(removeDircommand, null, new File("."));
                LOGGER.info("Remove dir command:" + removeDircommand);
                cleaningOk = mgr.startProcess(this) == 0;
            }
            if (!cleaningOk) { // sinon c'est le .vws à supprimer
    ...
    public void processError(String pErrorMessage) {
        ...
        if (pErrorMessage.toLowerCase().startsWith("cleartool: ") && !pErrorMessage.toLowerCase().startsWith("cleartool: error: no matching entries found for view tag")) {
