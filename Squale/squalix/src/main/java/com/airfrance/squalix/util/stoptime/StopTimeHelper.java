...
public class StopTimeHelper {
     ...
     */
    public StopTimeHelper(SqualixConfigurationDTO pConf, Calendar pLaunchingCal) throws ConfigurationException {
     ...
     * @throws ConfigurationException si erreur
     */
    private void buildLimitTime(SqualixConfigurationDTO pConf, Calendar pLaunchingCal) throws ConfigurationException {
        ...
        long ref = pLaunchingCal.get(Calendar.DAY_OF_WEEK)*MINUTES_PER_DAY+pLaunchingCal.get(Calendar.HOUR_OF_DAY)*MINUTES_PER_HOUR+pLaunchingCal.get(Calendar.MINUTE);
        while (it.hasNext()&&(result==null)) {
            StopTimeDTO time = (StopTimeDTO) it.next();
            long t = convertStringDayToIntDay(time.getDay())*MINUTES_PER_DAY+convertStringHourToMinutes(time.getTime());
    ...
    /**
     * 
     * @return un booléen indiquant si l'heure limite 
     * d'exécution de squalix a été dépassée ou pas
     */
    ...
    /**
     * Convertit un jour de la semaine en sa valeur correspondante 
     ...
     * Comparateur de date
     */
    class StopTimeComparator implements Comparator {
        ...
        public int compare(Object o1, Object o2) {
            ...
            int result = 0;
            long d1 = convertStringDayToIntDay(st1.getDay())*MINUTES_PER_DAY+convertStringHourToMinutes(st1.getTime());
            long d2 = convertStringDayToIntDay(st2.getDay())*MINUTES_PER_DAY+convertStringHourToMinutes(st2.getTime());
            if (d1>d2) {
                result = 1;
            } else if (d1<d2) {
                result = -1;
