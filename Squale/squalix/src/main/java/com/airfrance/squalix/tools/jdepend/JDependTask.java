 ...
 */
public class JDependTask extends AbstractTask {
    ...
    /** liste des sources à analyser (en tenant compte des exclusions et inclusions, 
     * (récupérées car réutilisées) */
    ...
    public void execute() throws TaskException {
        try {
            ...
            List srcs = (List) ((ListParameterBO) mProject.getParameters().getParameters().get(ParametersConstants.SOURCES)).getParameters();
    ...
    /**
     * Méthode qui crée la liste des packages considérés comme valide,
     * c'est à dire qu'ils font partie de l'arborescence directe du répertoire racine à analyser.
     * Par conséquent on ne prend pas en compte les packages "java.*" ...
     ...
     */
    private void buildValidPackagesSet(File pRootFile, String pParent) throws IOException {
        ...
        if (pRootFile.canRead()) {
            ...
            if (((list.length == 0 || (list.length != 0 && pRootFile.listFiles().length != 0)) && isMemberOfParameterList(mIncludedFileNames, pParent.replace('.', '/')))) {
    ...
    /**
     * indique si le fichier ou le répertoire dont le nom est pSuffix 
     * appartient à l'ensemble des sources ou des répertoires exclus 
     * (suivant la liste passée en argument) en effectuant une recherche récursive.
     * @param pSuffix le nom du package transformé dont les "." ont été remplacés par des "/"
     * @param pList la liste des noms absolus des fichiers
