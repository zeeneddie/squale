...
/**
 * Cette classe gère la configuration pour les tâches de montage et démontage 
 * des vues ClearCase snapshot.
...
public class ClearCaseConfiguration {
    ...
    /** 
     * le répertoire correspondant au chemin de la vue + nom de la branche
     * Permet de récupérer la taille du file system que crée la tache
     */
    ...
    /**
     * Commande permettant de supprimer le répertoire récursivement
     * dans le cas où le répertoire existe mais pas la vue  
     */
    private String mRemoveDirectoryCommand;

    /**
     * Commande permettant de supprimer la vue quand le .view existe encore mais plus
     * le répertoire
     */
     ...
     * @param pAudit audit.
     */
    public ClearCaseConfiguration(ProjectBO pProject, AuditBO pAudit) throws Exception {
        ...
        if (pAudit.getType().equals(AuditBO.MILESTONE)) {
            ...
            branchName = ((StringParameterBO) clearcaseMapBO.getParameters().get(ParametersConstants.BRANCH)).getValue();
        }

        // Recupere l'application du projet
        String appli = ((StringParameterBO) clearcaseMapBO.getParameters().get(ParametersConstants.APPLI)).getValue();

        List vobs = ((ListParameterBO) clearcaseMapBO.getParameters().get(ParametersConstants.VOBS)).getParameters();
    ...
    /**
     * Remplace le tag <code>{APP_NAME}</code> par la valeur fournie en 
     * paramètre. Cette méthode est appelée par la tâche <code>
     * ClearCaseTask</code>, une fois que cette dernière a récupéré le nom 
     * ClearCase de l'application.
    ...
    /**
     * Casse la liste des vobs suivant le séparateur fourni dans le fichier 
     * <code>common_messages.properties</code>. <br />
    ...
    /**
     * Remplace le tag <code>{BRANCH_NAME}</code> par la valeur fournie en 
     * paramètre.
    ...
    private void processBranchName(AuditBO pAudit, String pBranchName) {
        ...
        if (pAudit.getType().equals(AuditBO.MILESTONE)) {
            ...
            /* on met toute la commande en minuscules (n'a dans l'effet 
             * d'impact que sur le nom du jalon, le reste étant déjà en 
             * minuscules) */
            mVerifyViewExistenceCommand = mVerifyViewExistenceCommand.toLowerCase();

            /* on agit de la même manière pour les commandes de montage 
             * et de démontage d'une vue de jalon */
    ...
    private void processBranchSuffix(String pApplicationName, String pProjectName) {
        /* on concatène le nom de l'application, un underscore, 
         * et le nom du projet */
    ...
    /**
     * Cette méthode est appelée dans le cas d'un audit de jalon pour mettre en 
     * minuscules une partie précise (le nom de la branche) de la commande.<br />
     * Typiquement, elle modifie la chaîne de la façon suivante : <br />
     * <pre>/usr/atria/bin/Perl -S /DINB/outils/gcl/script/mkview.pl -application 
    ...
    private String lowerBranchName(String pCommand) {
        ...
        /* on cherche la position du caractère situé juste après le dernier 
         * séparateur UNIX ("/") de la chaîne. */
    ...
    /**
     * Cette méthode remplace un motif par sa valeur pour toutes les commandes 
     * UNIX disponibles dans le fichier xml de configuration.
    ...
    /**
     * Cette méthode provoque la récupération des chemins, des commandes 
     * UNIX et des patterns nécessaires à la tâche ClearCase.<br />
     * <b>ATTENTION :</b> les données de la balise <code>commands</code> doivent 
     * absolument être récupérées en premier.<br />
     * En effet, la méthode <code>processFromXML(Node pNode)</code> sur le 
     * noeud <code>patterns</code> fait appel 
     * à la méthode <code>replace(String pPattern, String pValue)</code> qui 
     * lance une <code>NullPointerException</code> si les variables contenant 
     * les commandes UNIX n'ont pas été initialisées.
     * @param pFile chemin du fichier à charger.
     * @throws Exception en cas d'erreur de parsing du fichier XML de
     * configuration.
    ...
    private void getConfigurationFromXML(final String pFile) throws Exception {
        ...
        if (null != root) {
            ...
            Node myNode = ConfigUtility.getNodeByTagName(root, ClearCaseMessages.getString("configuration.commands"));
            /* on traite les noeuds command en appliquant la méthode 
             * "mapKeyValue" */
            processFromXML(myNode, ClearCaseMessages.getString("configuration.commands.command"), ClearCaseMessages.getString("configuration.commands.command.key"), "mapKeyValue");
            ...
            processFromXML(myNode, ClearCaseMessages.getString("configuration.patterns.pattern"), ClearCaseMessages.getString("configuration.patterns.pattern.key"), "replace");
            ...
            /* on traite les noeuds option en appliquant la méthode 
             * "mapKeyValue" */
            processFromXML(myNode, ClearCaseMessages.getString("configuration.options.option"), ClearCaseMessages.getString("configuration.options.option.key"), "mapKeyValue");
    ...
    /**
     * Dans un premier temps, cette méthode récupère l'attribut et la valeur 
     * d'un noeud donné. Puis elle appelle la méthode dont le nom est passé en 
     * paramètre par réflexion. <br />
     * Elle ne fonctionne que si la méthode dont le nom est passé en paramètre 
     * prend elle-même pour paramètres 2 <code>java.lang.String</code>.
     ...
     * @param pMethodName le nom de la méthode à appeler par réflexion. 
     * ATTENTION : on ne peut passer en paramètre que des méthodes prenant 
     * elles-même 2 <code>java.lang.String</code> en paramètres.
     * @throws Exception génère une exception si le nom du noeud ne correspond pas 
     * à une des valeurs définies dans le fichier de configuration <code>
     * com.airfrance.squalix.tools.clearcase.clearcase.properties</code>.
     * @see ConfigUtility
     */
    private void processFromXML(final Node pNode, final String pRootAnchor, final String pChildAnchor, final String pMethodName) throws Exception {
        /* on récupère le 1er noeud contenant une commande */
        Node myNode = ConfigUtility.getNodeByTagName(pNode, pRootAnchor);

        /* instanciation des variables qui vont servir dans la boucle 
         * qui va suivre */
        ...
        while (null != myNode) {
            ...
            if (Node.ELEMENT_NODE == myNode.getNodeType()) {
                ...
                if (null != attrName && !"".equals(attrName)) {
                    ...
                    /* on crée la tableau de type de paramètres 
                     * ici, un tableau de 2 String */
    ...
    /**
     * Cette méthode attribue à des variables de classes des valeurs par 
     * réflexion.<br />
     * La méthode est utilisée, mais uniquement par réflexion.
     * @param pKey nom de commande.
     * @param pValue valeur de la commande.
     * @throws Exception exception si le nom de la commande passé en 
     * paramètre n'est pas reconnu.
     * @see #createReflectionMap()
     */
    protected void mapKeyValue(final String pKey, final String pValue) throws Exception {
        /* on invoque le setter correspondant à la clé pKey, 
         * en lui passant la valeur pValue */
        Object[] obj = { pValue };
    ...
    private void createReflectionMap() throws Exception {
        ...
        mMap.put(ClearCaseMessages.getString("configuration.commands.command.key.verify_view_existence"), this.getClass().getDeclaredMethod("setVerifyViewExistenceCommand", param));

        /* commande montant une vue de travail -> audit de suivi */
        mMap.put(ClearCaseMessages.getString("configuration.commands.command.key.mount_work_view"), this.getClass().getDeclaredMethod("setMountWorkViewCommand", param));

        /* commande montant une vue de consultation -> audit de jalon */
        mMap.put(ClearCaseMessages.getString("configuration.commands.command.key.mount_consultation_view"), this.getClass().getDeclaredMethod("setMountConsultationViewCommand", param));

        /* commande de démontage d'une vue */
        mMap.put(ClearCaseMessages.getString("configuration.commands.command.key.umount_view"), this.getClass().getDeclaredMethod("setUmountViewCommand", param));

        /* commande de démontage d'une vue dans le cas où le répertoire n'existe plus */
        mMap.put(ClearCaseMessages.getString("configuration.commands.command.key.aux_umount_view"), this.getClass().getDeclaredMethod("setAuxUmountViewCommand", param));

        /* commande de suppression récursive du répertoire quand le .view n'existe plus */
        mMap.put(ClearCaseMessages.getString("configuration.commands.command.key.remove_directory"), this.getClass().getDeclaredMethod("setRemoveDirectoryCommand", param));

        /* option -vob */
        mMap.put(ClearCaseMessages.getString("configuration.options.option.key.vob"), this.getClass().getDeclaredMethod("setVobOption", param));
    }

    /**
     * Getter.
     * @return la commande UNIX permettant de monter une vue de consultation sous 
     * ClearCase.
     */
    ...
    /**
     * Getter.
     * @return la commande UNIX permettant de monter une vue de travail sous 
     * ClearCase.
     */
    ...
    /**
     * Getter.
     * @return la commande UNIX permettant de démonter une vue sous 
     * ClearCase.
     */
    ...
    /**
     * Getter.
     * @return la commande UNIX permettant de vérifier l'existence d'une vue sous 
     * ClearCase.
     */
    ...
    /**
     * Setter.
     * @param pMountConsultationViewCommand la commande UNIX permettant de monter 
     * une vue de consultation sous ClearCase.
     */
    ...
    /**
     * Setter.
     * @param pMountWorkViewCommand la commande UNIX permettant de monter une vue 
     * de travail sous ClearCase.
     */
    ...
    /**
     * Setter.
     * @param pUmountViewCommand la commande UNIX permettant de démonter une vue sous 
     * ClearCase.
     */
    ...
    /**
     * Setter.
     * @param pVerifyViewExistenceCommand la commande UNIX permettant de vérifier 
     * l'existence d'une vue sous ClearCase.
     */
