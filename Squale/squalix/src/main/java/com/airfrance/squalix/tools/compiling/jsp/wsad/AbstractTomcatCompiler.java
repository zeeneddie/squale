...
public abstract class AbstractTomcatCompiler {
    ...
    /**
     * Compile les JSPs du projet <code>mJ2eeProject</code>
     * Les jsps contenues dans les répertoires dont le nom contient des caractères
     * spéciaux (#, §,...) seront ignorées de la compilation car Java ne reconnait pas certains
     ...
     */
    public void compileJsp() throws BuildException, IOException {
        Java java = null;
        List jspList;
        String currentJsp;
        String jspName;
            ...
            for (int j = 0; j < jspList.size(); j++) {
                ...
                java =
                    getJavaExecutable(
                        ...
                        JspFileUtility.convertWithJspNameMangler(currentFile.getName().replaceAll("\\.jsp", "")),
                        movedJsp);
                // On exécute
                java.execute();
    ...
    private String getPackage(int pId, String pJspDir) {
        ...
        String relativePath = pJspDir.replaceAll("\\\\", "/").replaceFirst(((String) mJ2eeProject.getJspPaths()[pId][J2eeWSADProject.DIR_ID]).replace('\\', '/') + "/*", "");
