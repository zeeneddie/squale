...
/**
 * A partir du fichier de configuration globale, il détermine la séquence des 
 * actions pour le projet associé et les exécute.<br />
 * Le scheduler assure :<ul>
 ...
 * <li>la modification du statut de l'audit lorsque celui-ci est terminé, et la création éventuelle du prochain
 * audit dans le cas d'un audit de suivi.</li> 
 * </ul>
 * <br />
 ...
 * de ressources lorsqu'il ferme ses pools.
 * <br /><br />
 * Les tâches non liées à un projet et les exécuteurs d'analyse sont associés au scheduler par un pattern
 * Observateur-Observable, dans lequel le scheduler est l'observateur. Sa méthode <code>update()</code>
 * est appelée lorsque l'exécuteur ou la tâche est terminé.
 * 
...
public class Scheduler extends Thread {
     ...
     */
    public Scheduler(long pSiteId) throws ConfigurationException {
        try {
    ...
    private List getAudits(StopTimeHelper pStopTime) throws JrafDaoException {
        ...
        List audits = (List) AuditDAOImpl.getInstance().findBeforeBySiteAndStatus(mSession, mSiteId, AuditBO.NOT_ATTEMPTED, pStopTime.getLimitCal().getTime());
    ...
    /**
     * Récupère la liste des projets d'un audit, et 
     * exécute les analyses dans l'ordre.
     * L'audit est rattaché à chaque projet
    ...
    public void run() {
        try {
            ...
            if (existsAlreadyRunningAudits(mSiteId)) {
                ...
                // SqualeCommonUtils.notifyByEmail(mMailer, null, SqualeCommonConstants.ONLY_ADMINS, null, object, content, false);
            }
            // Exécuter les audits
    ...
    /**
     * On fait une passe sur toutes les applications ayant un audit programmé pour 
     * vérifier les règles défini dans le paramétrage de la fréquence. 
     * On baisse ensuite la fréquence des applications si besoin et on envoie un mail 
     * de notification de changement de paramétrage à l'administrateur de l'application 
     * et aux admins SQUALE.
    ...
    private void manageAuditFrequency() throws JrafDaoException {
        ...
        for (Iterator it = applis.iterator(); it.hasNext();) {
            ...
            if (appli.changeFrequency(AuditFrequencyTransform.dto2bo(mConf.getFrequencies()))) {
                ...
                String content = header + Messages.getString("mail.application_frequency.changed.content", new String[] { appli.getName(), "" + oldFreq, "" + appli.getAuditFrequency() });
                String dest = SqualeCommonConstants.ONLY_ADMINS;
             // TODO FAB : décommenter quand l'envoi d'email sera OK
                // SqualeCommonUtils.notifyByEmail(mMailer, null, SqualeCommonConstants.MANAGERS_AND_ADMINS, null, object, content, false);
            }
        }
        mSession.commitTransactionWithoutClose();
    }
    ...
    private boolean isRotationDay() throws JrafDaoException {
        ...
        if (coll != null && coll.size() != 0) {
            ...
            while (it.hasNext()) {
                ...
                result = audit.getDate() != null && auditDate.get(Calendar.DAY_OF_WEEK) <= today.get(Calendar.DAY_OF_WEEK);
            }
        } else {
    ...
    /**
     * Lance l'exécution d'un audit et donc de 
     * tous ses exécuteurs d'audit pour les projets 
    ...
    public int calculateNbCorrection(AuditBO pPreviousAudit, AuditBO pCurrentAudit) throws JrafDaoException {
        ...
        if (pCurrentAudit.getRealDate().before(pPreviousAudit.getRealDate())) {
            ...
            LOGGER.info(CoreMessages.getString("roi.not_calculated", new Object[] { new Long(pCurrentAudit.getId())}));
        } else {
    ...
    * @throws JrafEnterpriseException en cas d'échec de sauvegarde de l'outil
     */
    private void manageSqualeReference(AuditBO pAudit, String pAppliName, long pAppliId) throws JrafEnterpriseException {
        ...
        if (pAudit.getStatus() == AuditBO.TERMINATED
            && (null == storedAudit
                || ((AuditBO.NORMAL.equals(storedAudit.getAuditType()) && AuditBO.MILESTONE.equals(pAudit.getType()))
                    || (storedAudit.getAuditType().equals(pAudit.getType()) && pAudit.getDate().getTime() > storedAudit.getDate().getTime())))) {
     ...
     * pour Oracle gestion des tables partitionnées...
     ...
     * @throws JrafDaoException en cas de problème lors de la récupération ou
     * de la sauvegarde des données
     * @throws InterruptedException en cas d'interruption inopiné du thread
     */
    private boolean admindb(StopTimeHelper pStopTime) throws InterruptedException, JrafDaoException {
        boolean result = true;
        String object = "";
        String content = "";
        String sender = Messages.getString("mail.sender.squalix");
