...
/**
 * Parse les noms de type java et les remplace par les objets correspondants.<br/>
 * Cette classe est implémentée de façon à fonctionner avec les nom spéciaux remontés par McCabe.<br/>
 * On ne traite pas les cas McCabe directement dans les classes McCabes car cela dépend du langage.
 */
public class JavaParser implements LanguageParser {
     ...
     *  une distinction entre un package et un nom de classe. McCabe utilise
    ...
    /** 
     * Ensemble des classes anonymes remontées par McCabe.<br/>
     * On utilise une LinkedHashSet pour ne pas avoir de doublons et pour
     * l'ordre dans lequel sont insérés les noms soit conservé.<br/>
     * Ce set va nous permettre de numéroter les classes anonymes remontées
     * par McCabe car on va sauvegarder ces noms de classes lorsqu'une méthode
     * définie dans une classe anonyme sera parsée.<br/>
     * Le rpt de McCabe doit ordonner les méthodes par ordre croissant de leur
     * numéro de ligne (on suppose ainsi que le compilateur java défini l'identifiant
     * d'une classe anonyme par le même principe).
     */
    ...
    /**
     * Retourne le nom entièrement qualifié du package de la classe <code>pFullClassName</code><br/>
     * Si la classe n'a pas de package, retourne une chaine vide.
     ...
     * nom relatif.
     * @param pAbsoluteName le nom entièrement qualifié de la classe
     * (ou le nom entièrement qualifié de la méthode)
     ...
     * <ul>
     * <li>Le nom donné par le compilateur java à une classe anonyme est : nomClass$Id
     * (Id est l'identifiant généré par le compilateur).</li>
     * <li>Le nom donné par McCabe pour une classe anonyme est : __Anon_Id
     * (Id pouvant être 001, 002,...).</li> 
     * </ul>
     * @param pName le nom d'une classe à renommer
     * @param pAbsoluteName le nom abolu de la classe ou de la méthode.
     * @return le nom qu'il faut donner à une classe anonyme ou <code>pName</code> si il ne
     * s'agit pas du nom d'une classe anonyme.
     */
     ...
     * @return l'index + 1 de <code>pAbsoluteName</code> dans le set qui contient
     * toutes les classes anonymes récupérer lors du premier passage sur les 
     * méthodes de cette classe anonyme remontée par McCabe. -1 si le nom
     * n'est pas présent dans le set.
     */
    ...
    /**
     * Indique si le nom <code>pName</code> correspond à un des bugs de nommage McCabe.<br/>
     * Les nommages considérés comme étant des bugs McCabe sont les suivants:<br/>
     * <ul>
     * <li>Le nom d'une class ou d'une méthode se termine par _#Id (Id étant un chiffre)
     * (ex : methodName_#1). Cela peut arriver lorsque deux classes portent le même nom dans un
     * même projet.</li>
     * <li>Le nom d'un package est un chiffre (ex : test.1.blockInnerClass). Cela peut arriver
     * lorsqu'une classe interne est définie dans un bloc.</li>
     * </ul>
     ...
     * <ul>
     * <li>Retire le nom de la classe externe dans le cas d'un nom de méthode (correspondant 
     * définie dans une classe interne elle-même définie dans un bloc.</li>
     * <li>Retire le nom de la méthode qui contient la classe anonyme
     * dont le nom correspond à <code>pName</code> (ou <code>pName</code> correspond au nom de la méthode 
     * définie dans cette classe anonyme).</li>
     * </ul>
    ...
    /**
     * Décompose le nom entièrement qualifié d'une méthode afin de récupèrer le nom absolu
     * de la classe.
