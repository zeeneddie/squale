...
/**
 * Crée un fichier config.pcf à partir de la configuration de l'application, et le place 
 * dans l'espace de travail McCabe.
 * 
 * @author m400842
 * @version 1.0
 */
public class McCabePCFFile {
    ...
    /**
     * A l'initialisation, contient le dossier du projet
     * dans l'espace local, puis juste avant la génération du fichier pcf,
     * contient le dossier parent commun le plus bas des fichiers à analyser.
     */
     ...
     */
    public McCabePCFFile(final McCabeConfiguration pConfig, final TaskData pDatas) throws Exception {
        mConfig = pConfig;
        ...
        mPcfFile = new File(mConfig.getSubWorkspace().getAbsolutePath() + File.separator + McCabeMessages.getString("pcf_name"));
        ...
        ListParameterBO sources = (ListParameterBO) pConfig.getProject().getParameters().getParameters().get(ParametersConstants.SOURCES);
        if(null == sources) {
        ...
        if (pConfig.getProject().getParameter(ParametersConstants.CPP) != null) {
            ...
            ListParameterBO included = (ListParameterBO) pConfig.getProject().getParameters().getParameters().get(ParametersConstants.INCLUDED_PATTERNS);
            ListParameterBO excluded = (ListParameterBO) pConfig.getProject().getParameters().getParameters().get(ParametersConstants.EXCLUDED_PATTERNS);
            ListParameterBO excludedDirs = (ListParameterBO) pConfig.getProject().getParameters().getParameters().get(ParametersConstants.EXCLUDED_DIRS);
            ...
            if (null != jspsParam) {
                ...
                ListParameterBO excludedJspDirs = (ListParameterBO) pConfig.getProject().getParameters().getParameters().get(ParametersConstants.JSP_EXCLUDED_DIRS);
            ...
            List includedFileNames = FileUtility.getIncludedFiles(viewPath, absSources, included, excluded, allExcludedDirs, mConfig.getExtensions());
