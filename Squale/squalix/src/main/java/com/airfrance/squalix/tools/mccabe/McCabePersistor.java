...
/**
 * Objet chargé de faire persister les composants identifiés par McCabe (méthodes 
 * et classes) ainsi que les résultats
 * @author m400842
 * @version 1.0
 */
public class McCabePersistor {
     ...
     * Configuration
     */
    private McCabeConfiguration mConfiguration;

    /**
     * Adaptateur
     */
    private McCabeAdaptator mAdaptator;
     ...
     */
    public McCabePersistor(LanguageParser pParser, final McCabeConfiguration pConfiguration, final AuditBO pAudit, final ISession pSession, TaskData pDatas, String pTaskName, String pClassTemplate)
        ...
        mAdaptator = new McCabeAdaptator(pParser, repository);
        mTaskName = pTaskName;
        mClassTemplate = pClassTemplate;
        ListParameterBO jsps = (ListParameterBO) mProject.getParameter(ParametersConstants.JSP);
        if (null != jsps) { // Il s'agit d'un projet J2EE
            mJspPaths = BuildProjectPath.buildProjectPath((String) pDatas.getData(TaskData.VIEW_PATH), jsps.getParameters());
        }
    }
     ...
     */
    public int parseClassReport(final String pFilename) throws Exception {
     ...
     */
    public int parseMethodReport(final String pFilename, TaskData pDatas) throws Exception {
        mReportFileName = pFilename;
        ...
        while (it.hasNext()) {
            ...
            if (bo.getFilename().endsWith(JSP_EXTENSION) && null != mJspPaths) {
                ...
                if (null == jspMetrics.getComponent()) {
                    ...
                    LOGGER.warn(McCabeMessages.getString("logs.warn.incorrect_jsp_method_name", jspMetrics.getComponentName()));
                } else {
