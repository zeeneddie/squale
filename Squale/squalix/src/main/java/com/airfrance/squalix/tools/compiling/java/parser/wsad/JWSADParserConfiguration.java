 ...
 * @version 2.1
 */
public class JWSADParserConfiguration
    extends JParserConfiguration
{
     ...
     */
    public JWSADParserConfiguration()
        throws Exception
    {
        super();
        createReflectionMap();
        ...
        throws Exception
    {
        ...
        if ( null != root )
        {
            ...
            if ( null != myNode )
            {
                ...
                myNode =
                    ConfigUtility.getNodeByTagName( myNode, CompilingMessages.getString( "configuration.java.parsing" ) );
                /* si le noeud n'est pas nul */
                if ( null != myNode )
                {
                    /*
                     * on récupère le noeud concernant le parsing pour des projets JAVA développés sous WSAD 4.x et 5.x
                     */
                    myNode =
                        ConfigUtility.getNodeByTagName( myNode,
                                                        CompilingMessages.getString( "configuration.java.parsing.wsad" ) );
                    /*
                     * on appelle différentes méthodes pour récupérer les données
                     */
                    getFilenameFromXML( myNode );
                    /*
                     * on récupère les valeurs de configuration des noms des clés.
                     */
                    ...
                    /*
                     * on récupère les valeurs de configuration des valeurs des clés.
                     */
     ...
     * @throws Exception exception en cas d'erreur de parsing.
     */
    private void getFilenameFromXML( Node pNode )
        throws Exception
    {
        /*
         * on récupère le noeud contenant le nom du fichier contenant le classpath du projet WSAD
         */
        Node myNode =
    ...
    /**
     * Cette méthode récupère le nom des clés nécessaires pour parser le fichier de classpath.
     * 
     * @param pNode noeud XML à parser.
    ...
    private void getTagsFromXML( final Node pNode, final String pRootAnchor, final String pChildAnchor,
                                 final String pChildName, final String pChildValue )
        throws Exception
    {
        LOGGER.trace( CompilingMessages.getString( "logs.task.entering_method" ) );
    ...
    /**
     * Cette méthode permet d'attribuer à des attributs de classes des valeurs issues du fichier de configuration xml
     * <code>compiling-
     * config.xml</code>. On utilise la réflexion pour faire chuter les complexités cyclomatique,
     * cyclomatique essentielle, et d'intégration.
     * 
        ...
        throws Exception
    {
        ...
        /*
         * on invoque le setter correspondant à la clé pKeyName, en lui passant la valeur pKeyValue
         */
