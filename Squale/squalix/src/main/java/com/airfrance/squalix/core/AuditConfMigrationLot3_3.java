...
/**
 * Main servant à affecter des configurations d'affichage aux audits exécutés avant la livraison
 * de SQUALE 3.3
 * Cette classe pourra être supprimée une fois lancée en production lors de la livraison de SQUALE 3.3
 */
public class AuditConfMigrationLot3_3 {
     ...
     */
    public static void main(String[] args) throws JrafPersistenceException {
        ...
        try {
            ...
            /* 
             * On récupère tous les audits terminés, partiels ou en échec (supprimé et en cours n'ont pas
             * besoin de configuration) de chaque projet non supprimé
             * Et pour chacun, on lui affecte la bonne configuration : configuration du bubble + configurations
             * de la volumétrie dépendant des résultats (si résultats RSM alors conf RSM sinon l'autre)
             * en regardant aussi si il y a des résultats pour les JSPs (dans ce cas conf J2ee)
             */
     ...
     */
    public static void createoldConfigurations(ISession session) throws JrafDaoException {
     ...
     * @throws JrafDaoException si erreur
     */
    public static void createAuditDisplayConfs(ISession session) throws JrafDaoException {
        // Initialisation
        ...
        for (Iterator it = projects.iterator(); it.hasNext();) {
            ...
            audits.clear();
            if (ProjectBO.DELETED != curProject.getStatus()) { // Inutile pour les projets supprimés
                audits.addAll(auditDAO.findWhereComponent(session, curProject.getId(), null, null, null, AuditBO.TERMINATED));
                audits.addAll(auditDAO.findWhereComponent(session, curProject.getId(), null, null, null, AuditBO.PARTIAL));
                audits.addAll(auditDAO.findWhereComponent(session, curProject.getId(), null, null, null, AuditBO.FAILED));
            }
     ...
     * @throws JrafDaoException si erreur
     */
    public static void addAuditDisplayConfFor(ISession pSession, AuditBO pAudit, AbstractDisplayConfBO pConf, ProjectBO pProject) throws JrafDaoException {
