...
/**
 * Classe de configuration pour la compilation des projets JAVA avec eclipse
 * Le fichier de configuration se nomme "eclipsecompiling-config.xml"
 */
public class EclipseCompilingConfiguration extends XmlImport {
    /**
     * Logger.
     */
    ...
    public void parse(InputStream pStream) throws ConfigurationException {
        ...
        Digester digester = preSetupDigester("-//EclipseCompiling Configuration DTD //EN", "/config/eclipsecompiling-config.dtd", errors);
        ...
        digester.addCallMethod("configuration/squaleEclipseHome", "setSqualeEclipsePlugins", 1, new Class[] { String.class });
        ...
        if (errors.length() > 0) {
            throw new ConfigurationException(CompilingMessages.getString("exception.eclipse.configuration", new Object[] { errors.toString()}));
        }
    }
