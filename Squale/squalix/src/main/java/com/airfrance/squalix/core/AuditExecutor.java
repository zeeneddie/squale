...
/**
 * Exécute un audit entier sur un projet.
 * <br />
 * Il exécute les tâches associée à une application en les ordonnançant. Cet ordonnancement
 * des tâches est déterminé par rapport au profil du projet. La configuration doit définir
 * un scénario d'exécution pour le profil du projet. Voir la configuration de Squalix pour
 * plus de renseignements.
 * <br /><br />
 * Toutes les tâches que lancent l'exécuteur d'analyse doivent hériter de la classe abstraite
 * <code>Task</code>.
 * <br /><br />
 * Deux groupes de tâches sont lancés par l'exécuteur d'analyse :
 * <ul>
 * <li>le premier concerne les analyses par des outils divers,</il>
 * <li>le second nécessite que toutes les tâches du premier soient terminées
 * (consolidation de résultats, génération de graphiques...).</li>
 * </ul>
 * 
 ...
 * @version 1.0
 */
public class AuditExecutor implements Runnable {
    ...
    * @param pTerminationTasks les tâches à réaliser lorsque toutes les analyses ont été 
    * terminées.
    * @throws Exception si l'Audit ne peut-être créé.
    * @roseuid 42AE866F01F7
    */
    public AuditExecutor(final List pTasks, final List pTerminationTasks, final AuditBO pAudit, final ProjectBO pProject) throws Exception {
        ...
        for (int i = 0; i < pTasks.size(); i++) {
            ...
            task = TaskUtility.createTask((TaskRefBO) pTasks.get(i), pProject.getId(), pProject.getParent().getId(), pAudit.getId(), taskData);
        ...
        for (int i = 0; i < pTerminationTasks.size(); i++) {
            task = TaskUtility.createTask((TaskRefBO) pTerminationTasks.get(i), pProject.getId(), pProject.getParent().getId(), pAudit.getId(), taskData);
     ...
     * @return un booléen indiquant si on doit continuer à faire les taches d'analyse,
     * return false si une tache a échouée
    ...
    private void terminateAudit() {
        try {
            LOGGER.info(Messages.getString("log.audit_executor.terminated") + mProject.getParent().getName() + Messages.getString("separator.project") + mProject.getName());
