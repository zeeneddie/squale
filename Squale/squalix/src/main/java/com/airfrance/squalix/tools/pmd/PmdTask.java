...
public class PmdTask extends AbstractTask {
        ...
        try {
            ...
            for (Iterator it = getTaskParameters().iterator(); it.hasNext();) {
                ...
                if (param.getName().equals("language")) {
                    ...
                    oneLangAtLeast = true;
                    AbstractPmdProcessing processing = factory.createPmdProcessing(param.getValue()); // Récupération du fichier de ruleset
                    PmdRuleSetBO ruleset = getRuleSet(param.getValue());
                    // La tâche peut avoir été annulée
                    if (mStatus != CANCELLED) {
                        Report report = processing.process(getData(), getProject().getParameters(), FileUtility.byteToFile(ruleset.getValue()));
                    ...
                    throw new ConfigurationException(PmdMessages.getString("exception.parameter.invalid", new Object[] { param.getName(), param.getValue()}));
                }
            }
     ...
     */
    protected PmdRuleSetBO getRuleSet(String pLanguage) throws JrafDaoException, ConfigurationException {
