...
public class CSVConfigurationGetter {
     ...
     */
    public CSVConfiguration getConfiguration(final String pTemplateName) throws CSVException {
    ...
    private Node getTemplateNode(final Collection pTemplates, final String pTemplateName) {
        ...
        while (it.hasNext() && null == templateNode) {
            node = (Node) it.next();
            if (ConfigUtility
                .getAttributeValueByName(node, CSVMessages.getString("configuration.template.name"))
                .equals(pTemplateName)) {
                templateNode = node;
            }
        }
        return templateNode;
     ...
     */
    private void setConfig(final Node pNode, final CSVConfiguration pConfig) throws Exception {
        // recupere la class du bean
        pConfig.setCSVBean(
            ConfigUtility
                .getNodeByTagName(pNode, CSVMessages.getString("configuration.csvbean"))
                .getFirstChild()
                .getNodeValue());
        Class csvclass = Class.forName(pConfig.getCSVBean());

        pConfig.setFooterSize(
            Integer.parseInt(
                ConfigUtility.getAttributeValueByName(
                    ConfigUtility.getNodeByTagName(pNode, CSVMessages.getString("configuration.footer")),
                    CSVMessages.getString("configuration.footer.size"))));
        pConfig.setHeaderSize(
            Integer.parseInt(
                ConfigUtility.getAttributeValueByName(
                    ConfigUtility.getNodeByTagName(pNode, CSVMessages.getString("configuration.header")),
        ...
        Iterator it =
            ConfigUtility
                .filterList(
                    ConfigUtility
                        .getNodeByTagName(pNode, CSVMessages.getString("configuration.fields"))
                        .getChildNodes(),
                    Node.ELEMENT_NODE)
                .iterator();
        ...
        while (it.hasNext()) {
            ...
            String fieldName = ConfigUtility.getAttributeValueByName(node, CSVMessages.getString("configuration.field.name"));
            String fieldType = ConfigUtility.getAttributeValueByName(node, CSVMessages.getString("configuration.field.type"));
            String columnString =
     ...
     */
    private Method getSetter(final Class pClass, final String pFieldName, final String pType) throws Exception {
        String setterName =
            CSVMessages.getString("setter.prefix") + pFieldName.substring(0, 1).toUpperCase() + pFieldName.substring(1);
