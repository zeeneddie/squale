...
public class JWSADParser {
     ...
     */
    public JWSADParser(List pProjectList) throws Exception {
        mProjectList = pProjectList;
        mConfiguration = new JWSADParserConfiguration();
    }

    /**
    * This method runs the parsing process (if the project list isn't null or 
    * empty).
    * <br />If the <code>parse()</code> method returns <code>false</code>, 
    * then the 
    * <code>Project</code> boolean <code>mIsCompiled</code> is set to 
    * <code>true</code> : this is to say that this project will not be 
    * compiled by the 
    * Compile class. Actually, the goal is to avoid trying to compile a 
    * project in 
    * which no classpath file was found.
    * @see JWSADProject
    ...
    */
    public void execute() throws Exception {
        ...
        if (null != mProjectList) {
            ...
            if (null != it && it.hasNext()) {
                ...
                while (it.hasNext()) {
                    /* on affecte le projet courant en variable de 
                     * classe */
                    ...
                    if (needsParsing(project)) {
                        ...
                        /* alors on considère que le projet WSAD 
                         * a déjà été compilé */
    ...
    /**
     * Traite le parsing des entrées récupérée lor du parsing du .classpath autre que "lib"
     * pour chaque projet parsé
     * @throws Exception si une erreur survient dans un traitement
     */
    private void executeParsingEntries() throws Exception {
     ...
     */
    private void parseManifest(JWSADProject pProject) throws Exception {
    ...
    /**
     * La partie concernée se présente de cette manière :
     * 
     * Export-Package: p1,
     * p2,
     * p4
     ...
     * @throws Exception si erreur
     */
    private void parseManifestForExportedPackages(JWSADProject pProject, File pManifest) throws Exception {
        parseManifest(pProject, pManifest, EXPORT_PACKAGE, this.getClass().getMethod("addExportedPackage", new Class[] { JWSADProject.class, String.class }));
    }
     ...
     */
    private boolean parse(JWSADProject pProject) throws Exception {
        /* on part du principe que le parsing fonctionnera */
        boolean isParsed = true;
        /* on récupère le noeud "<classpath>" du fichier ".classpath" 
         * du projet WSAD */
        Node myNode = JParserUtility.getRootNode(pProject.getPath() + mConfiguration.getFilename(), mConfiguration.getClasspathAnchor());

        /* si le noeud n'est pas nul, et qu'il s'agit d'un noeud de 
         * type ELEMENT_NODE */
        if (null != myNode && Node.ELEMENT_NODE == myNode.getNodeType()) {
            ...
            while (null != myNode) {
                ...
                if (Node.ELEMENT_NODE == myNode.getNodeType()) {
                    ...
                    /* on mappe les couples clé / valeur
                    car il faut traiter les lib en premier */
        ...
        /* on crée le descripteur de fichier et on appelle la 
         * méthode addCompilingRessourcesToClasspath(File) */
    ...
    /**
     * Cette méthode n'a été créée que dans le but de faire chuter la 
     * complexité cyclomatique de la méthode <code>parse()</code>.
     * @param pProject projet WSAD
     * @param pKeyName nom de la clé.
     * @param pKeyValue valeur de la clé.
     * @param pExported si pKeyName est exportée
     * @throws Exception exception : lancée uniquement si elle provient des 
     * méthodes <code>processLib(String)</code>, <code>processVar(String)</code> 
     * ou <code>processSrc(String)</code>.
     */
    protected void mapKeyValues(JWSADProject pProject, String pKeyName, String pKeyValue, boolean pExported) throws Exception {
     ...
     * @throws Exception si erreur
     */
    private void processCon(JWSADProject pProject, String pKeyValue) throws Exception {
    ...
    /**
     * Parse le manifest pour trouver les plugins nécessaires à la compilation
     * La partie concernée se présente de cette manière :
     * 
     * Require-Bundle: plugin1,
     * plugin2;option1=v1;visibility:=reexport, plugin3,
     * plugin4
     * 
     * @param pProject le projet
     * @param pManifest le manifest
     * @throws Exception si erreur
     */
    private void parseManifestForPluginDependencies(JWSADProject pProject, File pManifest) throws Exception {
        LOGGER.debug("On parse le manifest de " + pProject.getName() + " pour trouver les dépendances de plugins");
        parseManifest(pProject, pManifest, REQUIRE_BUNDLE, this.getClass().getMethod("processPlugin", new Class[] { JWSADProject.class, String.class }));
    }

    /**
     * Sert à parser des block du manifest qui ont la même DTD
     * 
     * pBlock: string1,
     * string2,
     * string3
     * (par exemple les parties Bundle-Required et Export-Package)
     * @param pProject le projet
     * @param pManifest le manifest
     ...
     * @throws Exception si erreur
     */
    private void parseManifest(JWSADProject pProject, File pManifest, String pBlock, Method pMethod) throws Exception {
     ...
     * @throws Exception si erreur
     */
    public void processPlugin(JWSADProject pProject, String pPlugin) throws Exception {
        ...
        if (!found && !isEclipsePlugin(pProject.getBundleDir(), plugin)) {
            ...
            throw new ConfigurationException("Required plugin (" + plugin + ") not found in project's compilation rules!");
        }
    }
    ...
    /**
     * If the <code>lib</code> value is encountered in a <code>kind</code> tag, this 
     * method is called by the <code>parse()</code> method. <br /><br />
     * Two cases exist : <br/>
     * <ul><li>the path value begins with a "/",</li>
     * <li>or not !</li></ul><br />
     * In the first case, two possibilities : <br />
     * <ul><li>
     * the string between the first and second "/" is the name of a project contained 
     * in the workspace. For example, one has three projects inside his workspace : 
     * <b>TonusIntranetWeb</b>, <b>TonusIntranetCommons</b> and 
     * <b>TonusIntranetEAR</b>.<br />In the <b>TonusIntranetWeb</b> classpath file, one 
     * founds the following line : <code>&lt;classpathentry kind="lib" 
     * path="/TonusIntranetEAR/my_jars/my.jar"/&gt;</code>.<br />
     * The path to <code>my.jar</code> is thus : 
     * <code>workspace_path/TonusIntranetWeb/../TonusIntranetEAR/my_jars/my.jar</code>, 
     * which is equal to <code>workspace_path/TonusIntranetEAR/my_jars/my.jar</code>.
     * </li>
     * <li>
     * the string between the first and second "/" is not a project name contained 
     * in the workspace. It's an absolute UNIX filepath. It is therefore directly 
     ...
     * @since 1.0
     */
    protected void processLib(JWSADProject pProject, final String pPath, final boolean pExported) throws Exception {
        ...
        if (0 == pPath.indexOf(JCompilingConfiguration.UNIX_SEPARATOR)) {
            ...
            /* aucune dépendance n'a été trouvée ou il n'y a pas de projets WSAD, il s'agit d'un chemin 
             * UNIX absolu */
            if (null == project) {
                path.append(pPath);
            ...
            /* pPath ne commence pas par un "/" il s'agit d'une ressource du 
             * projet WSAD à ajouter au classpath */
        } else {
            ...
            if (!file.exists()) {
                String warning = CompilingMessages.getString("logs.task.classpath.filenotfound") + path.toString().replaceFirst(pProject.getPath(), "");
                LOGGER.warn(warning);
    ...
    protected JWSADProject findProjectDependency(String pLibPath, StringBuffer pPath) {
        ...
        /* booléen utilisé pour signifier la découverte d'une 
         * dépendance, i.e. d'un projet WSAD */
        boolean found = false;

        /* si l'itérateur n'est pas nul */
        if (null != it) {
            /* on récupère la chaîne comprise entre les 2 premiers 
             * séparateurs UNIX */
            String nomProjet = pLibPath.substring(1, pLibPath.indexOf(JCompilingConfiguration.UNIX_SEPARATOR, 1));
            JWSADProject pTemp = null;

            /* tant que l'itérateur a des éléments ou qu'aucune 
             * dépendance n'est trouvée */
            while (null == result && it.hasNext()) {
                /* on stocke le projet WSAD courant de la liste */
                pTemp = (JWSADProject) it.next();

                /* si nomProject est égale au nom du projet, alors il 
                 * y a dépendance */
                if ((pTemp.getName()).equals(nomProjet)) {
                    /* on construit le path en conséquence */
                    pPath.append(pTemp.getPath());
                    pPath.append(pLibPath.substring(pLibPath.indexOf(JCompilingConfiguration.UNIX_SEPARATOR, 1) + 1, pLibPath.length()));
                    pPath.append(mConfiguration.getClasspathSeparator());

                    /* on a trouvé la dépendance, inutile d'itérer plus 
                     * longtemps. */
                    result = pTemp;
                }
            }
    ...
    /**
     * Cette méthode parcourt le répertoire (et ses sous-répertoires) 
     * contenant les ressources de 
     * compilation et les ajoute au classpath, i.e. ajoute tous les jars / 
     * zips / classes communes à toutes les compilations (typiquement 
     * <code>j2ee.jar</code>). <br />
     * @param pProject projet WSAD
     * @param pFile répertoire à parcourir.
     * @throws Exception exception lors du parcours du répertoire.
     */
    private void addCompilingRessourcesToClasspath(JWSADProject pProject, final File pFile) throws Exception {
            ...
            for (int i = 0; i < fileList.length; i++) {
                ...
                if (fileList[i].isFile()) {
                    ...
                    if (allowedExtensions.contains(fileExt)) {
                        ...
                        path.append(
                            fileList[i].getAbsolutePath().replaceAll(JCompilingConfiguration.WINDOWS_SEPARATOR + JCompilingConfiguration.WINDOWS_SEPARATOR, JCompilingConfiguration.UNIX_SEPARATOR));
                        path.append(mConfiguration.getClasspathSeparator());
                    }
    ...
    /**
    * If the <code>src</code> value is encountered in a <code>kind</code> tag, this 
    * method is called by the <code>parse()</code> method. <br /><br />
    * Two cases exist : <br/>
    * <ul><li>the path value begins with a "/",</li>
    * <li>or not !</li></ul><br />
    * In the first case, two sub-cases : <ul>
    * <li>it's a linked project,</li>
    * <li>it's an unix path.</li></ul>
    * If it's a known project, then the dependency is added, by calling the 
    * <code>processSrcDir(JWSADProject, String)</code> method. <br />
    * If an unix file path is encoutered, then it is directly added to the 
    * classpath, such as in the case in which the file path does not begin 
    * by a "/".
    * @param pProject project WSAD
    * @param pPath value of the <code>path</code> tag linked with.
    * @param pExported value of the <code>exported</code> tag linked with.
    ...
    */
    private void processSrc(JWSADProject pProject, final String pPath, final boolean pExported, final boolean pIsPlugin) throws Exception {
        ...
        if (pPath.startsWith(JCompilingConfiguration.UNIX_SEPARATOR)) {
            ...
            /*  alors il s'agit d'un chemin UNIX absolu : il s'agit d'un 
             * répertoire contenant des fichiers sources du projet 
             * courant*/
            doPath = !found;

            /* sinon il s'agit d'un répertoire contenant des fichiers sources 
             * du projet courant */
        }
        /* on affecte le répertoire comme étant le répertoire contenant 
         * les sources */
        if (doPath) {
     ...
     * @throws Exception si erreur
     */
    private boolean findDependency(JWSADProject pProject, String pProjectName, boolean pExported, boolean pIsPlugin) throws Exception {
        Iterator it = mProjectList.iterator();
        boolean found = false;
        if (null != it) {
            /* tant qu'il y a des éléments et qu'aucune dépendance 
             * n'a été trouvée */
            while (false == found && it.hasNext()) {
                /* on appelle la méthode processSrcDir(JWSADProject, String) 
                 * qui ajoute le cas échéant des dépendances au 
                 * projet courant (pProject). Si une dépendance est ajoutée, 
                 * alors found est mis à true. */
    ...
    /**
     * Cette méthode a été implémentée afin de faire chuter la complexité 
     * cyclomatique de la méthode <code>processSrc(String)</code>.
     * @param pMainProject JWSADProject.
     * @param pProject JWSADProject.
     * @param pNomProject Nom du projet : issu du tag <code>path</code> du fichier 
     * XML de classpath.
     * @param pExported true si le <code>pMainProject</code> exporte <code>pProject</code>
     * @param pIsPlugin si pProject est un plugin
     * @return <code>true</code> s'il existe une dépendance entre projets, 
     ...
     */
    private boolean processSrcDir(JWSADProject pMainProject, JWSADProject pProject, String pNomProject, boolean pExported, boolean pIsPlugin) throws Exception {
        boolean found = false;
        ...
        if (pNomProject.equals(pProject.getName())) {
            ...
            if (!pIsPlugin) {
                /* on ajoute le répertoire contenant les classes compilées 
                 * de pProject au classpath de pMainProject */
                addToClasspath(pMainProject, pProject.getDestPath());
                /* Ainsi que ses libraries exportées si il s'agit d'un projet*/
                LOGGER.debug("Traitement des librairies exportées de " + pProject.getName()+ " pour " + pMainProject.getName());
                addToClasspath(pMainProject, pProject.getExportedLib());
            } else {
                /* On dézippe les librairies exportées de pProject 
                 * dans le répertoire défini dans la configuration
                 * et ajoute les .class au classpath
     ...
     * @throws Exception si erreur
     */
    private void addExportedLibs(JWSADProject pMainPlugin, JWSADProject pPlugin) throws Exception {
    ...
    /**
     * Permet d'ajouter le répertoire contenant les .class des packages exportés par <code>pPlugin</code>
     * au classpath de <code>pMainPlugin</code>
     ...
     */
    public void addExportedPackagesToClasspath(JWSADProject pMainPlugin, JWSADProject pPlugin, File pRoot) throws IOException {
        ...
        for (int i = 0; i < pPlugin.getExportedPackages().size(); i++) {
            ...
            packageDir = new File(pRoot, ((String) pPlugin.getExportedPackages().get(i)).replaceAll("\\.", File.separator));
            // On crée le répertoire de destination des .class
            // Ex : chemin_pRoot/nomProjet/com/airfrance/jraf
            copyDest = new File(copyRoot, ((String) pPlugin.getExportedPackages().get(i)).replaceAll("\\.", File.separator));
     ...
     * @throws IOException si erreur lors de la copie
     */
    private void copyClassesTo(File pDir, File pDest) throws IOException {
        ...
        if (null != classes) {
            ...
            for (int i = 0; i < classes.length; i++) {
                ...
                if (currentFile.isFile() && currentFile.getName().endsWith(JWSADParser.DOT + JParserConfiguration.EXT_CLASS)) {
    ...
    /**
     * This method adds a string (i.e. a directory that should be added to the 
     * project classpath) to the <code>mClasspath</code> JWSADProject attribute.
     * @param pProject JWSADProject.
     * @param pCp String to be added to the <code>mClasspath</code> 
     * WSAD attribute.
     * @since 1.0
     ...
     */
    private void addProjectDependency(JWSADProject pProject, JWSADProject pProj, boolean pExported, boolean pIsAPlugin) throws Exception {
        if (pExported) {
     ...
     */
    private void addPluginDependencies(JWSADProject pMainPlugin, JWSADProject pDependencyPlugin) throws Exception {
     ...
     */
    class OnlyClassesFilter implements FileFilter {
