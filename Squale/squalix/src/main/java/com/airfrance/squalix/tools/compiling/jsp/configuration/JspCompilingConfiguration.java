...
/**
 * Configuration pour la compilation des JSPs.
 * La configuration de la compilation des JSPs est définie dans un fichier 
 * XML (<code>jspcompiling-config.xml</code>), celui-ci est lu par cette classe.
 */
public class JspCompilingConfiguration extends XmlImport {
    /**
     * Logger.
     */
    ...
    /** 
     * Nom du premier package pour les jsps 
     * Ce nom peut être suivi du numéro d'index+1 du répertoire de la liste des sources JSP donnée
     * en paramétre de configuration si la jsp est contenue dans ce répertoire (si index = 0 on ne met pas
     * de numéro)
     */
    ...
    public void parse(InputStream pStream) throws ConfigurationException {
        ...
        Digester digester = preSetupDigester("-//JspCompiling Configuration DTD //EN", "/config/jspcompiling-config.dtd", errors);
        // Traitement du répertoire des jars
        digester.addCallMethod("jspcompiling-configuration/jarDirectory", "setJarDirectory", 1, new Class[]{String.class});
