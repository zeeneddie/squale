 ...
 */
//Source file: D:\\cc_views\\squale_v0_0_act\\squale\\src\\squalix\\src\\org\\squale\\squalix\\tools\\compiling\\java\\JavaCompilingTask.java
...
import org.squale.squalix.tools.compiling.java.adapter.JComponent;
...
import org.squale.squalix.tools.compiling.java.compiler.impl.EclipseCompilerImpl;
import org.squale.squalix.tools.compiling.java.compiler.impl.JWSADCompilerImpl;
import org.squale.squalix.tools.compiling.java.compiler.impl.JXMLCompilerImpl;
import org.squale.squalix.tools.compiling.java.configuration.JCompilingConfiguration;
import org.squale.squalix.tools.compiling.java.parser.impl.JRSAParserImpl;
import org.squale.squalix.tools.compiling.java.parser.impl.JWSADParserImpl;
import org.squale.squalix.tools.compiling.java.parser.wsad.JWSADDotProjectParser;
import org.squale.squalix.util.file.FileUtility;

/**
 * Tâche de compilation JAVA.<br />
 * Compile à partir d'environnements WSAD 5.x et/ou de fichiers XML ANT. Hérite de la classe mère
 * <code>CompilingTask</code>.
 * 
 * @author m400832
 * @version 1.0
 */
public class JCompilingTask
    extends AbstractTask
    implements BuildListener
{

    /**
     * le message associé à une erreur de compil utilisé pour mettre les messages sous une bonne forme pour le web
     */
    ...
    /**
     * Séparateur entre répertoires dans la HashMap de paramètres du <code>ProjectBO</code>.
     */
    ...
    /**
     * Cette méthode influe sur le <code>ProjectBO</code> en modifiant la <code>HashMap</code> de paramètres.
     * 
    ...
    private void modifyClassesDirInHashMap()
    {
        ...
        if ( null != it && it.hasNext() )
        {
            ...
            while ( it.hasNext() )
            {
                ...
                /*
                 * si le JComponent est de type JWSADParserImpl i.e. projet WSAD ou RSA avec compilation avec le plugin
                 * Eclipse
                 */
    ...
    /**
     * Positionne la variable donnant le chemin du répertoire racine contenant les .class du projet
     * 
     * @param classesDir le chemin du répertoire contenant les .class Paramètre fourni par les méthodes
     *            create***Components
     */
    ...
    /**
     * Cette méthode influe sur le <code>ProjectBO</code> en modifiant la <code>HashMap</code> de paramètres, pour
     * le paramètre <code>
     * project.parameter.classpath</code>.
     * 
     * @throws Exception Exception lors de la modification.
     */
    private void modifyClasspathInHashMap()
        throws Exception
    {
        ...
        if ( null != it && it.hasNext() )
        {
            ...
            while ( it.hasNext() )
            {
                ...
                /*
                 * si le JComponent est de type JWSADParserImpl i.e. projet WSAD
                 */
                if ( jComp.getComponent() instanceof JWSADParserImpl )
                {
                    ...
                    /*
                     * si le composant est de type JXMLCompilerImpl i.e. projet ANT
                     */
                }
            ...
            /*
             * on récupère la clé de la map de paramètres du projet dont la valeur associée sera le classpath
             */
            ...
            /*
             * on modifie la hashmap de paramètres du projet en conséquence.
             */
     ...
     */
    private String modifyClasspathInHashMapWSAD( List pList )
        throws Exception
    {
        String value = null;
        ...
        if ( null != pList )
        {
            ...
            if ( null != it && it.hasNext() )
            {
                ...
                while ( it.hasNext() )
                {
                    ...
                    /*
                     * on ajoute le classpath du bean au buffer NB : pas besoin d'ajouter de SEPDIRS car le classpath
                     * retourné se termine toujours par un ";"
                     */
                    cp.append( project.getClasspath() );
                    /*
                     * on ajoute au buffer le chemin du répertoire contenant les classes compilées
                     */
     ...
     */
    private String modifyClasspathInHashMapXML( JXMLProject pProject )
        throws Exception
    {
        String value = null;
        ...
        if ( null != pProject && null != pProject.getPath() )
        {
            ...
            /*
             * on récupère la portion du chemin du projet XML contenu entre le début de ladite chaîne et le dernier "/"
             * rencontré. i.e. on récupère "/app/SQUALE" si le chemin est "/app/SQUALE/build.xml".
             */
        ...
        throws Exception
    {
        ...
        {
            ListParameterBO antList =
                (ListParameterBO) mProject.getParameters().getParameters().get( ParametersConstants.ANT );
            if ( antList != null )
            {
            ...
            {
                ListParameterBO rsaParams =
                    (ListParameterBO) mProject.getParameters().getParameters().get( ParametersConstants.RSA );
                if ( null != rsaParams )
                {
    ...
    /**
     * Cette méthode crée les composants nécessaires à la compilation des projects de type WSAD.
     * 
     * @param pMap MapParameterBO contenant les informations nécessaires à la création des composants.
     * @param pProjectsIt l'itérateur sur les projets
     * @param pIsRSA si il s'agit d'une liste de projet type RSA
     * @throws Exception exception.
     */
    private void createWSADOrRSAComponents( MapParameterBO pMap, Iterator pProjectsIt, boolean pIsRSA )
        throws Exception
    {
        ...
        String bundleDir =
            getEclipseBundleFile( (StringParameterBO) pMap.getParameters().get( ParametersConstants.BUNDLE_PATH ) );
        ...
        for ( ; pProjectsIt.hasNext(); )
        {
            ...
            if ( !pIsRSA )
            {
                ...
                if ( null != projectsParams )
                {
                    ...
                    {
                        StringParameterBO manifest =
                            (StringParameterBO) projectParamsMap.getParameters().get( ParametersConstants.MANIFEST_PATH );
            ...
            else
            {
                ...
                {
                    String earName =
                        ( (StringParameterBO) projectParamsList.get( ParametersConstants.EAR_NAME_ID ) ).getValue();
                    rsaProject.setEARProjectName( earName );
                }
                if ( projectParamsList.size() > ParametersConstants.MANIFEST_PATH_ID )
                {
                    String manifestPath =
                        ( (StringParameterBO) projectParamsList.get( ParametersConstants.MANIFEST_PATH_ID ) ).getValue();
            ...
            if ( !myProject.isWSAD() )
            {
                ...
                throw new Exception(
                                     CompilingMessages.getString( "java.exception.task.malformed_wsad_project_in_hashmap" )
                                         + myProject.getPath() );
            }
        }
     ...
     */
    private void addEclipseOrWSADCompilerToComponents( MapParameterBO pMap, List pJWSADProjectList, boolean pIsRSA )
        throws Exception
    {
        ...
        if ( null != isEclipse && isEclipse.getValue().equals( "false" ) )
        {
            ...
            /*
             * on crée les composants : pour compiler les projets WSAD, il faut d'abord parser les fichiers ".classpath"
             * puis compiler. D'où le recours à deux composants
             */
    ...
    private String getEclipseBundleFile( StringParameterBO bundlePath )
    {
        ...
        if ( null != bundlePath )
        {
            ...
            catch ( Exception e )
            {
                ...
                String message =
                    CompilingMessages.getString( "java.warning.eclipse_bundle_not_correct", new String[] {
                        bundlePath.getValue(), exceptionMsg } );
                LOGGER.warn( message );
                initError( message );
            }
        }
    ...
    private void setProjectParams( MapParameterBO pParams, JWSADProject pProject, String pDirName )
    {
        ...
        addExcludedDirs( pProject, (ListParameterBO) pParams.getParameters().get( ParametersConstants.EXCLUDED_DIRS ),
                         viewPath );
    ...
    private void addExcludedDirs( JWSADProject pProject, ListParameterBO pExcludedDirs, String pViewPath )
    {
        ...
        {
            /*
             * on ajoute au projet la liste des répertoires à exclure
             */
     ...
     * 
     * @param pAntMap le MapParameterBO contenant les informations nécessaires à la création du composant.
     * @throws Exception exception.
     */
    private void createXMLComponent( MapParameterBO pAntMap )
        throws Exception
    {

        /* on crée un bean project XML */
        JXMLProject project = new JXMLProject();
        StringParameterBO buildFile =
            (StringParameterBO) pAntMap.getParameters().get( ParametersConstants.ANT_BUILD_FILE );
    ...
    /**
     * Méthode d'ajout d'un <code>JComponent</code> à la liste <code>mComponents</code>.
     * 
     ...
     * 
     * @param pName : * WSAD = nom du répertoire contenant le projet / *XML : chemin vers le fichier XML.
     * @return le chemin complété.
     * @throws Exception exception lors du traitement.
     */
    private String makePath( String pName )
        throws Exception
    {
    ...
    /**
     * Cette méthode est utilisée par la méthode <code>modifyClasspathInHashMap()</code> afin d'éviter les doublons
     * dans la chaîne de caractères contenant le classpath.
     * 
     ...
     */
    private String generateProperClasspath( String pString1, String pString2 )
        throws Exception
    {
        ...
        if ( null != list1 && null != list2 )
        {
            ...
            {
                /*
                 * si la première liste ne contient pas l'élément courant de la seconde
                 */
    ...
    /**
     * Log JRAF L'évènement ANT est redirigé vers le log JRAF
     * 
