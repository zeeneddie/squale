...
public class ApplicationFacade implements IFacade {
    ...
    /**
     * Permet de récupérer un objet ApplicationConfDTO à partir d'un objet presentant 
     * au moins l'ID de l'application.
     ...
     */
    public static ApplicationConfDTO getApplicationConf(ApplicationConfDTO pApplicationConf) throws JrafEnterpriseException {
     ...
     */
    public static boolean exists(ComponentDTO pApplication) throws JrafEnterpriseException {
     ...
     * @dev-squale Demande à la partie DAO de sauvegarder / mettre à jour les objets 
     * métiers : <br />
     * - Un ApplicationBO<br />
     * - Des ProjectBO<br />
     * - Des UserBO<br />
     ...
     */
    public static void update(ApplicationConfDTO pApplicationConf) throws JrafEnterpriseException {
        update(pApplicationConf, null);
    }

    /**
     * Transforme les DTO en objets métiers via des transformeurs.
     * Demande à la partie DAO de sauvegarder / mettre à jour les objets 
     * métiers :<br />
     * - Un ApplicationBO<br />
     * - Des ProjectBO<br />
     * - Des UserBO<br />
     ...
     * @throws JrafEnterpriseException exception JRAF
     */
    public static void update(ApplicationConfDTO pApplicationConf, ISession pSession) throws JrafEnterpriseException {
        // Initialisation 
     ...
     */
    private static boolean updateApplicationRights(ISession pSession, ApplicationConfDTO pApplicationConf, ApplicationBO pAppliBO) throws JrafDaoException {
        // Initialisation
        UserBO userBO = null; // retour du load pour l'utilisateur
        ProfileBO profileBO = null; // profil des utilisateurs a ajouter
        // Les DAO
     ...
     * @throws JrafDaoException si erreur
     */
    static private boolean purgeUsers(ISession pSession, ApplicationBO pApplicationBO, Collection pUserDefined) throws JrafDaoException {
    ...
    /**
     * Mise à jour des audits sur une application
     * Lorsqu'une application est modifiée, les audits qui y sont associés peuvent passer
     * d'un audit de type normal à un audit de type jalon. Dans ce cas, il faut supprimer
     * l'audit normal s'il en existe un. Dans le cas inverse, il faut créer un audit normal
     * et le rattacher à l'application.
     * @param pSession session
     * @param pApplicationBO application
     ...
     * @throws JrafDaoException si erreur
     */
    private static void updateAudit(ISession pSession, ApplicationBO pApplicationBO, int pPreviousFrequency) throws JrafEnterpriseException, JrafDaoException {
        ...
        if (pPreviousFrequency == -1 && pApplicationBO.getStatus() == ApplicationBO.VALIDATED) {
            ...
            while (iterator.hasNext()) {
                ...
                } else { // La fréquence a changé sans changer le type des audits
    ...
    /**
     * Permet de créer en base une application (le premier audit sera créé à la validation) 
     * et vérifier si une application portant le même nom n'existe pas déjà
     ...
     */
    public static ApplicationConfDTO insert(ApplicationConfDTO pApplicationConf, UserDTO pUserCreating) throws JrafEnterpriseException {
     ...
     */
    public static ApplicationConfDTO insert(ApplicationConfDTO pApplicationConf, UserDTO pUserCreating, ISession pSession) throws JrafEnterpriseException {
        ...
        try {
            ...
            /* Chargement du UserBO et ProfileBO correspondant au gestionnaire d'application
             * si une application du meme nom n'existe pas */
     ...
     */
    public static ComponentDTO getApplication(ComponentDTO pApplication) throws JrafEnterpriseException {
     ...
     */
    public static void deleteApplicationConfList(Collection pApplicationDTOs) throws JrafEnterpriseException {
     ...
     */
    public static void deleteApplicationConfList(Collection pApplicationDTOs, ISession pSession) throws JrafEnterpriseException {
     ...
     */
    public static void validateApplicationConfList(Collection pApplicationIDs) throws JrafEnterpriseException {
     ...
     */
    public static List validateApplicationConfList(Collection pApplicationConfDTOs, ISession pSession) throws JrafEnterpriseException {
        ...
        try {
            ...
            while (iterator.hasNext()) {
                ...
                if (existantApplication > 1) {
                    ...
                    // et si aucun audit de suivi n'a déjà été programmé (cas d'un audit de test d'une application non validée)
                    List lList = auditDAO.findWhereComponent(pSession, applicationIDTemp.longValue(), null, null, AuditBO.NORMAL, AuditBO.NOT_ATTEMPTED);
    ...
    /**
     * Permet de retourner une collection de ApplicationConfDTO à valider par 
     * l'administrateur du portail
     * @return Collection de ApplicationConfDTO applications à valider
     * @throws JrafEnterpriseException exception JRAF
     * @roseuid 42CBFFB10215
     */
    public static Collection getApplicationConfList() throws JrafEnterpriseException {
     ...
     */
    public static void deleteAll(ApplicationConfDTO pApplicationConf) throws JrafEnterpriseException {
     ...
     */
    public static boolean existsMilestone(ApplicationConfDTO pApplicationConf) throws JrafEnterpriseException {
     ...
     */
    public static void deleteAll(ApplicationConfDTO pApplicationConf, ISession pSession) throws JrafEnterpriseException {
     ...
     */
    public static boolean existsMilestone(ApplicationConfDTO pApplicationConf, ISession pSession) throws JrafEnterpriseException {
    ...
    /**
     * Permet d'ajouter un prochain audit à une application donné après validation de l'application
     * par un administrateur
     * @return AuditBO un nouvel audit créé pour le jour meme a minuit
     * @throws JrafEnterpriseException exception JRAF
     */
    private static AuditBO createNewAudit() throws JrafEnterpriseException {
     ...
     */
    public static ComponentDTO loadByAuditId(Long auditId, ISession pSession) throws JrafDaoException {
     ...
     * @throws JrafDaoException en cas d'échecs
     */
    public static Collection listAll(ISession pSession) throws JrafDaoException {
        ApplicationDAOImpl dao = ApplicationDAOImpl.getInstance();
     ...
     * @throws JrafDaoException si erreur
     */
    public static void addUserAccess(ISession pSession, Collection pApplications, String pMatricule, Integer maxSize) throws JrafDaoException {
        ...
        for (Iterator it = pApplications.iterator(); it.hasNext();) {
            ...
            if (null != appliBO && appliBO.getStatus() == ApplicationBO.VALIDATED) { // code défensif
     ...
     * @throws JrafEnterpriseException si erreur 
     */
    public static void updateLastModifParams(Long pApplicationId, String pLastUser, Date pLastUpdate) throws JrafEnterpriseException {
        // Initialisation du DAO
        ApplicationDAOImpl appliDAO = ApplicationDAOImpl.getInstance();
        // La session
        ISession session = null;
