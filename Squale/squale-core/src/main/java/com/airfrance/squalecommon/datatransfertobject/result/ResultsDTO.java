...
public class ResultsDTO implements Serializable {
    ...
    /**
     Contient :
     Une liste pValues qui contient :
     [
     une liste colTypes :
     [ typeY    | typeX1 | typeX2 | ... | typeXn ]
     |
     et une à n liste(s) de valeurs (la première valeur etant le type de la ligne) :
     [ typeY1  | val11   | val21    | ... | valn1   ]
     |
     ... 
     |
     [ typeYm | val1m   | val2m   | ... | valnm  ]
     ]
     n > 0, m > 0 (=> (n+1)*(m+1) éléments)
     */
    private Map mResultMap;

    /**
     * Clé : AuditDTO (ou ComponentDTO)
     * Valeur : Tableau de répartitions des pratiques sur des intervalles de pas = 1
     */
    private Map mIntRepartitionPracticeMap;

    /**
     * Clé : AuditDTO (ou ComponentDTO)
     * Valeur : Tableau de répartitions des pratiques sur des intervalles de pas = 0.1
     */
    ...
    /**
     * Permet d'ajouter au resultMap une ligne supplementaire
     * Verifie que le format est le suivant :
     * null en cle -- liste d'objets en valeur ( n fois )
     * objet en cle -- liste de meme taille en valeur
    ...
    public ResultsDTO add(ResultsDTO pResultsSrc) {
        ...
        /* Parcours des cles du premier ResultsDTO et concatenation de la liste (valeur) 
         * du deuxieme ResultsDTO correspondant a la cle courante */
        ...
        /* Verification que tous les element du 2eme ResultsDTO 
        sont presents dans le premier */
        ...
        if (!mResultMap.keySet().containsAll(resultSrcMap.keySet())) {
            ...
            while (result2Iterator.hasNext()) {
                ...
                /* SI la clé n'existe pas dans le premier ResultsDTO 
                 * ALORS on ajoute une liste avec des nulls de la taille des
                 * listes du premier ResultsDTO
                 * PUIS on concatene la liste de deuxieme ResultsDTO*/
