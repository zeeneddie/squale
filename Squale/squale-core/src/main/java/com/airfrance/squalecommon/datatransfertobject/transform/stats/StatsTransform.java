...
public class StatsTransform {
    ...
    /**
     * bo2dto avec un comportement un peu particulier.
     * Les objets représentant les données stockées en base ne correspondent (volontairement) pas
     * à la représentation souhaitée, le transform effectue donc la mise en forme pour avoir des objets
     * permettant de faciliter l'affichage 
    ...
    private void setSiteTotal(SiteStatsDTO siteTotal, List siteList) {
        ...
        for (int i = 0; i < siteList.size(); i++) {
            ...
            siteTotal.setNbAppliWithAuditsSuccessful(siteTotal.getNbAppliWithAuditsSuccessful() + current.getNbAppliWithAuditsSuccessful());
            siteTotal.setNbAppliWithoutSuccessfulAudits(siteTotal.getNbAppliWithoutSuccessfulAudits() + current.getNbAppliWithoutSuccessfulAudits());
