...
/**
 * Formule de calcul Une formule de calcul est associée à une ou plusieurs pratiques. Une formule s'applique à un seul
 * type de composant : une classe ou une méthode par exemple. Elle décrit la façon dont une note est attribuée en
 * fonction de mesure(s). Plusieurs mesures peuvent être utilisées dans la formule, par exemple une mesure mccabe et une
 * mesure sonarj. Une condition d'activation permet de calculer ou non une formule. La syntaxe de la condition est du
 * type Jython et ressemble à celà : mccabe.wmc >= 8 Dans cet exemple, la mesure de type mccabe nommée wmc est utilisée.
 * 
