...
/**
 * Extracteur des mesures utilisées par une formule
 * Une formule de règle qualité est exprimée à l'aide de mesures qui
 * sont utilisées pour la construction du menu top ou du détail d'une pratique
 * pour afficher les valeurs des mesures liées à cette pratique
 */
public class FormulaMeasureExtractor implements PracticeRuleBO.MeasureExtractor {
    ...
    public String[] getUsedMeasures(AbstractFormulaBO pFormula) {
        ...
        while (paramsIt.hasNext()) {
            ...
            mUsedMeasures[i] = param.getMeasureKind()+"."+pFormula.getComponentLevel()+"."+param.getMeasureAttribute();
            i++;
        }
