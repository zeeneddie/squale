...
public class AuditComputing {
                ...
                try {
                    ...
                    while (practices.hasNext()) {
                        ...
                        try {
                            ...
                            Collection marks = computePracticeResults(pSession, pAudit, interpreter, practice, practiceResult, children);
                            ...
                            LOG.error(RuleMessages.getString("formula.error", new Object[] { practice.getName()}), fe);
                        } catch (WeightFunctionException wfe) {
                            // Si une erreur se produit sur le calcul d'une formule
                            // on stoppe le calcul pour cette pratique
                            LOG.error(RuleMessages.getString("function.error", new Object[] { practice.getWeightFunction(), practice.getName()}), wfe);
                        }
    ...
    private static void computeProjectPracticeMark(ISession pSession, ProjectBO pProject, AuditBO pAudit, FormulaInterpreter pInterpreter, PracticeRuleBO pPractice, PracticeResultBO pPracticeResult)
        ...
        for (int i = 0; i < measureKinds.length; i++) {
            MeasureBO measure = (MeasureBO) MeasureDAOImpl.getInstance().load(pSession, projectId, auditId, Mapping.getMeasureClass(measureKinds[i] + "." + formula.getComponentLevel()));
            measures[i] = measure;
     ...
     */
    private static void computePracticeMark(PracticeRuleBO pPractice, PracticeResultBO pPracticeResult, Collection pMarks) throws WeightFunctionException {
        /*
        * Formule : 
        *             
        * meanMark = weigh^-1(1/N * sum(weight(note(component))))
        * 
        * Cela revient à faire la moyenne des notes auxquelles on applique la fonction
        * la fonction de pondération weight puis à appliquer la fonction inverse
        *                 
        */
     ...
     * @throws WeightFunctionException si erreur
     */
    private static Float inverse(WeightFunctionInterpreter pInterpreter, String pFunction, float pY) throws WeightFunctionException {
        // précision de x : 10^-3
    ...
    private static Collection computePracticeResults(
        ISession pSession,
        AuditBO pAudit,
        FormulaInterpreter pInterpreter,
        PracticeRuleBO pPractice,
        ...
        while (it.hasNext()) {
            ...
            for (int i = 0; i < measureKinds.length; i++) {
                Long auditId = new Long(pAudit.getId());
                MeasureBO measure = (MeasureBO) MeasureDAOImpl.getInstance().load(pSession, componentId, auditId, Mapping.getMeasureClass(measureKinds[i] + "." + formula.getComponentLevel()));
                measures[i] = measure;
                // Une mesure peut être absente, ce qui serait lié à un problème de création des mesures
                // dans la phase qui précède le calcul des notes
                if (measure == null) {
    ...
    private static MarkBO computeMark(
        ISession pSession,
        FormulaInterpreter pInterpreter,
        MeasureBO[] pMeasures,
        PracticeRuleBO pPractice,
