 ...
 */
public class QualityGridDAOImpl extends AbstractDAOImpl {
     ...
     */
    public QualityGridBO createGrid(ISession pSession, QualityGridBO pGrid) throws JrafDaoException {
     ...
     */
    public QualityGridBO findWhereName(ISession pSession, String pName) throws JrafDaoException {
        QualityGridBO result = null;
        String whereClause = "where ";
        whereClause += getAlias() + ".name = '" + pName + "'";
        whereClause += " AND not exists (from QualityGridBO as oldgrid where " + getAlias() + ".name=oldgrid.name and oldgrid.dateOfUpdate > " + getAlias() + ".dateOfUpdate)";
     ...
     * @throws JrafDaoException si erreur
     */
    public Collection findGrids(ISession pSession, boolean pLastVersions) throws JrafDaoException {
        String whereClause = "";
        if (pLastVersions) {
            whereClause = "where not exists (from QualityGridBO as oldgrid where " + getAlias() + ".name=oldgrid.name and oldgrid.dateOfUpdate > " + getAlias() + ".dateOfUpdate)";
        }
     ...
     * @throws JrafDaoException si erreur
     */
    public Collection findGridsWithoutProfiles(ISession pSession) throws JrafDaoException {
        String whereClause = "where not exists (from QualityGridBO as oldgrid where " + getAlias() + ".name=oldgrid.name and oldgrid.dateOfUpdate > " + getAlias() + ".dateOfUpdate)";
     ...
     * @throws JrafDaoException si erreur
     */
    public void removeGrids(ISession pSession, ArrayList gridsId) throws JrafDaoException {
     ...
     * @throws JrafDaoException si erreur
     */
    public QualityGridBO loadById(ISession pSession,long pGridId) throws JrafDaoException {
        QualityGridBO result = null;
