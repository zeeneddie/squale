...
/**
 * Transformation d'une application sous forme BO
 * en dto représentatif des statistiques niveau application
 */
public class ApplicationStatsTransformer {

    /**
     * @param pApplis les applications
     * @param pDaysForTerminatedAudit le nombre de jours max pour lesquels il doit y avoir au moins un audit
     * réussi pour que l'application soit active
     ...
     */
    public static ApplicationStatsDTO bo2Dto(ApplicationBO pApplicationBO, int pDaysForTerminatedAudit, int pDaysForAllAudits) {
    ...
    /**
     * Compte les audits dont le statut est "en echec", "partiel" ou "réussi" réalisés durant 
     * les <code>pDaysForAllAudits</code> derniers jours
    ...
    private static int getNbAuditsSince(List pAudits, int pDaysForAllAudits) {
        ...
        for (int i = 0; i < pAudits.size() && dateInTime; i++) {
            ...
            if (dateInTime && currentAudit.getStatus() != AuditBO.NOT_ATTEMPTED && currentAudit.getStatus() != AuditBO.DELETED && currentAudit.getStatus() != AuditBO.RUNNING) {
