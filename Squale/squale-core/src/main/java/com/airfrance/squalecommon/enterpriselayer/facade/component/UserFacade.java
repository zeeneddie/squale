...
public class UserFacade implements IFacade {
     ...
     * @deprecated l'utilisateur logué recoit l'objet UserDTO qui contient la liste
     * de application.
     * @roseuid 42CBFFB2014C
     */
    public static Collection getApplicationList(UserDTO pUser) throws JrafEnterpriseException {
     ...
     * @throws JrafEnterpriseException exception JRAF
     */
    public static UserDTO getUser(UserDTO pUser, Boolean pAdmin) throws JrafEnterpriseException {
     ...
     * tous les applications validés sont ajoutés à l'utilisateur
     ...
     * @throws JrafEnterpriseException exception Jraf
     */
    private static UserDTO setRights(ISession pSession, UserDTO pUserDTO, UserBO pUserBO, Boolean pAdmin) throws JrafEnterpriseException {
            ...
            if (pAdmin.booleanValue()) {
                ...
                while (applicationIterator.hasNext()) {
                    ...
                    if (rightKey.getStatus() == ApplicationBO.VALIDATED) {
                        rights.put(ComponentTransform.bo2Dto(rightKey), ProfileTransform.bo2Dto((ProfileBO) pUserBO.getRights().get(rightKey)));
     ...
     * Le paramètre pRights est mis à jour pour contenir l'ensemble des applications avec leur droit
     * associé
     ...
     */
    static private void fillRights(ISession pSession, Iterator pApplicationIterator, String pProfile, Map pRights, List pPublicAppli) throws JrafDaoException {
     ...
     * La création se fait avec un profil minimal, la mise à jour ne concerne que l'adresse email ou le nom
     * @param pUser utilisateur donné
     * @param pAdmin indique si le privilège admin est placé
     * @return UserDTO avec l'identifiant 
     * @throws JrafEnterpriseException exception JRAF
     */
    public static UserDTO createOrUpdateUser(UserDTO pUser, Boolean pAdmin) throws JrafEnterpriseException {
        // Initialisation 
        UserBO userBO = null; // retour de la DAO
        UserDTO userDTO = null; // retour de la facade
        ISession session = null;
     ...
     * L'utilisateur est crée avec le matricule passé en paramètre et le profil par défaut
     * en fonction du droit admin (soit admin soit default)
     * @param pSession session
     ...
     * @throws JrafDaoException si erreur
     */
    static private UserBO createDefaultUser(ISession pSession, String pMatricule, Boolean pAdmin) throws JrafDaoException {
        UserBO userBO = new UserBO();
     ...
     */
    public static Collection getUsersWithEmails(Long pId, String pProfile, boolean pUnsubscribed) throws JrafEnterpriseException {
        ...
        try {
            ...
            Collection applicationUsers = userDAO.findWhereApplicationAndProfileAndHaveEmails(session, pId, profileBO, pUnsubscribed);
     ...
     * @param pUnsubscribed true si on veut récupérer aussi les utilisateur qui se sont désabonnés
     * de l'envoi automatique d'email.
     * @return utilisateurs avec le profil administrateur ou ayant le profil manager
     * sur l'application concernée
     * @throws JrafEnterpriseException si erreur
     */
    static public Collection getAdminsWithEmails(boolean pUnsubscribed) throws JrafEnterpriseException {
        Collection result = new ArrayList(); // retour de la facade
        ISession session = null;
        try {
            // récupération d'une session
