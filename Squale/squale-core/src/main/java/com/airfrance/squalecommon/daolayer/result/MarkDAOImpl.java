 ...
 *
 */
public class MarkDAOImpl extends AbstractDAOImpl {
    /**
     * Instance singleton
     */
     ...
     * @throws JrafDaoException si une erreur à lieu
     */
    public void removeWhereComponent(ISession pSession, AbstractComponentBO pComponent) throws JrafDaoException {
        String whereClause = "where ";
     ...
     */
    public List findWhere(ISession pSession, Long pComponentID, Long pAuditID, List pRuleIds) throws JrafDaoException {
     ...
     * @throws JrafDaoException exception DAO
     */
    public MarkBO load(ISession pSession, Long pComponentID, Long pAuditID, Long pRuleId) throws JrafDaoException {
        String whereClause = "where ";
     ...
     * @throws JrafDaoException exception DAO
     */
    public Collection findWhere(ISession pSession, Long pAuditId, Long pProjectId, Long pTreId, Integer pValue, Integer pMax) throws JrafDaoException {
        String whereClause = "where ";
     ...
     * @throws JrafDaoException exception DAO
     */
    public Collection findWhereInterval(ISession pSession, Long pAuditId, Long pProjectId, Long pTreId, Double pValueMin, Double pValueMax, Integer pMax) throws JrafDaoException {
        String whereClause = "where ";
        whereClause += getAlias() + ".practice.rule.id = " + pTreId;
        whereClause += " and ";
        whereClause += getAlias() + ".practice.audit.id = '" + pAuditId + "'";
     ...
     * @return le nombre de corrections correspondant au nombre de composants
     * qui étaient à zéro et qui ne le sont plus.
     * @throws JrafDaoException si erreur
     */
    public int findCorrectionsWithProgessions(ISession pSession, Long pAuditId, Long pPreviousId) throws JrafDaoException {
     ...
     * @param pPreviousId l'id de l'audit précédent
     * @return le nombre de corrections correspondant au nombre de composants
     * qui avaient une note à 0 et qui n'existent plus.
     * @throws JrafDaoException si erreur
     */
    public int findCorrectionsWithSuppressions(ISession pSession, Long pAuditId, Long pPreviousId) throws JrafDaoException {
    ...
    /**
     * Donne la requête qui donne la liste des notes sur les nouveaux composants.
     * NB : Cette méthode sert aussi à retouver les composants supprimés (on inverse les ids des audits)
     * @param pSession la session
     ...
     * @throws JrafDaoException si erreur
     */
    private String getDeletedOrNewComponentsWhereClause(ISession pSession, Long pAuditId, Long pPreviousId, Long pProjectId) throws JrafDaoException {
        String whereClause = "where ";
        whereClause += getAlias() + ".practice.project.id = '" + pProjectId + "'";
        whereClause += " and ";
        whereClause += getAlias() + ".practice.audit.id = '" + pPreviousId + "'";
        whereClause += " and " + getAlias() + ".component.id not in (" 
                        + getComponentIdsWhere(pSession, pProjectId, pAuditId) + ")";
        return whereClause;
    }
     ...
     * @param pProjectId l'id du projet
     * @param pAuditId l'id de l'audit
     * @return la requête récupérant les ids des composants du projet d'id <code>pProjectId</code>
     * notés lors de l'audit d'id <code>pAuditId</code>
     * @throws JrafDaoException si erreur
     */
    private String getComponentIdsWhere(ISession pSession, Long pProjectId, Long pAuditId) throws JrafDaoException {
    ...
    /**
     * Donne la liste des notes sur les nouveaux composants.
     * NB : Cette méthode sert aussi à retouver les composants supprimés (on inverse les ids des audits)
     * @param pSession la session
     * @param pAuditId l'audit courant
     * @param pPreviousId l'audit de comparaison
     ...
     * @return la liste des nouveaux composants entre <code>pAuditId</code>
     * et <code>pPreviousId</code>
     * @throws JrafDaoException si erreur
     */
    public Collection findDeletedComponents(ISession pSession, Long pAuditId, Long pPreviousId, Long pProjectId, Object[] pFilter, int pLimit) throws JrafDaoException {
        ...
        if (null != pFilter[PracticeEvolutionDTO.ONLY_PRACTICES_ID]) {
            ...
            if (nbPractices > 0) { // sécurité pour le code car aucun intérêt si il n'y a pas de pratique.
     ...
     * @param pFilter le filtre
     * @param pLimit le nombre de résultats à remonter
     * @return une liste de tableau à deux éléments de MarkBO représentant toutes les notes qui sont différentes
     * entre les deux audits pour un même composant une même règle.
     * @throws JrafDaoException si erreur
     */
    public Collection findChangedComponentWhere(ISession pSession, Long pAuditId, Long pPreviousId, Long pProjectId, Object[] pFilter, int pLimit) throws JrafDaoException {
        ...
        if (null != pFilter[PracticeEvolutionDTO.ONLY_UP_OR_DOWN_ID]) {
            if (((String) pFilter[PracticeEvolutionDTO.ONLY_UP_OR_DOWN_ID]).equals(PracticeEvolutionDTO.ONLY_UP)) {
                ...
                // n'est plus noté
                query += "(m1.value > m2.value or (m1.value = " + MarkBO.NOT_NOTED_VALUE + " and m2.value != " + MarkBO.NOT_NOTED_VALUE + "))";

            } else {
                query += " and ";
                // On récupère les composants dont la note s'est dégradée (si le composant
                // n'est plus noté il s'agit d'une amélioration)
                query += "((m1.value < m2.value or m2.value = " + MarkBO.NOT_NOTED_VALUE + ") and m1.value != " + MarkBO.NOT_NOTED_VALUE + ")";
     ...
     * d'audit <code>pAuditId</code> pour la pratique d'id <code>pPracticeId</code> et dont le composant n'est 
     * pas exclu du plan d'action.
     * @param pSession session Hibernate
     ...
     * @throws JrafDaoException exception DAO
     */
    public Collection findWorstWhere(ISession pSession, Long pPracticeId, float pMark, Integer pMax) throws JrafDaoException {
    ...
    /**
     * Compte les plus mauvaises notes (inférieur à <code>pMark</code>+1)qui ont pour id 
     * d'audit <code>pAuditId</code> pour la pratique d'id <code>pPracticeId</code> et dont le composant n'est 
     * pas exclu du plan d'action.
     * @param pSession session Hibernate
     * @param pPracticeId l'id de la pratique
     ...
     * @throws JrafDaoException exception DAO
     */
    public int countWorstWhere(ISession pSession, Long pPracticeId, float pMark) throws JrafDaoException {
        String whereClause = getWorstWhereClause(pPracticeId, pMark);
