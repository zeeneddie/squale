 ...
 * 
 * @hibernate.subclass
 * discriminator-value="RulesCheckingTransgression"
 */

public class RuleCheckingTransgressionBO extends MeasureBO {
    ...
    private int totalNumberForCategoryForLevel(String pCategory, String pTransgressionLevel) {
        ...
        while (itMetricCol.hasNext()) {
            metric = (IntegerMetricBO) itMetricCol.next();
            metricName = metric.getName();
            if (pCategory.equals(getRuleCategory(metricName)) && pTransgressionLevel.equals(getSeverityOfRule(metricName))) {
                value += ((Integer) metric.getValue()).intValue();
            }
        }
        return value;
     ...
     * 
     * @hibernate.bag 
     * table="RuleCheckingTransgressionItem" 
     * lazy="true" 
     * cascade="all"
     * 
     ...
     * class="com.airfrance.squalecommon.enterpriselayer.businessobject.result.rulechecking.RuleCheckingTransgressionItemBO"
     * 
     */
