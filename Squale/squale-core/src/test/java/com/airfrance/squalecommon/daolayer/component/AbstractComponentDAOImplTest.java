 ...
 *
 */
public class AbstractComponentDAOImplTest extends SqualeTestCase {
    ...
    public void testFindChildrenWhere() {
        try {
            ...
            AuditBO audit = getComponentFactory().createAuditResult(application);
            AbstractComponentDAOImpl dao = AbstractComponentDAOImpl.getInstance();
            Collection coll = dao.findChildrenWhere(getSession(), new Long(project.getId()), new Long(audit.getId()), ComponentType.PACKAGE, "");
            assertEquals(1, coll.size());
            Collection col2 = dao.findChildrenWhere(getSession(), new Long(project.getId()), new Long(audit.getId()), ComponentType.PACKAGE, "test");
