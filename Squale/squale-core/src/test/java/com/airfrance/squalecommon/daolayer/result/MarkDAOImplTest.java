 ...
 *
 */
public class MarkDAOImplTest extends SqualeTestCase {
     ...
     */
    public void testRemoveWhereComponent() throws JrafDaoException {
        ISession session = getSession();
        session.beginTransaction();
     ...
     */
    public void testRemoveWhereProject() throws JrafDaoException {
        ISession session = getSession();
        session.beginTransaction();
    ...
    /**
     * Teste la remontée du nombre de corrections pour le calcul du ROI
     * lorsque correction=composant qui passe de 0 à > 0.
     * @throws JrafDaoException si erreur
     */
    public void testFindProgressions() throws JrafDaoException {
        ...
        int nbProgressions = mMarkDAO.findCorrectionsWithProgessions(getSession(), new Long(mAudit.getId()), new Long(audit.getId()));
    ...
    /**
     * Teste la remontée du nombre de corrections pour le calcul du ROI
     * lorsque correction=composant qui était à 0 et qui n'existe plus.
     * @throws JrafDaoException si erreur
     */
    public void testFindSuppressions() throws JrafDaoException {
        ISession session = getSession();
        session.beginTransaction();
        ...
        int nbProgressions = mMarkDAO.findCorrectionsWithSuppressions(getSession(),  new Long(audit.getId()), new Long(mAudit.getId()));
        getSession().commitTransactionWithoutClose();
        assertEquals(1, nbProgressions);
     ...
     * @throws JrafDaoException si erreur
     */
    public void testFindWhere() throws JrafDaoException {
        getSession().beginTransaction();
     ...
     * @throws JrafDaoException si erreur
     */
    public void testFindNewComponents() throws JrafDaoException {
        getSession().beginTransaction();
        // On crée un nouvel audit
        AuditBO audit = getComponentFactory().createAudit(getSession(), mProject);
        // une nouvelle pratique
        ...
        Collection pkgs = mMarkDAO.findDeletedComponents(getSession(), new Long(audit.getId()), new Long(mAudit.getId()), new Long(mProject.getId())
                                                            , new Object[NB_FILTER], 1);
        // Il doit y avoir un seul nouveau package (newPkg)
        assertEquals(1, pkgs.size());
    }
    
    /**
     * Teste la récupération des notes différentes entre deux audits pour un même composant
     * et une même pratique
     * @throws JrafDaoException si erreur
     */
    public void testFindChangedComponent() throws JrafDaoException {
        getSession().beginTransaction();
        // On crée un nouvel audit
        AuditBO audit = getComponentFactory().createAudit(getSession(), mProject);
        // une nouvelle pratique
        ...
        String type = Mapping.getComponentClass(ComponentType.PACKAGE).getName();
        Collection pkgs = mMarkDAO.findChangedComponentWhere(getSession(), new Long(mAudit.getId()), new Long(audit.getId())
