 ...
 */
public class AuditComputingTest extends SqualeTestCase {
    /**
     * Test de calcul d'audit
     * Un audit est créé artificiellement avec une grille qualité de base, les métriques
     * sont elles aussi créées en dur dans le test.
     * Une phase d'audit est ensuite réalisée avec une vérification des résultats obtenus.
     */
    public void testComputeAuditResult() {
        ...
        try {
            ...
            QualityGridBO grid = (QualityGridBO) QualityGridDAOImpl.getInstance().load(session, new Long(((QualityGridDTO) grids.iterator().next()).getId()));
            ...
            /*
             * McCabe :
             *  - cls : Maxvg = 2; Sumvg = 2; wmc = 8;
             *  - method : nsloc = 2;
             * Checkstyle :
             *  - project : totalError = 20;
             */
            ...
            /*
             * D'après la grille "grid_compute.xml" on doit avoir :
             * practice 1 class1 = 0 car 2(mccabe.maxvg) >= 0.5*2(mccabe.sumvg) et 8(mccabe.wmc) >= 8 (classe)
             * practice 1 class2 = 2 car 350(mccabe.maxvg) >= 0.3*1000(mccabe.sumvg) et 8(mccabe.wmc) >= 8 (classe)
             * --> practice 1 = 1.0 car f(x)=x est la fonction de pondération donc f^-1(1/2*(0+2)=1 
             * practice 2 = 3*(2.0/3.0)**(100.0*20/1000) = 1.33334 (projet)
             * practice 3 = 3 car aucune condition n'est remplie (mccabe.nsloc = 2 < 40) (methode)
             */
            // Note sur la classe
            final float expectedClassMark = 0f;
            Collection classMarks = MarkDAOImpl.getInstance().findWhere(session, new Long(cls.getId()), new Long(audit.getId()));
            assertEquals(1, classMarks.size());
            assertEquals(new Float(expectedClassMark), new Float(((MarkBO) classMarks.iterator().next()).getValue()));
            // Note sur la classe2
            final float expectedClassMark2 = 2f;
            Collection classMarks2 = MarkDAOImpl.getInstance().findWhere(session, new Long(cls2.getId()), new Long(audit.getId()));
            assertEquals(1, classMarks2.size());
            assertEquals(new Float(expectedClassMark2), new Float(((MarkBO) classMarks2.iterator().next()).getValue()));
            // Résultat pour practice1
            float expectedPractice1Mark = 1f;
            // Note sur la méthode
            final float expectedMethodMark = 3f;
            Collection methodMarks = MarkDAOImpl.getInstance().findWhere(session, new Long(method.getId()), new Long(audit.getId()));
            assertEquals(1, methodMarks.size());
            assertEquals(new Float(expectedMethodMark), new Float(((MarkBO) methodMarks.iterator().next()).getValue()));
            // Note sur le projet
            final float expectedProjectMark = new Float(3 * Math.pow((2.0 / 3.0), 100.0 * 20 / 1000)).floatValue();
            Collection factorResults = QualityResultDAOImpl.getInstance().findWhere(session, new Long(project.getId()), new Long(audit.getId()));
            assertEquals(2, factorResults.size());
            // La pondération sur les pratiques est identique
            Collection factorResults2 = QualityResultDAOImpl.getInstance().findWhere(session, new Long(project.getId()), new Long(audit.getId()));
            assertEquals((expectedPractice1Mark + expectedMethodMark + expectedProjectMark) / 3f, ((FactorResultBO) factorResults2.iterator().next()).getMeanMark(), 0.1);
        } catch (Exception e) {
            e.printStackTrace();
