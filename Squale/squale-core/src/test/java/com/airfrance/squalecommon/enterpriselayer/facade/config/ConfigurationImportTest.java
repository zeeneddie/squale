 ...
 */
public class ConfigurationImportTest extends SqualeTestCase {
    ...
    /**
     * Importation d'une configuration simple
    ...
    /*public void testImportSimple() {
        ...
        try {
            ...
            assertEquals(0, errors.length());
            // Les dates limites:
            Collection stopTimes = conf.getStopTimes();
            assertEquals(1, stopTimes.size());
            StopTimeDTO stopTimeDTO = (StopTimeDTO) stopTimes.iterator().next();
            assertEquals("monday", stopTimeDTO.getDay());
            assertEquals("4:00", stopTimeDTO.getTime());
            // Le récupérateur de source:
            SourceManagementDTO manager = (SourceManagementDTO) conf.getSourceManagements().iterator().next();
            assertEquals("clearcase", manager.getName());
            // Une tâche d'analyse:
            assertEquals(1, manager.getAnalysisTasks().size());
            TaskDTO analysisManager = (TaskDTO) manager.getAnalysisTasks().iterator().next();
            assertEquals("ClearCaseTask", analysisManager.getName());
            assertEquals("com.airfrance.squalix.tools.clearcase.task.ClearCaseTask", analysisManager.getClassName());
            // Une tâche de terminaison
            assertEquals(1, manager.getTerminationTasks().size());
            // Le profil:
            ProjectProfileDTO profile = (ProjectProfileDTO) conf.getProfiles().iterator().next();
            assertEquals("java1.4", profile.getName());
            assertEquals(true, profile.getExportIDE());
            // Une tâche d'analyse:
            assertEquals(1, profile.getAnalysisTasks().size());
            TaskDTO analysisProfile = (TaskDTO) profile.getAnalysisTasks().iterator().next();
            assertEquals("JCompilingTask", analysisProfile.getName());
            assertEquals("com.airfrance.squalix.tools.clearcase.task.ClearCaseTask", analysisManager.getClassName());
            // Aucune tâche de terminaison
            assertEquals(0, profile.getTerminationTasks().size());
        } catch (JrafEnterpriseException e) {
            e.printStackTrace();
            fail("unexpected exception");
        }
    ...
    /*public void testProfileDuplicate() {
        StringBuffer errors = new StringBuffer();
        InputStream stream = getClass().getClassLoader().getResourceAsStream("data/config/squalix-config_duplicate_profile.xml");
        SqualixConfigurationDTO conf;
        try {
            conf = ConfigurationImport.importConfig(stream, errors);
