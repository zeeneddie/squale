 ...
 *
 */
public class AuditDAOImplTest extends SqualeTestCase {
    /** Logger */
     ...
     */
    public void testFindDeleted() throws JrafDaoException {
        Collection coll = new ArrayList(0);
        getSession().beginTransaction();
        assertTrue(dao.findDeleted(getSession(), ((ApplicationBO)p1.getParent()).getServeurBO().getServeurId(), new ArrayList()).size() == 0);
        getSession().commitTransactionWithoutClose();
        a1.setStatus(AuditBO.DELETED);
        getSession().beginTransaction();
        dao.save(getSession(), a1);
        getSession().commitTransactionWithoutClose();
        assertEquals(dao.findDeleted(getSession(), ((ApplicationBO)p1.getParent()).getServeurBO().getServeurId(), new ArrayList()).size(), 1);
        ArrayList applis = new ArrayList(1);
        applis.add(new Long(1));
        assertEquals(dao.findDeleted(getSession(), ((ApplicationBO)p1.getParent()).getServeurBO().getServeurId(), applis).size(), 0);
    }
     ...
     */
    public void testCountWhereStatusAndSite() throws JrafDaoException {
        getSession().beginTransaction();
        int result = dao.countWhereStatusAndSite(getSession(), ((ApplicationBO)p1.getParent()).getServeurBO().getServeurId(), AuditBO.RUNNING);
        getSession().commitTransactionWithoutClose();
     ...
     */
    public void testFindAllLastAudits() throws JrafDaoException {
