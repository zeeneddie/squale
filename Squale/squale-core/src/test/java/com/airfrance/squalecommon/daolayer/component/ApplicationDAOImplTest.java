 ...
 *
 */
public class ApplicationDAOImplTest extends SqualeTestCase {

    /**
     * test la suppression d'une application
     * @throws JrafDaoException en cas d'échec
     */
    public void testRemove() throws JrafDaoException {
     ...
     * @throws JrafDaoException en cas d'échec
     */
    public void testCountWhereHaveAudit() throws JrafDaoException {
        ...
        int nbAppliQVI = appliDao.countWhereHaveAuditByStatus(getSession(), appli.getServeurBO().getServeurId(), new Integer(AuditBO.ALL_STATUS), null);
        assertTrue(nbAppliQVI == 1);
        int nbAppliQVG = appliDao.countWhereHaveAuditByStatus(getSession(), qvg.getServeurId(), new Integer(AuditBO.ALL_STATUS), null);
        assertTrue(nbAppliQVG == 0);
        int nbAppliTLS = appliDao.countWhereHaveAuditByStatus(getSession(), tls.getServeurId(), new Integer(AuditBO.ALL_STATUS), null);
     ...
     * @throws JrafDaoException en cas d'échec
     */
    public void testCountWhereHaveNoAudit() throws JrafDaoException {
        ApplicationDAOImpl appliDao = ApplicationDAOImpl.getInstance();
        AuditDAOImpl auditDao = AuditDAOImpl.getInstance();
        // on crée les différents éléments
        ApplicationBO appli = getComponentFactory().createApplicationWithSite(getSession(), "qvi");
        ...
        AuditBO executedAudit = getComponentFactory().createAuditWithStatus(getSession(), proj, new Integer(AuditBO.TERMINATED)); // terminé
        // l'audit est crée pour un projet, il faut rajouter l'audit à l'application mère 
     ...
     * @throws JrafDaoException en cas d'échec
     */
    public void testCountWhereHaveAuditTerminated() throws JrafDaoException {
        // la date n'a pas d'importance, car on ne peut pas tester sur la date, to_date est réservé à Oracle
        ApplicationDAOImpl appliDao = ApplicationDAOImpl.getInstance();
        AuditDAOImpl auditDao = AuditDAOImpl.getInstance();
        // on crée les différents éléments
        ...
        AuditBO auditTerminated1 = getComponentFactory().createAuditWithStatus(getSession(), proj1QviAppli1, new Integer(AuditBO.TERMINATED));
        // les audits rattachés à des applis de tls
        AuditBO auditFailed2 = getComponentFactory().createAuditWithStatus(getSession(), proj1QvgAppli2, new Integer(AuditBO.FAILED));
        // les audits rattachés à des applis de qvg
        AuditBO auditNotAttempted2 = getComponentFactory().createAuditWithStatus(getSession(), proj1TlsAppli3, new Integer(AuditBO.NOT_ATTEMPTED));
        // l'audit est crée pour un projet, il faut rajouter l'audit à l'application mère 
        ...
        int nbAppliQVITerm = appliDao.countWhereHaveAuditByStatus(getSession(), appli1.getServeurBO().getServeurId(), new Integer(AuditBO.TERMINATED), null);
        assertTrue(nbAppliQVITerm == 1);
        int nbAppliQVGTerm = appliDao.countWhereHaveAuditByStatus(getSession(), appli2.getServeurBO().getServeurId(), new Integer(AuditBO.TERMINATED), null);
        assertTrue(nbAppliQVGTerm == 0);
        int nbAppliTLSTerm = appliDao.countWhereHaveAuditByStatus(getSession(), appli3.getServeurBO().getServeurId(), new Integer(AuditBO.TERMINATED), null);
        ...
        assertTrue(nbAppliTLS == 0);
        Collection applisQvi = appliDao.findWhereHaveNotAttemptedAuditBySite(getSession(), appli1.getServeurBO().getServeurId());
     ...
     */
    public void testFindWhereHaveNotAttemptedAuditBySite() throws JrafDaoException {
        // Initialisation DAO
        ApplicationDAOImpl appliDao = ApplicationDAOImpl.getInstance();
        ...
        AuditBO auditNotAttempted = getComponentFactory().createAuditWithStatus(getSession(), projTls, new Integer(AuditBO.NOT_ATTEMPTED));
        ...
        AuditBO auditFailed = getComponentFactory().createAuditWithStatus(getSession(), projQvg, new Integer(AuditBO.FAILED));
        ...
        Collection applisTls = appliDao.findWhereHaveNotAttemptedAuditBySite(getSession(), appliTls.getServeurBO().getServeurId());
        assertEquals(1, applisTls.size());
        Collection applisQvg = appliDao.findWhereHaveNotAttemptedAuditBySite(getSession(), appliQvg.getServeurBO().getServeurId());
     ...
     * @throws JrafDaoException si erreur
     */
    public void testFindWhereStatus() throws JrafDaoException {
        ApplicationDAOImpl appliDao = ApplicationDAOImpl.getInstance();
        AuditDAOImpl auditDao = AuditDAOImpl.getInstance();
        // on crée les différents éléments
        getSession().beginTransaction();
