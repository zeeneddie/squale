 ...
 * 
 */
public class NewsDAOImplTest extends SqualeTestCase {
    ...
    /**
     * Test la récupération en base de news en fonction de leurs catégories
     * (toutes, les courantes, les anciennes)
     *
     */
    public void testFindWhereKind() {
        ...
        try {
            ...
            /* le todate ne marche pas en test unitaire 
            on ne peut pas effectuer les tests suivants 
            session.beginTransaction();
            coll = dao.findWhereKind(session, "old");
            session.commitTransactionWithoutClose();
            assertTrue(coll.size() == 0);
            session.beginTransaction();
            coll = dao.findWhereKind(session, "current");
            session.commitTransaction();
            assertTrue(coll.size() > 0);
            */
