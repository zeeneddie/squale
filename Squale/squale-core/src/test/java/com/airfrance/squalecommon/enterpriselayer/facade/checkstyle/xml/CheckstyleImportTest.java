...
/**
 * Test d'importation au format XML du fichier de configuration checkstyle
 * Le test vérifie essentiellement que la mapping est correct au niveau
 * du digester en s'assurant que tous les attributs présents dans le fichier
 * XML sont bien mappés dans le classes métier
 */
public class CheckstyleImportTest extends SqualeTestCase {
    ...
    /**
     * Test d'importation avec mauvais regroupement
     * Le fichier de test comporte une incohérence sur la sévérité entre
     * un module checkstyle et la règle checkstyle
     */
    public void testImportWithBadRegrouping() {
        try {
                InputStream stream = getClass().getClassLoader().getResourceAsStream("data/checkstyle/AFcheckStyle_new_parsing_bad_regrouping.xml");
                StringBuffer errors=new StringBuffer();
                // parsing du contenue du fichier
                CheckstyleImport importch= new CheckstyleImport();
                CheckstyleRuleSetBO versionRes=importch.importCheckstyle(stream,errors);
