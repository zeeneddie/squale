		...
		<af:canvasCenter>
			...
			<af:form action="roi.do" method="POST">
				...
				<table width="100%" class="tblh" cellpadding="0" cellspacing="0" 
					...
					<tr class="fondClair">
						<td class="td1">
							<bean:message key="roi.total_in_unit" />
						</td>
						<td width="75%">
							<bean:write name="roiForm" property="total"/>
						</td>
					</tr>
					<%-- Le ROI total sur 1 an (t-1an) en K€ --%>
					<tr>
						<td class="td1">
							<bean:message key="roi.total_in_keuros" />
						</td>
						<td width="85%">
							<bean:write name="roiForm" property="totalInKEuros"/>
						</td>
					</tr>
				</table>
				...
				<%-- On affiche le graphe si une formule a été définie, sinon on affiche un message pour le signaler--%>
					...
					<logic:notEmpty name="roiForm" property="formula" >
						<img src="/squale/roi.do?action=viewRoiChart" title="<%=imageDetails%>" />
					</logic:notEmpty>
					<br/><br/><br/>
				...
				<table width="100%" class="formulaire" cellpadding="0"
					...
					<tr>
						...
						<td valign="middle">
							<af:select property="roiApplicationId">
								<af:option key="roi.all_applications" value="-1" />
								<logic:present name="<%=com.airfrance.welcom.struts.util.WConstants.USER_KEY%>" scope="session">
					...
					<tr>
						<af:field key="roi.unit_in_keuros" property="KEuros" size="10" styleClassLabel="td1"/>
						<td>
							<af:button name="recalculate" callMethod="setKEuros" 
										toolTipKey="toolTip.roi.recalculate"/>
						</td>
					</tr>
					
					<tr>
						<af:field key="roi.formula" property="formula" styleClassLabel="td1" size="40"/>
						<td>
							<af:button name="modify" callMethod="updateFormula" 
										toolTipKey="toolTip.roi.modify.formula"/>
						</td>
					</tr>
				</table>
			</af:form>
