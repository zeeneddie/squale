...
<logic:equal name="kindOfTaskId" value="<%=ParametersConstants.ECLIPSE_VARS%>">
	<af:form action="create_eclipse_var.do?addParameters">
		...
		<input name="kindOfTask" value="<%=kindOfTask%>" type="hidden"
			...
			<THEAD>
				...
				<tr>
					<af:field key="project_creation.java.eclipse.var.creation.name" size="20"
							property="name" styleClassLabel="td1" isRequired="true" />
				</tr>
				<tr class="fondClair">
...
<logic:equal name="kindOfTaskId" value="<%=ParametersConstants.ECLIPSE_LIBS%>">
	<af:form action="create_eclipse_lib.do?addParameters">
		...
		<input name="kindOfTask" value="<%=kindOfTask%>" type="hidden"
			...
			<tr>
				<af:field key="project_creation.java.eclipse.lib.creation.name" size="20"
							property="name" styleClassLabel="td1" isRequired="true" />
			</tr>
		...
		<squale:iteratePaths name="eclipseUserLibForm"
			key="project_creation.java.eclipse.lib.creation.lib"
			property="libs" disabled="<%=disabled%>" isRequired="true"/>
			</table>
			<af:buttonBar>
...
<af:form action="create_project_list_compiling.do">
	...
	<logic:empty name="antProject">
		<h3><bean:message key="project_creation.compiling.java.eclipse.parameters"/></h3>
		<br/>
		...
		<table id="eclipseTable" width="100%" class="tblh" cellpadding="0"
			...
			<THEAD>
				<tr>
					<bean:message
						key="project_creation.compiling.java.eclipse.version" />
				</tr>
			</THEAD>
	...
	pageContext.setAttribute("eclipseVersions", eclipseVersions);%>
			<tr>
				...
				<td><af:select property="bundlePathDefault" disabled="<%=disabled%>">
	...
	if (lastDot > 0) {
		...
		<div id="eclipseDiv" style="<%=displayEclipseParams%>">
			<h3><bean:message key="project_creation.compiling.java.eclipse.variable.title" /></h3>
			<%-- variables eclipse --%> 
			<af:dropDownPanel titleKey="buttonTag.menu.aide">
			...
			<af:table
				name="jCompilingForm" property="eclipseVars" totalLabelKey="empty" emptyKey="table.results.none">
				...
				<logic:notEqual name="userProfile"
								value="<%=ProfileBO.READER_PROFILE_NAME%>">
					<af:buttonBar>
						<af:button onclick="<%=\"submitWithAction('jCompilingForm', 'addParameters', '\"+ParametersConstants.ECLIPSE_VARS+\"');\"%>"
									name="add_eclipse_var" singleSend="true" />
						<bean:size id="nbVars" name="jCompilingForm"
							property="eclipseVars" />
			...
			<br />
			<h3><bean:message key="project_creation.compiling.java.eclipse.lib.title" /></h3>
			...
			<af:table
				name="jCompilingForm" property="eclipseLibs" totalLabelKey="empty" emptyKey="table.results.none">
				...
				<logic:notEqual name="userProfile"
								value="<%=ProfileBO.READER_PROFILE_NAME%>">
					<af:buttonBar>
						<af:button onclick="<%=\"submitWithAction('jCompilingForm', 'addParameters', '\"+ParametersConstants.ECLIPSE_LIBS+\"');\"%>"
									name="add_eclipse_lib" singleSend="true" />
						<bean:size id="nbLibs" name="jCompilingForm"
							property="eclipseLibs" />
