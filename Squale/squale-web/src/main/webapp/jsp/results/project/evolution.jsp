...
<bean:define id="previousAuditId" name="evolutionForm"
	property="previousAuditId" type="String" />

<bean:define id="results" name="evolutionForm" property="results"
	type="java.util.Map" />
<%if(results.size() == 0){%>
	...
	<af:body canvasLeftPageInclude="/jsp/canvas/project_menu.jsp">
		...
		<af:canvasCenter>
			...
			<af:form action="evolution.do" scope="session">
				...
				<af:dropDownPanel titleKey="reference.filter"
					...
					<table width="50%" class="formulaire" cellpadding="0"
						...
						<tr class="fondClair">
							...
							<table>
								<tr>
									...
									<td class="td1"><af:select property="filterOnlyUpOrDown">
							...
							<table>
								<tr>
									...
									<td class="td1"><bean:message
										key="evolution.filter_threshold" />&nbsp; <af:select
										property="comparisonSign">
										<af:option key="&lt;" value="&lt;" />
										<af:option key="&gt;" value="&gt;" />
										<af:option key="=" value="=" />
									</af:select></td>
									<af:field key="empty" property="threshold" size="10"
										isRequired="true" />
								</tr>
							</table>
							</td>
						...
						<tr class="fondClair">
							...
							<table>
								<tr>
									...
									<td class="td1"><bean:message
										key="evolution.filter_practices" /></td>
								</tr>
							</table>
							</td>
						</tr>
						<tr class="fondClair">
							...
							<table>
								<tr>
									<td><af:select property="practices" multiple="true"
										isRequired="true">
										<logic:iterate name="evolutionForm"
											property="availablePractices" id="practiceId" type="String">
											<af:option key="<%=practiceId%>"
												value='<%=practiceId.replaceFirst("rule.", "")%>' />
										</logic:iterate>
									</af:select> <%-- Champs caché juste pour conserver les données 
										dans le cas d'un wValidate --%> <af:select
										property="availablePractices" style="display:none">
					...
					<af:buttonBar>
						<af:button type="form" name="evolution.filter.button.validate"
							callMethod="list" toolTipKey="evolution.filter.toolTip.validate" />
					</af:buttonBar>
				</af:dropDownPanel>
				<br />
				...
				<af:select property="sortType"
					onchange="form.action.value='list';form.submit()">
					<af:option key="evolution.group_by_components"
						value="<%=EvolutionForm.COMPONENT_FOR_KEY%>" />
					<af:option key="evolution.group_by_practices"
						value="<%=EvolutionForm.PRACTICE_FOR_KEY%>" />
				</af:select>

				<logic:notPresent name="emptyList">

					<%-- Message dans le cas où la limite des résultats affichables est atteinte--%>
					<logic:equal name="<%=EvolutionAction.DISPLAY_RESULTS_MSG%>"
						value="<%=Boolean.TRUE.toString()%>">
						<div class="exergue"><br />
						<bean:message key="evolution.info.limit"
							arg0='<%=""+EvolutionAction.MAX_RESULTS%>' /> <br />
						<br />
						</div>
					</logic:equal>

					<logic:equal name="evolutionForm" property="sortType"
						value="<%=EvolutionForm.COMPONENT_FOR_KEY%>">
							...
							<af:cols id="component" idIndex="componentId">
								...
								<af:col property="name" sortable="true"
									...
									<af:dropDownPanel titleKey="<%=titleKey%>"
										...
										%>
										<af:table name='<%="practices" + componentId%>'
											property="list" emptyKey="table.results.none"
											totalLabelKey="empty">
											<af:cols id="practice">
												<bean:define id="practiceName" name="practice"
													property="name" type="String" />
												<af:col property="name" sortable="true" key="practice.name">
													<html:link
														href='<%="project.do?action=practice&projectId="+projectId
							...
							<af:cols id="practice" idIndex="idPractice">
								...
								<af:col property="name" sortable="true" key="practice.name">
									...
									<af:dropDownPanel titleKey="<%=currentName%>"
										...
										%>
										<af:table name='<%="components" + idPractice%>'
											property="list" emptyKey="table.results.none"
											totalLabelKey="empty">
											<af:cols id="component">
												<bean:define id="type" name="component" property="type"
													type="String" />
												<af:col property="name" key="project.component.name"
													sortable="true"
													href='<%="project_component.do?action=component&projectId="+projectId
																+"&currentAuditId="+currentAuditId+"&previousAuditId="+previousAuditId%>'
													paramId="component" paramName="component"
													paramProperty="id" />
												<af:col property="type" key="project.component.type"
													sortable="true">
												...
												<af:col property="practices.list[0].predecessorMark"
													key="project.result.practice.value.previous"
													sortable="true" type="Number">
													<squale:mark name="component"
														mark="practices.list[0].predecessorMark" />
												</af:col>
												<af:col property="practices.list[0].currentMark"
													key="project.result.practice.value" sortable="true"
													type="Number">
													<squale:mark name="component"
														mark="practices.list[0].currentMark" />
												</af:col>
												<af:col property="practices.list[0].currentMark"
													width="100px" key="project.result.factor.tendance">
													<squale:trend name="component"
														current="practices.list[0].currentMark"
														predecessor="practices.list[0].predecessorMark" />
												</af:col>
											</af:cols>
										</af:table>
									</af:dropDownPanel>
