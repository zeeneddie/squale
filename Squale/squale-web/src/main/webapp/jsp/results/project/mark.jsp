...
<bean:define id="tres" name="markForm"
	property="treNames" type="java.util.List" />
...
<af:page titleKey="project.results.title">
	...
	<af:body canvasLeftPageInclude="/jsp/canvas/project_menu.jsp">
		...
		<af:canvasCenter>
			...
			<br />
			<h2><bean:message key="<%=subTitleKey%>" arg0="<%=minMarkMessage%>"
				arg1="<%=maxMarkMessage%>" arg2="<%=practiceName%>" /></h2>
			<br />
			...
			<br />
				...
				<af:table name="markForm" property="components" scope="session">
					<af:cols id="component">
						<bean:define id="fullName" name="component" property="fullName" type="String"/>
						<bean:define id="componentId" name="component" property="id" type="Long"/>
						<%
							String urlComponent =  "/squale/project_component.do?action=component&projectId="
    												+ projectId
    												+ "&currentAuditId="
    												+ currentAuditId
    												+ "&previousAuditId="
						...
						<af:col property="name" sortable="true" key="component.name"
							...
							<a href="<%=urlComponent%>" title="<%=fullName%>"><bean:write name="component" property="name" /></a>
						</af:col>
...
if (lastDot > 0) {
    shortTre = "<span title=\"" + WebMessages.getString(request, tre) + "\">" + tre.substring(lastDot + 1) + "</span>";
			...
			<af:buttonBar>
				<%
					String urlExportPDF = "/squale/mark.do?action=exportMarkToPDF&projectId=" + projectId + "&currentAuditId=" + currentAuditId + "&previousAuditId=" + previousAuditId;
					String urlExportExcel = "/squale/mark.do?action=exportMarkToExcel&projectId=" + projectId + "&currentAuditId=" + currentAuditId + "&previousAuditId=" + previousAuditId;
				%>
			...
			<br />
			<fieldset><legend><b><bean:message key="image.legend" /></b></legend>
				<%=SqualeWebActionUtils.getLegendForTres(request, tres)%>
			</fieldset>
		</af:canvasCenter>
	</af:body>
</af:page>