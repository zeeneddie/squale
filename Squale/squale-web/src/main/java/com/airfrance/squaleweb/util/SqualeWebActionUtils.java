...
public class SqualeWebActionUtils {
    ...
    public static final String[] IMG =
        { "/squale/images/pictos/bad.png", "/squale/images/pictos/good.png", "/squale/images/pictos/best.png", "/squale/images/pictos/super.png", "/squale/images/pictos/na.gif" };
    ...
    /** 
     * défini une valeur de seuil significatif pour une variation de la tendance
     * de constante à un peux mieux
     * Tant que la variation de la note est en dessous de ce seuil, on considère qu'il
    ...
    */
    public static String getImageForTrend(String currentMark, String predecessorMark) {
        ...
        if (isValidMark(currentMark) && isValidMark(predecessorMark)) {
            float diff = Float.parseFloat(currentMark.replace(',', '.')) - Float.parseFloat(predecessorMark.replace(',', '.'));
    ...
    public static String generatePictoWithTooltip(String pNote, HttpServletRequest pRequest) {
        ...
        if (imgIndex != IMG.length - 1) {
            ...
            if (imgIndexRound > imgIndex) {
                pictureHelp += " (" + WebMessages.getString(pRequest, "project.results.mark.nearly.status_" + imgIndexRound) + ")";
            }
        }
        // on remplace ' par \' pour le javascript
        return "<img src=\"" + IMG[imgIndex] + "\"title=\"" + pictureHelp.replaceAll("'", "\\\\'") + "\" border=\"0\" />";
    }
     ...
     * @return la note sous sa forme flottante en la tronquant
     ...
     * @return le float arrondi à un chiffre après la virgule ou "-" si 
     * la chaine ne peut pas etre correctement formatée
     */
     ...
     * @return la note sous sa forme flottante en l'arrondissant
     * à un chiffre après la virgule
     */
     ...
     * La classe de l'objet doit avoir le setter associé au nom de l'attribut : setMonAttribut
     * et doit prendre une String en paramètre.
     * 
    ...
    public static void setValue(final Object pObject, final String pSetterName, final String pValue) {
        ...
        try {
            ...
            if (null != pValue && ((pValue.matches("-?[0-9]*") && Integer.parseInt(pValue) != -1) || !pValue.matches("-?[0-9]*"))) {
                params[0] = pValue;
            }
     ...
     * @param pMessages précise si les nom des attributs sont des clés à résoudre, ils sont alors suffixés de ".attribute".
     * 
     */
    public static void fullFillBean(final Object pBean, final List pAttributes, final List pValues, final boolean pMessages) {
     ...
     * @param pKey précise si les valeurs suffixés sont des clés, auquel cas elles sont remplacées par les valeurs associées.
    ...
    /**
     * Permet d'aller à la ligne dans un menu pour pouvoir afficher le nom en entier
     * au lieu qu'il soit tronqué car la charte graphique d'Air France impose une taille fixe
     * 
    ...
    /**
     * Récupére la clé de configuration représentant le nombre minimum d'applications que doit avoir 
     * le menu pour être groupé
     ...
     * @return le type servant à la clé représentant le statut
     * de l'audit.
     */
