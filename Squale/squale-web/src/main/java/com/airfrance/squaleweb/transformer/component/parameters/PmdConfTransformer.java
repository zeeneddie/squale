...
/**
 * Transformation des paramètres PMD
 * Les paramètres PMD sont communs dans un contexte java pur ou java avec jsp,
 * le transformer tient compte du contexte
 */
public class PmdConfTransformer implements WITransformer {
    ...
    public void objToForm(Object[] pObject, WActionForm pForm) throws WTransformerException {
        ...
        for (Iterator it = versions.iterator(); it.hasNext();) {
            ...
            } else { // Il s'agit forcement de jsp
        ...
        if (pmdParams != null) {
            StringParameterDTO param = (StringParameterDTO) pmdParams.getParameters().get(ParametersConstants.PMD_JAVA_RULESET_NAME);
        ...
        if (jspRequired(task)) {
            ...
            // Les paramètre PMD peuvent ne pas être présents
            if (pmdParams != null) {
                StringParameterDTO param = (StringParameterDTO) pmdParams.getParameters().get(ParametersConstants.PMD_JSP_RULESET_NAME);
                // Le paramètre peut ne pas être renseigné
                if (param != null) {
