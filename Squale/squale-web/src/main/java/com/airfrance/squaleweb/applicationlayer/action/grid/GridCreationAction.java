 ...
 */
public class GridCreationAction
    extends AdminAction
{
                                     ...
                                     HttpServletResponse pResponse )
    {
        ...
        try
        {
            ...
            Collection grids =
                (Collection) ac.execute( "importGrid", new Object[] { form.getInputStream(), importErrors } );
     ...
     */
    public ActionForward confirmGrid( ActionMapping pMapping, ActionForm pForm, HttpServletRequest pRequest,
                                      HttpServletResponse pResponse )
    {
        ActionForward forward;
        ActionErrors errors = new ActionErrors();
        ...
        if ( ( gridFile != null ) && gridFile.exists() )
        {
            try
            {
                ...
                else
                {
                    ...
                    GridListForm gridListForm =
                        (GridListForm) WTransformerFactory.objToForm( GridListTransformer.class, grids );
                    // en session car WIZARD
                    pRequest.getSession().setAttribute( GRID_CONFIRM_FORM, gridListForm );
                }
            }
     ...
     * @return forward correspondant
     */
    public ActionForward notify( ActionMapping pMapping, ActionForm pForm, HttpServletRequest pRequest,
                                 HttpServletResponse pResponse )
    {
        ActionForward forward;
        ActionErrors errors = new ActionErrors();
            ...
            while ( gridsIt.hasNext() )
            {
                ...
                while ( projIt.hasNext() )
                {
                    ...
                    if ( !appliColl.contains( appliId ) )
                    {
                        ...
                        String content =
                            header
                                + (String) WebMessages.getString( pRequest.getLocale(), "mail.grid.change.content",
                                                                  params );
                        SqualeCommonUtils.notifyByEmail( MailerHelper.getMailerProvider(),
                                                         SqualeCommonConstants.ONLY_MANAGERS, appliId, object, content,
                                                         false );
                    }
                }
            }
     ...
     * @return forward correspondant
     */
    public ActionForward cancelGrid( ActionMapping pMapping, ActionForm pForm, HttpServletRequest pRequest,
                                     HttpServletResponse pResponse )
    {
