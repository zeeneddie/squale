 ...
 * @author
 */
public class ProjectResultsAction extends ReaderAction {

    /** 
     * Le nom désignant l'attribut en session pour indiquer si on veut tous les facteurs ou non
     * pour le kiviat.
     */
     ...
     */
    public ActionForward select(ActionMapping pMapping, ActionForm pForm, HttpServletRequest pRequest, HttpServletResponse pResponse) {
     ...
     * @return l'action à réaliser.
     */
    public ActionForward summary(ActionMapping pMapping, ActionForm pForm, HttpServletRequest pRequest, HttpServletResponse pResponse) {
        ...
        try {
            ...
            if (null == forward) {
                if (getSummary(pRequest, pForm)) {
                    ...
                    AuditDTO audit = (AuditDTO) pRequest.getSession().getAttribute(BaseDispatchAction.CURRENT_AUDIT_DTO);
                    ...
                    if (project != null && audit != null) {
                        ...
                        String fileNameKiviat = ServletUtilities.saveChartAsPNG(chartKiviat, KiviatMaker.DEFAULT_WIDTH, KiviatMaker.DEFAULT_HEIGHT, infoKiviat, pRequest.getSession());
                        ...
                        pRequest.getSession().setAttribute("kiviatChart", chartKiviat.createBufferedImage(KiviatMaker.DEFAULT_WIDTH, KiviatMaker.DEFAULT_HEIGHT));
        ...
        updateHistTracker(WebMessages.getString(pRequest, "tracker.synthesis"), "project.do?action=summary", TrackerStructure.UNDEFINED, pRequest, true);
     ...
     */
    private boolean getSummary(final HttpServletRequest pRequest, final ActionForm pForm) throws Exception {
        ComponentDTO project = (ComponentDTO) WTransformerFactory.formToObj(ProjectTransformer.class, ActionUtils.getCurrentProject(pRequest))[0];
        ...
        if (result) {
            ...
            Boolean canBeExportedToIDE = ((Boolean) ac.execute("canBeExportedToIDE", new Object[]{new Long(project.getID())}));
     ...
     */
    private Collection getFactorsList(final List pAuditsList, final ComponentDTO pProject) throws Exception {
        IApplicationComponent ac = AccessDelegateHelper.getInstance("Results");
     ...
     * @return l'action à réaliser.
     */
    public ActionForward factor(ActionMapping pMapping, ActionForm pForm, HttpServletRequest pRequest, HttpServletResponse pResponse) {
        ...
        // Mise en place du traceur historique
        updateHistTracker(WebMessages.getString(pRequest, name), "project.do?action=factor&which=" + factorRuleId, TrackerStructure.FACTOR_VIEW, pRequest, false);
        //Indique que l'on vient d'une vue synthèse et pas d'une vue composant
        changeWay(pRequest, "false");
     ...
     */
    private FactorRuleDTO getFactorRule(long pFactorRuleId) throws JrafEnterpriseException {
     ...
     * @throws Exception si un pb apparaît.
     */
    private ResultForm getFactorResult(final FactorRuleDTO pFactor, final List pAudits, final ComponentDTO pProject, final HttpServletRequest pRequest) throws Exception {
        IApplicationComponent ac = AccessDelegateHelper.getInstance("Results");
     ...
     */
    private ResultListForm getCriteriaList(final FactorRuleDTO pFactorRule, final List pAudits, final ComponentDTO pProject) throws Exception {
        IApplicationComponent ac = AccessDelegateHelper.getInstance("Results");
     ...
     * @throws Exception si un pb apparaît.
     */
    private void getCriteriumResult(final CriteriumRuleDTO pCriteriumRuleDTO, final CriteriumForm pCriteriumForm, final List pAudits, final ComponentDTO pProject) throws Exception {
        IApplicationComponent ac = AccessDelegateHelper.getInstance("Results");
    ...
    /* =======================================================================================================
     *                  Page de détails d'une pratique
     * =======================================================================================================
     */
     ...
     * @return l'action à réaliser.
     */
    public ActionForward practice(ActionMapping pMapping, ActionForm pForm, HttpServletRequest pRequest, HttpServletResponse pResponse) {
        ActionForward forward = null;
        ActionErrors errors = new ActionErrors();
        try {
            ...
            if (form instanceof ResultRulesCheckingForm) {
                ...
                if (resForm.getFormulaType() != null) {
                    ...
                    RepartitionBarMaker barMaker =
                        new RepartitionBarMaker(pRequest, resForm.getProjectId(), resForm.getCurrentAuditId(), resForm.getPreviousAuditId(), resForm.getId(), resForm.getParentId());
                    ...
                    String repartitionFileName =
                        ServletUtilities.saveChartAsPNG(barMaker.getChart(), RepartitionBarMaker.DEFAULT_WIDTH, RepartitionBarMaker.DEFAULT_HEIGHT, infoRepartition, pRequest.getSession());
                    ...
                    if (resForm.getFormulaType().equals(AbstractFormulaBO.TYPE_SIMPLE)) {
                        RepartitionMaker maker =
                            new RepartitionMaker(pRequest, resForm.getProjectId(), resForm.getCurrentAuditId(), resForm.getPreviousAuditId(), resForm.getId(), resForm.getParentId());
                        maker.build(resForm.getFloatRepartition());
                        // Sauvegarde de l'image de l'histogramme au format png dans un espace temporaire
                        infoRepartition = new ChartRenderingInfo(new StandardEntityCollection());
                        repartitionFileName =
                            ServletUtilities.saveChartAsPNG(maker.getChart(), RepartitionMaker.DEFAULT_WIDTH, RepartitionMaker.DEFAULT_HEIGHT, infoRepartition, pRequest.getSession());
                    ...
                    } else { // on remet l'histogramme à null pour le réinitialiser et ne pas l'afficher
     ...
     */
    private WActionForm getForm(ActionMapping pMapping, HttpServletRequest pRequest, ActionForm pForm) throws JrafEnterpriseException, NumberFormatException, WTransformerException {
        ...
        if (pForm == null || null == (init(pMapping, pRequest, pForm))) {
            ...
            result = WTransformerFactory.objToForm(transformerClass, new Object[] { resultDTO, factor, practice, project, audits, infoForm, pRequest.getLocale() });
     ...
     */
    private void manageTrackerFromPractice(HttpServletRequest pRequest, String pParamParent, String pPracticeName, Long pPracticeId) {
        //Mise en place du traceur historique
    ...
    /* =======================================================================================================
     *                  Page de détails d'une transgression
     * =======================================================================================================
     */
     ...
     * @return l'action à réaliser.
     */
    public ActionForward ruleCheckingDetails(ActionMapping pMapping, ActionForm pForm, HttpServletRequest pRequest, HttpServletResponse pResponse) {
        ...
        try {
            ...
            RuleCheckingItemsListForm itemsForm = (RuleCheckingItemsListForm) WTransformerFactory.objToForm(RuleCheckingItemsListTransformer.class, new Object[] { ruleName, details });
        ...
        String url = "project.do?action=ruleCheckingDetails&measureId=" + measureId + "&ruleId=" + ruleId + "&ruleName=" + rule;
    ...
    /* =======================================================================================================
     *                  Page d'affichage des composants ayant une note précise sur une pratique
     * =======================================================================================================
     */
     ...
     * @return l'action à réaliser.
     */
    public ActionForward mark(ActionMapping pMapping, ActionForm pForm, HttpServletRequest pRequest, HttpServletResponse pResponse) {
        ...
        try {
            if (null == (forward = init(pMapping, pRequest, pForm))) {
                ...
                practice = (PracticeRuleDTO) acGrid.execute("getQualityRule", new Object[] { practice, new Boolean(false)});
                FactorRuleDTO factor = new FactorRuleDTO();
                if (factorParent != null) {
                    // Obtention du facteur
                    long factorId = Long.parseLong(factorParent);
                    factor.setId(factorId);
                    factor = (FactorRuleDTO) acGrid.execute("getQualityRule", new Object[] { factor, new Boolean(false)});
                } else {
                    factor.setId(-1);
                    factor.setName("");
                }
                ...
                if (value != null) { // Clic sur le tableau de répartition
                    ...
                    Object[] paramIn = { audit, project, new Long(practiceId), intValue, new Integer(WebMessages.getString(Locale.getDefault(), "component.max"))};
                    // Appel de la couche métier
                    result = (ResultsDTO) ac.execute("getValueResults", paramIn);
                    //Transformation vers le formulaire
                    WTransformerFactory.objToForm(MarkTransformer.class, (WActionForm) pForm, new Object[] { result, factor, practice, intValue });
                    ...
                    // Un nombre maximal de résultats est retrouvé
                    Object[] paramIn = { audit, project, new Long(practiceId), minMark, maxMark, new Integer(WebMessages.getString(Locale.getDefault(), "component.max"))};
                    // Appel de la couche métier
                    result = (ResultsDTO) ac.execute("getValueResultsForInterval", paramIn);
                    // Transformation vers le formulaire
                    WTransformerFactory.objToForm(MarkTransformer.class, (WActionForm) pForm, new Object[] { result, factor, practice, minMark, maxMark });
    ...
    /**
     * Initialise l'action.
     * Si une opération est à effectuer parce que les données
     * de session ne permettent pas de poursuivre l'action, alors un forward non null
     * est renvoyé.
     * @param pMapping le mapping.
     ...
     */
    private ActionForward init(ActionMapping pMapping, HttpServletRequest pRequest, ActionForm pForm) throws JrafEnterpriseException, WTransformerException {
        ActionForward forward = null;
     ...
     * Les données concernant le projet sont placées en session, à savoir le formulaire
     * du projet ainsi que le contenu du menu "Top"
     ...
     * @throws WTransformerException si erreur
     */
    ProjectForm setupProject(Long pProjectId, HttpServletRequest pRequest, ActionForm pForm) throws JrafEnterpriseException, WTransformerException {
        ProjectForm project = null;
        ...
        if (null != dto) {
            ...
            if (null == currentApplication || currentApplication.getId() != dto.getIDParent()) {
                ApplicationForm application = ActionUtils.getComponent(dto.getIDParent(), getUserApplicationList(pRequest));
            }
            Object paramToTransform[] = { dto };
            // Création du form pour afficher le composant
            ComponentForm projectCompForm = (ComponentForm) WTransformerFactory.objToForm(ComponentTransformer.class, paramToTransform);
     ...
     * @throws WTransformerException si erreur
     */
    private List initAudit(HttpServletRequest pRequest, ComponentDTO pComponent) throws WTransformerException, JrafEnterpriseException {
     ...
     */
    public ActionForward exportPDFDetailedActionPlan(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ServletException {
        /*
         * 1. On récupère les plus mauvaises pratiques triées par l'effort à fournir pour progresser
         * 2. Pour chaque pratique, on remonte les pires composants (ou transgressions) dans la limite de 100
         */
        ...
        try {
            IApplicationComponent ac = AccessDelegateHelper.getInstance("Results");
            badPractices = (Collection) ac.execute("getWorstPractices", new Object[] { auditId, projectId, new Boolean(true)});
            ...
            PDFDataJasperReports data =
                new PDFDataJasperReports(request.getLocale(), getResources(request), badPractices, "/com/airfrance/squaleweb/resources/jasperreport/ActionPlan.jasper", false, parameters);
    ...
    /**
     * Export IDE au format XML.
     * L'export comportement les plus mauvaises pratiques avec les 100 plus mauvais composants
     ...
     */
    public ActionForward exportIDE(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ServletException {
        ActionForward forward = null;
        Collection badPractices;
        // On récupère l'id du projet et de l'audit
        RootForm rootForm = (RootForm) form;
        ...
        try {
            ...
            badPractices = (Collection) ac.execute("getWorstPractices", new Object[] { auditId, projectId, new Boolean(true)});
            ac = AccessDelegateHelper.getInstance("Component");
            String workspace = (String) ac.execute("getProjectWorkspace", new Object[] { new Long(Long.parseLong(projectId))});
            XMLData xmlData = new IDELinkXMLData(request, rootForm.getAuditDate(), rootForm.getApplicationName(), workspace, badPractices);
            // On récupère le label de l'audit si il s'agit d'un audit de jalon
            if (rootForm.getAuditName().matches(".+\\(.+\\)")) { // le label se trouve avant la première parenthèse
     ...
     * @throws ServletException exception pouvant etre levee
     */
    public ActionForward exportPDFActionPlan(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ServletException {
        /*
         * 1. On récupère les plus mauvaises pratiques triées par l'effort à fournir pour progresser
        ...
        try {
            IApplicationComponent ac = AccessDelegateHelper.getInstance("Results");
            badPractices = (Collection) ac.execute("getWorstPractices", new Object[] { auditId, projectId, new Boolean(false)});
     ...
     * @throws ServletException exception pouvant etre levee
     */
    public ActionForward exportPDF(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ServletException {
        ...
        try {
            ...
            parameters.put("scatterplot", bubbleChart.createBufferedImage(BubbleMaker.DEFAULT_WIDTH, BubbleMaker.DEFAULT_HEIGHT));
            // Le nom de l'utilisateur
            LogonBean logon = (LogonBean) request.getSession().getAttribute(WConstants.USER_KEY);
            parameters.put("userName", logon.getMatricule());
            ...
            PDFDataJasperReports pdfData =
                new PDFDataJasperReports(request.getLocale(), getResources(request), data, "/com/airfrance/squaleweb/resources/jasperreport/ProjectSummary.jasper", false, parameters);
     ...
     * @throws ServletException exception pouvant etre levee
     */
    public ActionForward exportMarkToExcel(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ServletException {
        //Le nom de l'utilisateur
        LogonBean logon = (LogonBean) request.getSession().getAttribute(WConstants.USER_KEY);
        try {
            ExcelDataMarkList data = new ExcelDataMarkList(request.getLocale(), getResources(request), (MarkForm) form, logon.getMatricule());
     ...
     * @throws ServletException exception pouvant etre levee
     */
    public ActionForward exportMarkToPDF(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ServletException {
        MarkForm theForm = (MarkForm) form;
        Collection data = theForm.getComponents();
        try {
            HashMap parameters = new HashMap();
            PDFDataJasperReports pdfData =
                new PDFDataJasperReports(request.getLocale(), getResources(request), data, "/com/airfrance/squaleweb/resources/jasperreport/metricsResults.jasper", false, parameters);
            ...
            if (theForm.getMinMark() != theForm.getMaxMark()) {
                ...
                minMarkMessage = WebMessages.getString(request, "project.results.mark.status_" + theForm.getMarkValue());
            }
     ...
     * @throws ServletException exception pouvant etre levee
     */
    public ActionForward exportTransgressionsToPDF(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ServletException {
        ProjectSummaryForm theForm = (ProjectSummaryForm) form;
        try {
            ...
            HashMap parameters = new HashMap();
            // Le nom de la pratique
            parameters.put("practiceName", WebMessages.getString(request, ((ResultRulesCheckingForm) theForm.getResults()).getName()));
            // Note de la pratique
            ...
            PDFDataJasperReports pdfData =
                new PDFDataJasperReports(request.getLocale(), getResources(request), data, "/com/airfrance/squaleweb/resources/jasperreport/transgressions.jasper", false, parameters);
            PDFFactory.generatePDFToHTTPResponse(pdfData, response, "", PDFEngine.JASPERREPORTS);
        } catch (Exception e) {
            throw new ServletException(e);
        }
     ...
     */
    private Collection fillTransgressions(Collection pRulesCheckingForm) throws JrafEnterpriseException, WTransformerException {
        // Initialisation
        ...
        for (Iterator it = pRulesCheckingForm.iterator(); it.hasNext();) {
            ...
            if (current.getTransgressionsNumber() > 0) {
                ...
                itemsForm = (RuleCheckingItemsListForm) WTransformerFactory.objToForm(RuleCheckingItemsListTransformer.class, new Object[] { current.getNameRule(), details });
                formForPDF = new RuleCheckingPDFForm(current.getNameRule(), current.getSeverity(), current.getTransgressionsNumber(), itemsForm);
                data.add(formForPDF);
            }
        }
        return data;
