 ...
 */
public class JCompilingConfTransformer implements WITransformer {
    ...
    public void objToForm(Object[] pObject, WActionForm pForm) throws WTransformerException {
        ...
        StringParameterDTO bundlePath = (StringParameterDTO) params.getParameters().get(ParametersConstants.BUNDLE_PATH);
        if (null != bundlePath) {
        ...
        if (null != eclipseParams) {
            StringParameterDTO isEclipseCompilation = (StringParameterDTO) eclipseParams.getParameters().get(ParametersConstants.ECLIPSE_COMPILATION);
            if (null != isEclipseCompilation && isEclipseCompilation.getValue().equals("false")) {
                ...
                MapParameterDTO eclipseVars = (MapParameterDTO) eclipseParams.getParameters().get(ParametersConstants.ECLIPSE_VARS);
                List vars = new ArrayList();
                if (null != eclipseVars) {
                    for (Iterator it = eclipseVars.getParameters().keySet().iterator(); it.hasNext();) {
                        String varName = (String) it.next();
                        vars.add(new EclipseVarForm(varName, ((StringParameterDTO) eclipseVars.getParameters().get(varName)).getValue()));
                    }
                }
                jCompilingForm.setEclipseVars(vars);
                // Les librairies eclipse
                MapParameterDTO eclipseLibs = (MapParameterDTO) eclipseParams.getParameters().get(ParametersConstants.ECLIPSE_LIBS);
    ...
    /**
     * Retourne la liste des règles de compilation remplie avec
     * les règles ant, wsad ou RSA
     * 
    ...
    private List fillWsadCompilationRules(MapParameterDTO params, ListParameterDTO pWsadParams) {
        ...
        while (wsadIt.hasNext()) {
            ...
            MapParameterDTO projects_params = (MapParameterDTO) params.getParameters().get(ParametersConstants.WSAD_PROJECT_PARAM);
            if (projects_params != null) {
                MapParameterDTO project_param = (MapParameterDTO) projects_params.getParameters().get(ruleForm.getWorkspacePath());
                if (project_param != null) {
                    // On récupère le manifest
                    StringParameterDTO manifestParam = (StringParameterDTO) project_param.getParameters().get(ParametersConstants.MANIFEST_PATH);
    ...
    private List fillRsaCompilationRules(ListParameterDTO pRsaParams) {
        ...
        while (rsaIt.hasNext()) {
            ...
            StringParameterDTO path = (StringParameterDTO) projectList.getParameters().get(ParametersConstants.WORKSPACE_ID);
            ...
            if (projectList.getParameters().size() > ParametersConstants.EAR_NAME_ID) {
                StringParameterDTO earNameParam = (StringParameterDTO) projectList.getParameters().get(ParametersConstants.EAR_NAME_ID);
                ruleForm.setEarName(earNameParam.getValue());
            }
            if (projectList.getParameters().size() > ParametersConstants.MANIFEST_PATH_ID) {
                StringParameterDTO manifestParam = (StringParameterDTO) projectList.getParameters().get(ParametersConstants.MANIFEST_PATH_ID);
                ruleForm.setManifestPath(manifestParam.getValue());
            }
            rules.add(ruleForm);
        }
