...
/**
 * Actions sur le référentiel
 * Les actions sur le référentiel permettent de consulter les facteurs et les métriques
 * de certaines applications mises en référence
 */
public class ReferenceAction extends AdminAction {

    /**
      * Ajout ou retrait de projets du référentiel
      * Les projets sélectionnés sont ajoutés ou retirés du référentiel, puis les données affichées
      * sont mise à jour
      ...
      */
    public ActionForward updateReferentiel(ActionMapping pMapping, ActionForm pForm, HttpServletRequest pRequest, HttpServletResponse pResponse) {
        // Initialisation
        ...
        try {
            ...
            for(int i=0; i<referenceListForm.getList().size(); i++) {
                Iterator refsIt = ((ReferenceListForm)((ReferenceGridForm) referenceListForm.getList().get(i)).getReferenceListForm()).getList().iterator();
                ...
                while (refsIt.hasNext()) {
                    ...
                    if (refForm.isSelected()) {
                        SqualeReferenceDTO refDTO = (SqualeReferenceDTO) WTransformerFactory.formToObj(ReferenceTransformer.class, refForm)[0];
