 ...
 */
public class ResultsHeaderTag extends TagSupport {

    /** Le nom du formulaire */
    private String mName;
    /** 
     * Indique si il faut afficher l'information sur la compararaison des audits 
     * La comparaison entre audits n'est pas possible si la grille sur le projet n'est pas là même
     * (i.e. pas le même id en base) ou si les audits n'ont pas été fait sur les même projets. 
     */
    ...
    public int doStartTag() throws JspException {
        ...
        param[0] = "<a href='/squale/application.do?action=summary&applicationId=" + appliId + "'>" + (String) RequestUtils.lookup(pageContext, mName, "applicationName", null) + "</a>";
        ...
        if (null != param[0] && param[0].length() > 0) {
            param[0] = "<a href='/squale/project.do?action=select&projectId=" + projectId + paramsLink + "'>" + param[0] + "</a>";
            blockquote.append("<b>");
            blockquote.append(WebMessages.getString(locale, "description.name.project", param));
            blockquote.append("</b> <br />");
        }
        // Le nom de l'audit courant
        param[0] = "<a href='/squale/audits.do?action=select&currentAuditId=" + auditId + "'>" + (String) RequestUtils.lookup(pageContext, mName, "auditName", null) + "</a>";
        ...
        if (null == param[0] || param[0].length() == 0) {
            param[0] = WebMessages.getString(((HttpServletRequest) pageContext.getRequest()), "project.practices.list.previousAudit.unknown");
            mDisplayComparable = false; // il n'y a pas d'audit précédent
        } else {
            // On ajoute un lien
            // lien idem à la sélection de l'audit
            param[0] = "<a href='/squale/audits.do?action=select&currentAuditId=" + previousAuditId + "'>" + param[0] + "</a>";
        }
        blockquote.append("<b>");
