...
/**
 * Action abstraite sur les rulesets
 * Les actions sur les rulesets permettent de lister ceux-ci ou
 * de purger certains d'entre eux.
 * Cette classe factorise le comportement identique sur ces aspects
 */
public abstract class AbstractRuleSetAction extends AdminAction {
    /**
    ...
    public ActionForward purge(ActionMapping pMapping, ActionForm pForm, HttpServletRequest pRequest, HttpServletResponse pResponse) {
        ...
        try {
            ...
            Collection ruleSets = (Collection) WTransformerFactory.formToObj(getRuleSetListTransformer(), (WActionForm) pForm)[0];
            if (ruleSets.isEmpty()) {
                ...
                if (usedRuleSets.size() > 0) {
                    ActionMessage error = new ActionError("rulechecking.rulesetused", buildListStr(pRequest.getLocale(), usedRuleSets));
                    errors.add(ActionErrors.GLOBAL_ERROR, error);
                }
                // On récupère les rulesets supprimés
                ruleSets.removeAll(usedRuleSets);
                if (ruleSets.size() > 0) {
                    ActionMessage error = new ActionError("rulechecking.rulesetdeleted", buildListStr(pRequest.getLocale(), ruleSets));
                    errors.add(ActionErrors.GLOBAL_ERROR, error);
                }
    ...
    private String buildListStr(Locale pLocale, Collection pRuleSets) {
        ...
        for (Iterator it = pRuleSets.iterator(); it.hasNext();) {
            ...
            String formattedDate = SqualeWebActionUtils.getFormattedDate(pLocale, dto.getDateOfUpdate(), "datetime.format.simple");
            message += "<li>" + WebMessages.getString(pLocale, "rulechecking.confirmation.item", new String[] {dto.getName(), formattedDate});
        }
        message += "</ul>";
        return message;
    }
