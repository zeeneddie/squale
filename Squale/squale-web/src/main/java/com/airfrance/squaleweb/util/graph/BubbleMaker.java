 ...
 * @author 6370258
 *
 * Génération du graphe de type Bubble,
 * stocké dans un form et appelé depuis une page Jsp
 */
public class BubbleMaker extends AbstractGraphMaker {
    ...
    /**
     * Constructeur avec juste la position des axes
     * Permet de factoriser le traitement du cas null
     * @param pHorizontalAxisPos la position de l'axe horizontal
     * @param pVerticalAxisPos la position de l'axe vertical
     */
     ...
     */
    public BubbleMaker(String pTitle, String pDomainAxisLabel, String pValueAxisLabel, Long pHorizontalAxisPos, Long pVerticalAxisPos) {
        this(pHorizontalAxisPos,pVerticalAxisPos);
        mTitle = pTitle;
        mXLabel = pDomainAxisLabel;
        mYLabel = pValueAxisLabel;
     ...
     * @param pVerticalAxisPos la position de l'axe des ordonées
     */
    public BubbleMaker(Locale pLocale, String pTitle, String pDomainAxisLabel, String pValueAxisLabel, Long pHorizontalAxisPos, Long pVerticalAxisPos) {
            ...
            for (int i = 0; i < pTotal.length; i++) {
                ...
                if (radius <= DEFAULT_LOWER_LIMIT) {
                    ...
                    if (radius >= DEFAULT_UPPER_LIMIT) {
                        ...
                        pSize[i] =
                            ((double) (radius - DEFAULT_LOWER_LIMIT) * (double) (DEFAULT_UPPER_SIZE - DEFAULT_LOWER_SIZE) / (double) (DEFAULT_UPPER_LIMIT - DEFAULT_LOWER_LIMIT)) + DEFAULT_LOWER_SIZE;
                    }
                }
            }
    ...
    /**
     * Selon la valeur du couple (vg, evg)
     * calcule la distribution des méthodes dans les quatre coins du graphes en fonction des critères
     * - Maintenable et structuré
     * - Maintenable mais mal structuré
     * - Difficilement maintenable mais structuré
     * - Difficilement maintenable et mal structuré 
     ...
     * @return le diagramme JFreeChart.
     */
    public JFreeChart getChart(String pProjectId, String pCurrentAuditId, String pPreviousAuditId, long[] pComponentId, double[] pVgs, double[] pEvgs, double[] pTotal) {
        JFreeChart retChart = super.getChart();
        if (null == retChart) {
            ...
            BubbleUrlGenerator generator = new BubbleUrlGenerator(pProjectId, pCurrentAuditId, pPreviousAuditId, pComponentId);
    ...
    private JFreeChart getCommonChart() {
        ...
        JFreeChart chart = ChartFactory.createBubbleChart(mTitle, mXLabel, mYLabel, mDataSet, PlotOrientation.VERTICAL, mShowLegend, true, false);
    ...
    /**
     * Affichage sous la forme de sous-titres de la répartition des méthodes
     * - Maintenable et structuré,
     * - Maintenable mais mal structuré
     * - Difficilement maintenable mais structuré
     * - Difficilement maintenable et mal structuré
    ...
    private void displayRepartitionSubtitles(JFreeChart pChart) {
        ...
        // Ajout du sous-titre de répartition des méthodes avec le pourcentage correspondant : partie supérieure, coin gauche et droit
        StringBuffer stringBufferTop = new StringBuffer();
        stringBufferTop.append(WebMessages.getString(locale, "bubble.project.subtitle.topLeft") + " " + numberFormat.format(percentTopLeft) + "%     ");
        stringBufferTop.append(WebMessages.getString(locale, "bubble.project.subtitle.topRight") + " " + numberFormat.format(percentTopRight) + "%");
        TextTitle subtitleTop = new TextTitle(stringBufferTop.toString());

        // Ajout du sous-titre de répartition des méthodes avec le pourcentage correspondant : partie inférieure, coin gauche et droit   
        StringBuffer stringBufferBottom = new StringBuffer();
        stringBufferBottom.append(WebMessages.getString(locale, "bubble.project.subtitle.bottomLeft") + " " + numberFormat.format(percentBottomLeft) + "%     ");
        stringBufferBottom.append(WebMessages.getString(locale, "bubble.project.subtitle.bottomRight") + " " + numberFormat.format(percentBottomRight) + "%");
