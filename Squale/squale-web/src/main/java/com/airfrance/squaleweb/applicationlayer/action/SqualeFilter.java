...
/**
 * Filtre Squale Ce filtre permet de s'assurer que l'on ne sert pas une URL si la session n'est pas correctement
 * renseignée Dans ce cas on redirige vers la racine du site
 */
public class SqualeFilter
    implements Filter
{
    /** URL racine */
    ...
    /**
     * @see javax.servlet.Filter#void () {@inheritDoc}
     */
    public void destroy()
    {
    ...
    /**
     * @see javax.servlet.Filter#void (javax.servlet.ServletRequest, javax.servlet.ServletResponse,
     *      javax.servlet.FilterChain) {@inheritDoc}
        ...
        throws ServletException, IOException
    {
        ...
        if ( ( request.getSession().getAttribute( WConstants.USER_KEY ) == null )
            && ( !request.getRequestURL().toString().endsWith( ROOT_URL ) ) )
        {
            // Dans ce cas on vérifie qu'on ne vient pas de la porlet sinon on fait un traitement spécifique
            if ( request.getRequestURL().toString().endsWith( PORTLET_URL )
                || null != request.getParameter( "fromPortlet" ) )
            {
