 ...
 */
public class UMLQualityConfTransformer implements WITransformer {
    ...
    public void objToForm(Object[] pObject, WActionForm pForm) throws WTransformerException {
        ...
        MapParameterDTO umlQualityParams = (MapParameterDTO) params.getParameters().get(ParametersConstants.UMLQUALITY);
        if(null != umlQualityParams) {
            StringParameterDTO xmiFileParam = (StringParameterDTO) umlQualityParams.getParameters().get(ParametersConstants.UMLQUALITY_SOURCE_XMI);
            String xmiFilePath =  xmiFileParam.getValue();
            umlQualityForm.setXmiFile(xmiFilePath);
            // excludedClasses
            ListParameterDTO classesDTO = (ListParameterDTO) umlQualityParams.getParameters().get(ParametersConstants.MODEL_EXCLUDED_CLASSES);
            if (classesDTO != null) {
