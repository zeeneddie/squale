...
/**
 */
public class AbstractRepartitionUrlGenerator extends AbstractURLGenerator {
    ...
    /** 
     * Le nombre de batonnets, varie en fonction de la formule associée
     * à la pratique
     */
    ...
    public String generateURL(Object pDataSet, int pSerieIndex, int pCategory) {
        ...
        double minMark = ((int) (coeff * ((double) pSerieIndex) / ((double) nbSeries / (double) PracticeResultBO.EXCELLENT))) / coeff;
        double maxMark = Math.round((float)coeff * (minMark + ((double) PracticeResultBO.EXCELLENT / nbSeries))) / coeff;
        url = "mark.do?action=mark&projectId=" + mProjectId + "&currentAuditId=" + mCurrentAuditId 
        + "&previousAuditId=" + mPreviousAuditId 
        + "&tre=" + practiceId + "&minMark=" + minMark + "&maxMark=" + maxMark;
