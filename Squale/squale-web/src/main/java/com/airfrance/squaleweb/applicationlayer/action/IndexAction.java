...
public class IndexAction extends DefaultAction {
    ...
    public ActionForward execute(ActionMapping pMapping, ActionForm pForm, HttpServletRequest pRequest, HttpServletResponse pResponse) {
        ...
        try {
            ...
            ArrayList userApplicationsDTO = (ArrayList) WTransformerFactory.formToObj(ApplicationListTransformer.class, appliForm)[0];
            ...
            if (!user.isAdmin()) {
                ...
                Integer maxAccesses = new Integer(Integer.parseInt(WebMessages.getString(pRequest, "application.max.accesses")));
    ...
    public boolean initUserSession(ActionForm pForm, HttpServletRequest pRequest) {
        ...
        try {
            ...
            ApplicationListForm appliForm = new ApplicationListForm();
            appliForm.setList((ArrayList) applications);
            Object[] paramIn = {(ArrayList) WTransformerFactory.formToObj(ApplicationListTransformer.class, appliForm)[0], cal.getTime(), new Boolean(true)};
            if (displayAllAudits) {
                paramIn[1] = null;
            }
            Collection audits = (Collection) ac.execute("getAllAuditsAfterDate", paramIn);
