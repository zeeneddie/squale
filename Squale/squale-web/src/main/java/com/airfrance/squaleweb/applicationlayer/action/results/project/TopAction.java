 ...
 * Fenêtre&gt;Préférences&gt;Java&gt;Génération de code&gt;Code et commentaires
 */
public class TopAction extends ReaderAction {
     ...
     * @return l'action à réaliser.
     */
    public ActionForward displayBubble(ActionMapping pMapping, ActionForm pForm, HttpServletRequest pRequest, HttpServletResponse pResponse) {
        ActionForward forward = null;
        ActionErrors errors = new ActionErrors();
        try {
            ...
            String fileNameBubble = ServletUtilities.saveChartAsPNG(chartBubble, BubbleMaker.DEFAULT_WIDTH, BubbleMaker.DEFAULT_HEIGHT, infoBubble, pRequest.getSession());
     ...
     */
    public JFreeChart getBubbleChart(HttpServletRequest pRequest) throws JrafEnterpriseException {
        ...
        if (bubbleMaker == null
            || !projectId.toString().equals(bubbleMaker.getProjectId())
            ...
            if (null != inputParams) {
                ...
                bubbleMaker = new BubbleMaker(pRequest.getLocale(), (Long) inputParams[indexInParam++], (Long) inputParams[indexInParam++]);
                bubbleMaker.addSerie((String) inputParams[indexInParam++], (double[]) inputParams[indexInParam++], (double[]) inputParams[indexInParam++], (double[]) inputParams[indexInParam++]);
                chartBubble =
                    bubbleMaker.getChart(
                        projectId.toString(),
                        currentAuditId.toString(),
                        previousAuditId.toString(),
                ...
                chartBubble = bubbleMaker.getChart(projectId.toString(), currentAuditId.toString(), previousAuditId.toString(), new long[0], new double[0], new double[0], new double[0]);
            }
            // On met en session
    ...
    private void getResults(final HttpServletRequest pRequest, final TopListForm pTopListForm) throws Exception {
        ...
        Object[] paramIn = { project, pTopListForm.getComponentType(), audit, pTopListForm.getTre(), new Integer(WebMessages.getString(pRequest.getLocale(), "component.max"))};
     ...
     */
    public ActionForward exportToExcel(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ServletException {
        ...
        try {
            ...
            ExcelDataTopList data = new ExcelDataTopList(request.getLocale(), getResources(request), (TopListForm) form, logon.getMatricule());
    ...
    * @throws ServletException exception pouvant etre levee
    */
    public ActionForward exportToPDF(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws ServletException {
        TopListForm theForm = (TopListForm) form;
        Collection data = theForm.getComponentListForm();
        try {
            HashMap parameters = new HashMap();
            PDFDataJasperReports pdfData = new PDFDataJasperReports(request.getLocale(), getResources(request), data, "/com/airfrance/squaleweb/resources/jasperreport/tops.jasper", false, parameters);
